...
internal readonly struct XjFamilyDomainEvent
{
	...
	internal static XjFamilyDomainEvent JinDanSucceeded(Actor actor, string guoWei, string daoTu, string actualRealmId = "")
	{
		string realmId = XjRealmHelper.NormalizeId(actualRealmId);
		if (string.IsNullOrWhiteSpace(realmId)) realmId = XjRealmIds.JinDan;
		return Create(actor, TypeJinDanSucceeded, "JinDan", realmId, string.Empty, 0, string.Empty, string.Empty, 0, string.Empty, string.Empty, daoTu, guoWei);
	}
		...
		string boundAuthority)
	{
		...
		if (XjSectAuthorityStore.TryGetSectId(actorId, out long authoritativeSectId)
			&& XjSectRepository.TryGetBySectId(authoritativeSectId, out XjSectArchiveRecord sect)
			&& sect != null)
		{
			zongMenId = authoritativeSectId;
			zongMenName = (sect.Name ?? string.Empty).Trim();
		}
	...
	{
		// Family-domain events are often emitted from a deferred annual actor transaction.
		// The live world may already be several years ahead when that transaction finally
		// executes at high speed. Preserve the transaction's semantic year so one Jindan
		// success cannot be recorded as 501 in the core history and 509 in the family book.
		return XjAnnualExecutionContext.ResolveYear(actor);
	}
}
