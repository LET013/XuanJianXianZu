...
internal static class XjFaBaoCatalog
{
	...
	internal const string XianQiClass = "仙器";
	...
	// 仙器不是新的装备槽，而是道胎以五百年为一轮温养本命金丹法宝后，
	// 极低概率出现的同器升格。基础档只作为“无词条旧档/外部编辑器”兜底；
	// 正常升格会保留并强化原法宝词条。
	internal static readonly XjFaBaoBonusProfile XianQiBonus = new XjFaBaoBonusProfile(
		0.25f,
		0.35f,
		0.40f,
		0.20f,
		...
		sameRealmDamageBonus: 0.25f,
		shieldBreakBonus: 0.25f,
		breakthroughChanceBonus: 0.10f,
		trueDamageRatio: 0.12f);
	...
	internal static readonly string[] DefenseEquipmentKinds = { "灵盔", "灵甲", "灵履", "灵戒", "灵链", "灵符" };
	...
	{
		"每秒回血", "修炼速度", "果位意象", "命数加成", "道慧加成", "寿命增加", "突破概率"
	};
	...
	private static readonly string[] BingGuWords =
	{
		"万古", "同庭", "古庭", "玄巫", "玉衡", "青宣", "全丹", "执孛", "司天", "都卫",
		"宿符", "古篆", "方镇", "天序", "玄祝", "玉庭", "古仪", "丹性", "巫庭", "天纲"
	};
	...
	internal static string ResolveDaoTuCategory(string daoTu)
	{
		...
		return value switch
		{
			"并古" or "鸺葵" or "上巫" or "玉真" or "衡祝" or "青宣" or "全丹" or "执孛" or "司天" or "都卫" => "并古",
			"太阳" or "少阳" or "明阳" => "三阳",
			"太阴" or "少阴" or "厥阴" => "三阴",
			"玄雷" or "霄雷" or "元雷" => "三雷",
			"兑金" or "庚金" or "齐金" or "库金" or "逍金" or "长庚" => "金德",
			"角木" or "正木" or "集木" or "更木" or "保木" => "木德",
			"坎水" or "渌水" or "合水" or "府水" or "牝水" => "水德",
			"离火" or "并火" or "真火" or "灴火" or "牡火" => "火德",
			// 虹霞虽是独立道途，但器物仍承薛畋借戊土空证的土德根基。
			"艮土" or "戊土" or "宣土" or "归土" or "宝土" or "虹霞" => "土德",
			_ => "十二炁"
		};
	}
	...
	internal static bool IsXianQi(string className)
	{
		return string.Equals(className, XianQiClass, StringComparison.Ordinal);
	}

	/// <summary>
	/// 兼容口径：仙器由金丹本命法宝同器升格，所有“金丹法宝及以上”的保护、
	/// 归属和本命锁定逻辑都应继续成立。需要判断“尚未升格的纯金丹法宝”时，
	/// 请直接与 JinDanFaBaoClass 做精确比较。
	/// </summary>
	...
	{
		bool isXianQi = IsXianQi(className);
		int maxCount = isXianQi ? 7 : IsJinDanFaBao(className) ? 5 : IsZhuJiFaQi(className) ? 1 : 3;
		float maxPercent = isXianQi ? 50f : IsJinDanFaBao(className) ? 30f : IsZhuJiFaQi(className) ? 3f : 10f;
	...
	private static string NormalizeAffixes(string affixes, string role, int maxCount, float maxPercent)
	{
		...
		for (int i = 0; i < raw.Length && result.Count < maxCount; i++)
		{
			...
			else if (string.Equals(label, "辉光加成", StringComparison.Ordinal)) label = "道慧加成";
