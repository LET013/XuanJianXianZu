...
{
	private const string EvidenceModDynamicName = "玄鉴动态名录规则";
	...
	internal static string FormatStatus(XjCanonStatus status)
	{
		return status switch
		{
			...
			XjCanonStatus.ModInterpretation => "道统补录",
			_ => "待校核"
		};
	}
	...
	private static void RegisterDynamicGenerators()
	{
		...
		AddRecord(XjCanonCategory.GuoWei, "generated:guowei", "果位名生成器", XjCanonStatus.InferredCanon, "依道途与果、余、闰位体系组合生成", "Systems/HighRealm/XjGuoWeiCalculator.cs", "果位名由既有道途和位格规则推导，不复制每个运行时果位实例。", true);
	}
	...
	private static void RegisterCaiQiBranches()
	{
		for (int i = 0; i < XjCaiQiCatalog.Entries.Length; i++)
		{
			...
			bool isBingGu = string.Equals(entry.PlaceTypeId, XjCaiQiPlaceTypeIds.BingGu, StringComparison.Ordinal);
			AddRecord(
				XjCanonCategory.DaoTu,
				entry.DisplayName,
				entry.DisplayName,
				isBingGu ? XjCanonStatus.ModInterpretation : XjCanonStatus.InferredCanon,
				isBingGu
					? "并古道途名称沿用既有道途登记；本批采气地、先天之气与采气法名为玄鉴玩法补足"
					: "采气分支沿既有道途与十二炁体系整理，旧式资源映照只作兼容",
				"Data/CaiQi/XjCaiQiDefinitions.cs",
				isBingGu
					? "并古道途本身不改名；对应采气资源用于补齐紫府金丹修法的采气入口。"
	...
	{
		AddRecord(XjCanonCategory.FaBao, XjFaBaoCatalog.ZhuJiFaQiClass, XjFaBaoCatalog.ZhuJiFaQiClass, XjCanonStatus.ModInterpretation, "法宝阶类与现世规则映照", "Data/FaBao/XjFaBaoCatalog.cs", "只登记阶类标签，不复制加成曲线。");
		AddRecord(XjCanonCategory.FaBao, XjFaBaoCatalog.ZiFuLingBaoClass, XjFaBaoCatalog.ZiFuLingBaoClass, XjCanonStatus.ModInterpretation, "法宝阶类与现世规则映照", "Data/FaBao/XjFaBaoCatalog.cs", "只登记阶类标签，不复制加成曲线。");
		AddRecord(XjCanonCategory.FaBao, XjFaBaoCatalog.JinDanFaBaoClass, XjFaBaoCatalog.JinDanFaBaoClass, XjCanonStatus.ModInterpretation, "法宝阶类与现世规则映照", "Data/FaBao/XjFaBaoCatalog.cs", "只登记阶类标签，不复制加成曲线。");
	}
	...
	private static void RegisterJinDanDaoSpells()
	{
		for (int i = 0; i < XjJinDanDaoSpellCatalog.All.Count; i++)
		{
			...
			AddRecord(
				...
				spell.DisplayName,
				XjCanonStatus.ModInterpretation,
				"金丹道术战斗目录；记述施法间隔、威势范围、显化与实际斗法效果",
	...
	{
		RegisterXianJiDictionary("DocumentUpperXianJiMap", XjCanonStatus.InferredCanon, "上品仙基池依既有文档与原著体系推定");
		RegisterXianJiDictionary("DocumentLowerXianJiMap", XjCanonStatus.InferredCanon, "下品仙基池依既有文档与原著体系推定");
		RegisterXianJiDictionary("DaoTuXianJiMap", XjCanonStatus.ModInterpretation, "现行上品仙基池；未入文档池的补足项按玄鉴玩法演绎处理");
		RegisterXianJiDictionary("LowDaoTuXianJiMap", XjCanonStatus.ModInterpretation, "现行下品仙基池；未入文档池的补足项按玄鉴玩法演绎处理");
	}
	...
	private static void RegisterGuoWeiAndQuanBing()
	{
		...
		for (int i = 0; i < daoTus.Count; i++)
		{
			...
			AddRecord(XjCanonCategory.GuoWei, XjGuoWeiCalculator.BuildGuoWeiSlotName(daoTu, XjGuoWeiCalculator.ZhengWei, 1), XjGuoWeiCalculator.BuildGuoWeiSlotName(daoTu, XjGuoWeiCalculator.ZhengWei, 1), XjCanonStatus.InferredCanon, "依道途与果位规则生成", "Systems/HighRealm/XjGuoWeiCalculator.cs", "固定道途果位显示名；闰位/余位多槽实例由 generated:guowei 统一处理。");
	...
	{
		AddRecord(XjCanonCategory.DongTian, "SecretRealmStage:Fudi", "福地", XjCanonStatus.ModInterpretation, "宗门秘境工程的固定阶段", "Data/DongTian/XjSecretRealmData.cs", "阶段标签，不复制宗门工程状态。");
		AddRecord(XjCanonCategory.DongTian, "SecretRealmStage:Dongtian", "洞天", XjCanonStatus.ModInterpretation, "宗门秘境工程的固定阶段", "Data/DongTian/XjSecretRealmData.cs", "阶段标签，不复制宗门工程状态。");
		AddRecord(XjCanonCategory.DongTian, "SecretRealmStage:SecretRealm", "秘境", XjCanonStatus.ModInterpretation, "宗门秘境工程的固定阶段", "Data/DongTian/XjSecretRealmData.cs", "秘境显示类别，不复制宗门工程状态。");
	}
