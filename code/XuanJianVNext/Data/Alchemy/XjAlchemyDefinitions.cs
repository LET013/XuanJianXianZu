...
internal enum XjAlchemyCultivationPath
{
	Universal = 0,
	ZiJin = 1,
	FuQi = 2
...
internal enum XjAlchemyEffectType
{
	...
	SustainedZhenYuan = 8,
	FuQiBodyProjectYears = 9,
	FuQiPerfectionProjectYears = 10,
	FuQiJinXingNurtureYears = 11
}
...
internal sealed class XjAlchemyPillDef
{
	...
	internal XjAlchemyCultivationPath PathAffinity { get; }
	...
	internal XjAlchemyPillDef(
		...
		XjAlchemyCultivationPath pathAffinity)
	{
		Id = id ?? string.Empty;
		NameKey = nameKey ?? string.Empty;
		DescriptionKey = descriptionKey ?? string.Empty;
...
internal static class XjAlchemyCatalog
{
	...
	internal const string PillBaoYiWenJin = "xj_alchemy_pill_bao_yi_wen_jin";
	...
	internal const string PillCunShenYangShen = "xj_alchemy_pill_cun_shen_yang_shen";
	...
	internal const string PillXingMingHeZhen = "xj_alchemy_pill_xing_ming_he_zhen";
	...
	internal const string RecipeBaoYiWenJin = "xj_alchemy_recipe_bao_yi_wen_jin";
	...
	internal const string RecipeCunShenYangShen = "xj_alchemy_recipe_cun_shen_yang_shen";
	...
	internal const string RecipeXingMingHeZhen = "xj_alchemy_recipe_xing_ming_he_zhen";
	...
	internal static readonly string[] OrderedRecipeIds =
	{
		...
		RecipeQingShen, RecipeTianYiTuCui, RecipeChengJin, RecipeBaoYiWenJin,
		RecipeSanQuan, RecipeNanGongXuanSui, RecipeCunShenYangShen,
		RecipeMingZhenHeShen, RecipeXingMingHeZhen, RecipeJiuYaoYanShou
	};
	...
	internal static void Init()
	{
		...
		AddPill(PillSheYuan, "she_yuan", XjRealmIds.TaiXi, XjRealmIds.TaiXi,
			...
			pathAffinity: XjAlchemyCultivationPath.ZiJin);
		...
		AddPill(PillChengJin, "cheng_jin", XjRealmIds.ZiFu, XjRealmIds.ZiFu,
			...
			pathAffinity: XjAlchemyCultivationPath.ZiJin);
		AddPill(PillBaoYiWenJin, "bao_yi_wen_jin", XjRealmIds.FuQiZhenRen, XjRealmIds.FuQiZhenRen,
			XjAlchemyEffectType.FuQiJinXingNurtureYears, 5f, string.Empty, 20, 40, XjAlchemyHistoryLevel.World, pill9,
			pathAffinity: XjAlchemyCultivationPath.FuQi);
		...
		AddPill(PillNanGongXuanSui, "nan_gong_xuan_sui", XjRealmIds.ZiFu, XjRealmIds.ZiFu,
			...
			pathAffinity: XjAlchemyCultivationPath.ZiJin);
		AddPill(PillCunShenYangShen, "cun_shen_yang_shen", XjRealmIds.HuangGuan, XjRealmIds.HuangGuan,
			XjAlchemyEffectType.FuQiBodyProjectYears, 6f, string.Empty, 10, 35, XjAlchemyHistoryLevel.FamilyOrZongMen, pill11,
			pathAffinity: XjAlchemyCultivationPath.FuQi);
		AddPill(PillMingZhenHeShen, "ming_zhen_he_shen", XjRealmIds.ZiFu, XjRealmIds.ZiFu,
			...
			pathAffinity: XjAlchemyCultivationPath.ZiJin);
		AddPill(PillXingMingHeZhen, "xing_ming_he_zhen", XjRealmIds.FuQiZhenRen, XjRealmIds.FuQiZhenRen,
			XjAlchemyEffectType.FuQiPerfectionProjectYears, 8f, string.Empty, 20, 40, XjAlchemyHistoryLevel.FamilyOrZongMen, pill12,
			pathAffinity: XjAlchemyCultivationPath.FuQi);
		...
		AddRecipe(RecipeBaoYiWenJin, "bao_yi_wen_jin", pill9, PillBaoYiWenJin, XjRealmIds.FuQiZhenRen,
			MaterialYuSuiZhu, 4, MaterialZiYueLan, 4, 8, 0.15f, 1, 2, 40, 5,
			"family_core_secret_or_authorized_zongmen", XjAlchemyHistoryLevel.World);
		...
		AddRecipeMulti(RecipeNanGongXuanSui, "nan_gong_xuan_sui", pill11, PillNanGongXuanSui, XjRealmIds.ZiFu,
			...
			6, 0.32f, 1, 2, 35, 4,
			"family_secret_or_authorized_zongmen", XjAlchemyHistoryLevel.FamilyOrZongMen);
		AddRecipeMulti(RecipeCunShenYangShen, "cun_shen_yang_shen", pill11, PillCunShenYangShen, XjRealmIds.FuQiZhenRen,
			new[]
			{
				new XjAlchemyIngredient(MaterialZiSuiLan, 4),
				new XjAlchemyIngredient(MaterialBingPoLian, 3),
		...
		AddRecipeMulti(RecipeMingZhenHeShen, "ming_zhen_he_shen", pill12, PillMingZhenHeShen, XjRealmIds.ZiFu,
			...
			6, 0.30f, 1, 2, 25, 4,
			"family_secret_or_authorized_zongmen", XjAlchemyHistoryLevel.FamilyOrZongMen);
		AddRecipeMulti(RecipeXingMingHeZhen, "xing_ming_he_zhen", pill12, PillXingMingHeZhen, XjRealmIds.FuQiZhenRen,
			new[]
			{
				new XjAlchemyIngredient(MaterialHanAi, 3),
				new XjAlchemyIngredient(MaterialXuanYinTeng, 3),
	...
	private static void AddPill(
		...
		XjAlchemyCultivationPath pathAffinity = XjAlchemyCultivationPath.Universal)
	{
		...
		bool productionEnabled = true)
	{
		Add(new XjAlchemyRecipeDef(
			...
			historyLevel,
			productionEnabled,
			pills.TryGetValue(pillId, out XjAlchemyPillDef pill)
				? pill.PathAffinity
				: XjAlchemyCultivationPath.Universal));
	}
	...
	private static void Validate()
	{
		...
		Dictionary<string, XjAlchemyRecipeDef> fixedFormulas = new(StringComparer.Ordinal);
		foreach (XjAlchemyRecipeDef recipe in recipes.Values)
		{
			...
			if (recipe.PathAffinity != pill.PathAffinity)
				throw new InvalidOperationException("丹方与成丹修法不一致：" + recipe.Id + " -> " + recipe.PillId);
			...
			if (fixedFormulas.TryGetValue(formula, out XjAlchemyRecipeDef existing))
			{
				if (!IsApprovedCrossPathPair(existing.Id, recipe.Id)
					|| existing.PathAffinity == recipe.PathAffinity
					|| !string.Equals(existing.IconPath, recipe.IconPath, StringComparison.Ordinal))
				{
					throw new InvalidOperationException("丹方药材组合重复，且不是批准的双修法同图配对：" + recipe.Id);
				}
			}
			else
			{
				fixedFormulas[formula] = recipe;
			}
		}
	...
	private static bool IsApprovedCrossPathPair(string left, string right)
	{
		return IsPair(left, right, RecipeChengJin, RecipeBaoYiWenJin)
			|| IsPair(left, right, RecipeNanGongXuanSui, RecipeCunShenYangShen)
			|| IsPair(left, right, RecipeMingZhenHeShen, RecipeXingMingHeZhen);
	}

	private static bool IsPair(string left, string right, string first, string second)
	{
		return string.Equals(left, first, StringComparison.Ordinal) && string.Equals(right, second, StringComparison.Ordinal)
			|| string.Equals(left, second, StringComparison.Ordinal) && string.Equals(right, first, StringComparison.Ordinal);
	}
}
