...
internal partial class XjVNextPatches
{
	...
	private static bool XuanJianVNext_ActorEquipmentSlot_TakeAwayItem_BoundPrimaryGuard_Prefix(ActorEquipmentSlot __instance)
	{
		...
		return equipped == null || !XjEquipmentForgeConsumer.IsBoundPrimaryRemovalLocked(equipped);
	}
	...
	private static void XuanJianVNext_ActorEquipmentSlot_SetItem_FaBaoClaim_Postfix(
		...
		[HarmonyArgument("pActor")] Actor actor)
	{
		if (__instance == null || item == null || actor?.data == null)
		{
			return;
		}

		if (ReferenceEquals(__instance.getItem(), item))
		{
			XjEquipmentForgeConsumer.ClaimUnownedFaBao(item, actor);
			int year = 0;
			try { year = World.world?.map_stats?.year ?? 0; } catch { }
