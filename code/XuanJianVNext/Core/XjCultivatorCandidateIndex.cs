...
/// aptitude holder. Realm-entry sets serve world systems; progression state
...
internal static class XjCultivatorCandidateIndex
{
	...
	// Diagnostic-only sum. Sets may overlap; zero is the only invariant that matters.
	internal static int RuntimeMembershipCount => RealmEnteredIds.Count
		+ ProgressionStates.Count + DirtyProgressionIds.Count + ZhenYuanChangedIds.Count
		+ ZiFuProgressionIds.Count + JinDanProgressionIds.Count;
	...
	/// <summary>
	/// Releases arrays materialized only for read-side iteration. The authoritative
	/// candidate sets and progression states remain intact and rebuild these
	/// snapshots lazily on the next query.
	/// </summary>
	internal static void ReleaseSnapshotCaches()
	{
		realmEnteredSnapshot = Array.Empty<long>();
		realmEnteredSnapshotDirty = RealmEnteredIds.Count > 0;
	}
	...
	private static XjProgressionCandidateState BuildProgressionState(Actor actor, long actorId, int currentYear)
	{
		...
		bool daoZhu = false;
		try { daoZhu = actor.hasTrait("ChuShen8"); } catch (System.Exception xjCaught514) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Core/XjCultivatorCandidateIndex.cs:514", xjCaught514); }
	...
	private static int ResolveNextBreakthroughYear(Actor actor, string targetRealmId, int currentYear, bool daoZhu)
	{
		...
		if (!daoZhu)
		{
			...
			int age = 0;
			try { age = (int)Math.Floor(Math.Max(0f, actor.getAge())); } catch (System.Exception xjCaught612) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Core/XjCultivatorCandidateIndex.cs:612", xjCaught612); }
			if (minimumAge > age)
			{
	...
	private static bool IsProgressionKey(string key)
	{
		return key == XjActorDataKeys.XjZz
			...
			|| key == XjActorDataKeys.CultivationPath
			...
			|| key == XjActorDataKeys.QingXuanRaisedShenTongMask
			|| key == XjActorDataKeys.QingXuanUpliftTargetId
			|| key == XjActorDataKeys.QingXuanUpliftStartYear
			|| key == XjActorDataKeys.QingXuanUpliftCompleteYear
			|| key == XjActorDataKeys.QingXuanFiveShenTongReady
			|| key == XjActorDataKeys.QingXuanFiveShenTongReadyYear;
	}
