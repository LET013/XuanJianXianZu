...
internal static class XjStageZeroObservation
{
	...
	private sealed class CensusState
	{
		...
		internal readonly int[] GongFaGrades = new int[8];
	...
	internal static void OnYearChanged(int currentYear, int annualQueue, int maintenanceQueue, int annualSweep, int seedQueue)
	{
		...
		int xuanJianYear = XjChronology.ToXuanJianYear(currentYear);
		bool isCenturyBoundary = xuanJianYear > 0 && xuanJianYear % CensusIntervalYears == 0;
	...
	internal static void RecordAptitudeResult(Actor actor, in XjAptitudeExecuteResult result)
	{
		...
		int age = 0;
		try { age = (int)Math.Floor(Math.Max(0f, actor.getAge())); } catch (System.Exception xjCaught493) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Core/XjStageZeroObservation.cs:493", xjCaught493); }
	...
	private static void ObserveActor(Actor actor, long actorId, CensusState census)
	{
		...
		gongFaGrade = Math.Clamp(gongFaGrade, 0, 7);
	...
	private static string ResolvePrimaryBlocker(Actor actor, string realmId, int currentYear)
	{
		...
		bool daoZhu = false;
		try { daoZhu = actor.hasTrait("ChuShen8"); } catch (System.Exception xjCaught674) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Core/XjStageZeroObservation.cs:674", xjCaught674); }
		...
		int age = 0;
		try { age = (int)Math.Floor(Math.Max(0f, actor.getAge())); } catch (System.Exception xjCaught681) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Core/XjStageZeroObservation.cs:681", xjCaught681); }
	...
	{
		return XjBreakthroughRules.CanAttemptByAptitude(xjZz, targetRealmId);
	}
	...
	private static int ResolveRealmIndex(string normalizedRealmId, string rawRealmId)
	{
		...
		if (string.Equals(normalizedRealmId, XjRealmIds.DaoTai, StringComparison.Ordinal)
			|| string.Equals(normalizedRealmId, XjRealmIds.FuQiDaoTai, StringComparison.Ordinal)) return 6;
		if (string.Equals(rawRealmId, XjRealmIds.ShenDan, StringComparison.Ordinal)) return 5;
		return 0;
	}
