...
internal static class XjYearTracker
{
    ...
    /// </summary>
    public static bool HasYearChangedSince(int lastYear)
    {
        return _currentYear > lastYear;
    }

    /// <summary>
    /// 检测当前年份是否在某年份之后
    /// </summary>
    public static bool IsAfterYear(int year)
    {
    ...
    /// 获取actor的年龄（世界年）
    /// </summary>
    public static int GetActorAgeYears(Actor actor)
    {
        if (actor == null) return 0;
        try
        {
            return Mathf.FloorToInt(Mathf.Max(0f, actor.getAge()));
        }
        catch
        {
            return 0;
