...
internal enum XjDetectionJob : byte
{
	...
	WorldEventCatalog = 22,
	DaoLineageMaintenance = 23,
	RuleInvariantAudit = 24,
	GuZunMaintenance = 25,
	KingdomNameNormalization = 26,
	ShiReincarnationReturns = 27,
	MemoryMaintenance = 28,
	XianGuoPolitics = 29
}
...
internal static class XjDetectionGate
{
	...
	internal const string AnnualWorldEventCatalog = "annual.world_event_catalog";
	internal const string AnnualDaoLineageMaintenance = "annual.dao_lineage_maintenance";
	internal const string AnnualRuleInvariantAudit = "annual.rule_invariant_audit";
	internal const string AnnualGuZunMaintenance = "annual.guzun_maintenance";
	internal const string AnnualKingdomNameNormalization = "annual.kingdom_name_normalization";
	internal const string AnnualShiReincarnationReturns = "annual.shi_reincarnation_returns";
	internal const string AnnualMemoryMaintenance = "annual.memory_maintenance";
	internal const string AnnualXianGuoPolitics = "annual.xianguo_politics";
	...
	{
		int intervalYears = ResolveAnnualIntervalYears(job);
		// 固定历法边界任务必须只在真正的 5/10/25 年边界认领门禁。
		// 若先在普通年份写入 lastYear，再由业务内部返回，会造成相位永久错开。
		if (RequiresFixedCalendarBoundary(job)
			&& (year <= 0 || year % System.Math.Max(1, intervalYears) != 0))
		{
			return false;
		}
		...
		return TryBeginAnnualJob(key, year, intervalYears);
	}
	...
	private static bool RequiresFixedCalendarBoundary(XjDetectionJob job)
	{
		return job == XjDetectionJob.GuZunMaintenance
			|| job == XjDetectionJob.KingdomNameNormalization
			|| job == XjDetectionJob.MemoryMaintenance;
	}
	...
	internal static int ResolveAnnualIntervalYears(XjDetectionJob job)
	{
		return job switch
		{
			...
			// 秘境驻守/次级维护三年一次；工程任务自身仍按到期年推进，避免改变既有工期语义。
			XjDetectionJob.SecretRealm => 3,
			// 讲法只在十年世界窗口进入宗门列表；真人每次均可主持，真君仍由领域内50年冷却控制。
			XjDetectionJob.Lecture => 10,
			// 秘境大比改为三年批次，避免每年物化全部宗门列表后再逐宗门判相位。
			XjDetectionJob.SecretRealmTraining => 3,
			// 三书长期改为“事件源即时记史 + 五年世界摘要”，不再年度重扫世界事实。
			XjDetectionJob.ThreeBookChronicle => 5,
			XjDetectionJob.SectTerritoryReconcile => 5,
			XjDetectionJob.SectRebellionSweep => 5,
			XjDetectionJob.WorldSnapshotMaintenance => 10,
			XjDetectionJob.GuZunMaintenance => 10,
			XjDetectionJob.KingdomNameNormalization => 10,
			XjDetectionJob.MemoryMaintenance => 5,
			_ => 1
		};
	}
	...
	internal static string ResolveAnnualJobKey(XjDetectionJob job)
	{
		return job switch
		{
			...
			XjDetectionJob.WorldEventCatalog => AnnualWorldEventCatalog,
			XjDetectionJob.DaoLineageMaintenance => AnnualDaoLineageMaintenance,
			XjDetectionJob.RuleInvariantAudit => AnnualRuleInvariantAudit,
			XjDetectionJob.GuZunMaintenance => AnnualGuZunMaintenance,
			XjDetectionJob.KingdomNameNormalization => AnnualKingdomNameNormalization,
			XjDetectionJob.ShiReincarnationReturns => AnnualShiReincarnationReturns,
			XjDetectionJob.MemoryMaintenance => AnnualMemoryMaintenance,
			XjDetectionJob.XianGuoPolitics => AnnualXianGuoPolitics,
	...
	internal static int ResolveEntityIntervalYears(XjEntityDetectionJob job)
	{
		return job switch
		{
			...
			XjEntityDetectionJob.ZiFuEquipmentMaintenance => 4,
			...
			XjEntityDetectionJob.TalismanAutonomousPlan => 4,
			XjEntityDetectionJob.TalismanMaterialGather => 4,
			XjEntityDetectionJob.FormationFamilyCommission => 12,
			XjEntityDetectionJob.FormationMaterialGather => 3,
			XjEntityDetectionJob.HighRealmChess => 50,
			XjEntityDetectionJob.ThreeBookSocialObservation => 10,
	...
	private static int ResolveEntityPhaseOffset(XjEntityDetectionJob job, long entityId, int intervalYears)
	{
		...
		return job switch
		{
			// 上修执棋已统一压到五十年级；专属系统使用与5年家族批次兼容的相位，这里只保留一致的通用定义。
			...
			// 三书社交观察改为十年错峰：(year + actorId % 10) % 10 == 0。
	...
	internal static int ResolveRuntimeIntervalTicks(XjRuntimeDetectionJob job)
	{
		return job switch
		{
			...
			// 角色注册表与修士缓存共用这一道限定生命周期审计。
			XjRuntimeDetectionJob.LifecycleAudit => 64,
	...
	{
		// 该门禁当前只驱动龙属年度维护，保持紫府金丹专属；
		// 真人级跨修法缓存由宗门、军队等外部消费者单独使用。
	...
	internal static bool ShouldProcessJinDanTerrainEffects(int frameCounter, int cadenceFrames)
	{
		return ShouldRunCadence(XjRuntimeDetectionJob.JinDanTerrain, frameCounter, cadenceFrames)
			&& XjCultivatorCache.HasZhenJunOrHigher;
	}
