...
internal static class XjStageFiveEcologyGuard
{
	...
	internal static void RecordRealmTransition(string nextRealmId, int year)
	{
		...
		{
			_lastZiFuPromotionYear = Math.Max(_lastZiFuPromotionYear, year);
		}
		else if (string.Equals(next, XjRealmIds.JinDan, StringComparison.Ordinal))
		{
			_lastJinDanPromotionYear = Math.Max(_lastJinDanPromotionYear, year);
		}
		...
		{
			_lastShenDanPromotionYear = Math.Max(_lastShenDanPromotionYear, year);
		}
	}
