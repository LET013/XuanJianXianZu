...
internal static class XjCraftProficiencySystem
{
	...
	internal static float GetRankingScore(Actor actor, string traitId)
	{
		if (!TryResolveProfession(actor, traitId, out _, out string rankKey, out int proficiency)) return 0f;
		int rank = ReadOrMigrateRank(actor, rankKey, proficiency);
		return rank <= RankNone ? 0f : rank * 10000f + Math.Clamp(proficiency, 0, 9999);
	}

	internal static string GetRankingDisplay(Actor actor, string traitId)
	{
		if (!TryResolveProfession(actor, traitId, out string profession, out string rankKey, out int proficiency))
		{
			return "未入此艺";
		}
		int rank = ReadOrMigrateRank(actor, rankKey, proficiency);
		return GetRankDisplayName(rank, profession) + " · 经验 " + Math.Max(0, proficiency);
	}
	...
	{
		if (XjCultivationPathRules.IsJinDanEquivalentRealm(realmId)) return RankJinDan;
		if (XjCultivationPathRules.IsZhenRenEquivalentRealm(realmId)) return RankZiFu;
		if (string.Equals(realmId, XjRealmIds.ZhuJi, StringComparison.Ordinal)
			|| string.Equals(realmId, XjRealmIds.HuangGuan, StringComparison.Ordinal)) return RankZhuJi;
		return RankLianQi;
	}
	...
	internal static string BuildArtifactProgressSummary(Actor actor)
	{
		...
		string target = string.Equals(practiceRealmId, XjRealmIds.JinDan, StringComparison.Ordinal) ? "法宝与器库余器"
			: string.Equals(practiceRealmId, XjRealmIds.ZiFu, StringComparison.Ordinal) ? "传承法器（本命灵宝由真人自炼）"
	...
	internal static void RecordArtifactSuccess(Actor actor, string className)
	{
		...
		int currentYear = 0;
		try { currentYear = World.world?.map_stats?.year ?? 0; } catch (System.Exception xjCaught317) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/Craft/XjCraftProficiencySystem.cs:317", xjCaught317); }
	...
	private static string BuildArtifactAttemptStatus(Actor actor, bool nativeTraining)
	{
		...
		int currentYear = 0;
		try { currentYear = World.world?.map_stats?.year ?? 0; } catch (System.Exception xjCaught451) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/Craft/XjCraftProficiencySystem.cs:451", xjCaught451); }
		...
		return currentYear > 0 && currentYear >= readyYear
			? "上次炼制" + XjChronology.FormatYear(lastYear) + "，炼制周期已就绪"
			: "上次炼制" + XjChronology.FormatYear(lastYear) + "，下次可炼制" + XjChronology.FormatYear(readyYear);
	}
