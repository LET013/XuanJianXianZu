...
internal static class XjCraftDomainRegistry
{
	...
	private static long _readRevision;
	private static long _taskReadRevision = -1L;
	private static long _resourceReadRevision = -1L;
	private static long _batchReadRevision = -1L;
	private static long _carryReadRevision = -1L;
	private static IReadOnlyList<XjCraftTaskArchiveRecord> _cachedTasks = Array.Empty<XjCraftTaskArchiveRecord>();
	private static IReadOnlyList<XjCraftResourceArchiveRecord> _cachedResources = Array.Empty<XjCraftResourceArchiveRecord>();
	private static IReadOnlyList<XjTalismanBatchArchiveRecord> _cachedBatches = Array.Empty<XjTalismanBatchArchiveRecord>();
	private static IReadOnlyList<XjTalismanCarryArchiveRecord> _cachedCarries = Array.Empty<XjTalismanCarryArchiveRecord>();
	...
	{
		if (_taskReadRevision == _readRevision) return _cachedTasks;
		if (TasksById.Count == 0)
		{
			_cachedTasks = Array.Empty<XjCraftTaskArchiveRecord>();
			_taskReadRevision = _readRevision;
			return _cachedTasks;
		}
	...
	{
		if (_resourceReadRevision == _readRevision) return _cachedResources;
		if (Resources.Count == 0)
		{
			_cachedResources = Array.Empty<XjCraftResourceArchiveRecord>();
			_resourceReadRevision = _readRevision;
	...
	{
		if (_batchReadRevision == _readRevision) return _cachedBatches;
		...
		_cachedBatches = result.Count == 0 ? Array.Empty<XjTalismanBatchArchiveRecord>() : result;
		_batchReadRevision = _readRevision;
		return _cachedBatches;
	}

	internal static IReadOnlyList<XjTalismanCarryArchiveRecord> ReadCarries()
	{
		if (_carryReadRevision == _readRevision) return _cachedCarries;
		...
		_cachedCarries = result.Count == 0 ? Array.Empty<XjTalismanCarryArchiveRecord>() : result;
		_carryReadRevision = _readRevision;
		return _cachedCarries;
	}

	/// <summary>
	/// 第三方换身是同一逻辑角色的 actorId 迁移。只迁当前个人资产、携带符箓与
	/// 未完成百艺任务；已经完成的任务/成品中的 CrafterActorId 保留为历史事实。
	/// 该入口仅发生在稀有兼容换身流程，允许按该角色个人仓储做小规模重键。
	/// </summary>
	internal static bool RebindActorAfterExternalReplacement(long oldActorId, long newActorId, int currentYear)
	{
		...
		if (OpenTaskIdByActor.TryGetValue(newActorId, out long conflictingTaskId)
			&& (!OpenTaskIdByActor.TryGetValue(oldActorId, out long sourceTaskId) || conflictingTaskId != sourceTaskId))
		{
			if (TasksById.TryGetValue(conflictingTaskId, out XjCraftTaskArchiveRecord conflictingTask) && conflictingTask.IsOpen)
			{
				conflictingTask.Status = XjCraftTaskStatus.Interrupted;
				conflictingTask.FailureReason = "外部换身身份迁移，目标旧任务已中断";
				conflictingTask.LastAdvancedYear = Math.Max(conflictingTask.LastAdvancedYear, safeYear);
			}
			OpenTaskIdByActor.Remove(newActorId);
			changed = true;
		}

		if (OpenTaskIdByActor.TryGetValue(oldActorId, out long taskId))
		{
			OpenTaskIdByActor.Remove(oldActorId);
			if (TasksById.TryGetValue(taskId, out XjCraftTaskArchiveRecord task) && task.IsOpen)
			{
				task.ActorId = newActorId;
				if (task.OwnerScope == XjCraftOwnerScope.Personal && task.OwnerId == oldActorId)
					task.OwnerId = newActorId;
				OpenTaskIdByActor[newActorId] = taskId;
			}
			changed = true;
		}

		XjCraftOwnerKey oldOwner = new XjCraftOwnerKey(XjCraftOwnerScope.Personal, oldActorId);
		XjCraftOwnerKey newOwner = new XjCraftOwnerKey(XjCraftOwnerScope.Personal, newActorId);
		List<XjCraftResourceKey> resourceKeys = new List<XjCraftResourceKey>();
		foreach (KeyValuePair<XjCraftResourceKey, XjCraftResourceArchiveRecord> pair in Resources)
		{
			if (pair.Key.Owner.Equals(oldOwner)) resourceKeys.Add(pair.Key);
		}
		for (int i = 0; i < resourceKeys.Count; i++)
		{
			XjCraftResourceKey oldKey = resourceKeys[i];
			if (!Resources.TryGetValue(oldKey, out XjCraftResourceArchiveRecord resource) || resource == null) continue;
			Resources.Remove(oldKey);
			resource.OwnerScope = newOwner.Scope;
			resource.OwnerId = newOwner.OwnerId;
			resource.UpdatedYear = Math.Max(resource.UpdatedYear, safeYear);
			Resources[new XjCraftResourceKey(newOwner, resource.ResourceId)] = resource;
			changed = true;
		}
		...
		{
			XjTalismanBatchArchiveRecord batch = pair.Value;
			if (batch != null && batch.OwnerScope == XjCraftOwnerScope.Personal && batch.OwnerId == oldActorId)
				batchIds.Add(pair.Key);
		}
		for (int i = 0; i < batchIds.Count; i++)
		{
			long batchId = batchIds[i];
			if (!TalismanBatchesById.TryGetValue(batchId, out XjTalismanBatchArchiveRecord batch) || batch == null) continue;
			XjTalismanOwnerTypeKey oldTypeKey = new XjTalismanOwnerTypeKey(oldOwner, batch.TalismanId);
			if (TalismanBatchIdsByOwnerType.TryGetValue(oldTypeKey, out List<long> oldIds))
			{
				oldIds.Remove(batchId);
				if (oldIds.Count == 0) TalismanBatchIdsByOwnerType.Remove(oldTypeKey);
			}
			batch.OwnerScope = newOwner.Scope;
			batch.OwnerId = newOwner.OwnerId;
			XjTalismanOwnerTypeKey newTypeKey = new XjTalismanOwnerTypeKey(newOwner, batch.TalismanId);
			if (!TalismanBatchIdsByOwnerType.TryGetValue(newTypeKey, out List<long> newIds))
			{
				newIds = new List<long>();
				TalismanBatchIdsByOwnerType[newTypeKey] = newIds;
			}
			if (!newIds.Contains(batchId)) newIds.Add(batchId);
			newIds.Sort();
			changed = true;
		}

		if (CarryKeysByActor.TryGetValue(newActorId, out List<string> targetCarryKeys))
		{
			string[] keys = targetCarryKeys.ToArray();
			for (int i = 0; i < keys.Length; i++) CarriesByActorType.Remove(keys[i]);
			CarryKeysByActor.Remove(newActorId);
			CarryCountByActor.Remove(newActorId);
			changed = true;
		}
		if (CarryKeysByActor.TryGetValue(oldActorId, out List<string> sourceCarryKeys))
		{
			string[] keys = sourceCarryKeys.ToArray();
			CarryKeysByActor.Remove(oldActorId);
			CarryCountByActor.Remove(oldActorId);
			for (int i = 0; i < keys.Length; i++)
			{
				string oldKey = keys[i];
				if (!CarriesByActorType.TryGetValue(oldKey, out XjTalismanCarryArchiveRecord carry) || carry == null) continue;
				CarriesByActorType.Remove(oldKey);
				carry.ActorId = newActorId;
				StoreCarry(BuildCarryKey(newActorId, carry.TalismanId), carry);
			}
			changed = true;
		}
	...
	internal static void ForgetUnavailableActor(long actorId)
	{
		...
		ForgetActor(actorId);
		bool changed = false;
		XjCraftOwnerKey owner = new XjCraftOwnerKey(XjCraftOwnerScope.Personal, actorId);
		List<XjCraftResourceKey> resourceKeys = new List<XjCraftResourceKey>();
		foreach (KeyValuePair<XjCraftResourceKey, XjCraftResourceArchiveRecord> pair in Resources)
			if (pair.Key.Owner.Equals(owner)) resourceKeys.Add(pair.Key);
		for (int i = 0; i < resourceKeys.Count; i++) changed |= Resources.Remove(resourceKeys[i]);

		List<long> batchIds = new List<long>();
		foreach (KeyValuePair<long, XjTalismanBatchArchiveRecord> pair in TalismanBatchesById)
		{
			XjTalismanBatchArchiveRecord batch = pair.Value;
			if (batch != null && batch.OwnerScope == XjCraftOwnerScope.Personal && batch.OwnerId == actorId)
				batchIds.Add(pair.Key);
		}
		for (int i = 0; i < batchIds.Count; i++)
		{
			long batchId = batchIds[i];
			if (!TalismanBatchesById.TryGetValue(batchId, out XjTalismanBatchArchiveRecord batch) || batch == null) continue;
			XjTalismanOwnerTypeKey typeKey = new XjTalismanOwnerTypeKey(owner, batch.TalismanId);
			if (TalismanBatchIdsByOwnerType.TryGetValue(typeKey, out List<long> ids))
			{
				ids.Remove(batchId);
				if (ids.Count == 0) TalismanBatchIdsByOwnerType.Remove(typeKey);
			}
			TalismanBatchesById.Remove(batchId);
			changed = true;
		}
	...
	internal static void ExportArchiveRecords(XjCraftArchiveBundle target)
	{
		...
		foreach (XjCraftTaskArchiveRecord task in ReadTasks()) target.Tasks.Add(Clone(task));
		foreach (XjCraftResourceArchiveRecord resource in ReadResources()) target.Resources.Add(Clone(resource));
		foreach (XjTalismanBatchArchiveRecord batch in ReadTalismanBatches()) target.TalismanBatches.Add(Clone(batch));
		foreach (XjTalismanCarryArchiveRecord carry in ReadCarries()) target.TalismanCarries.Add(Clone(carry));
	}
	...
	internal static void ImportArchiveRecords(XjCraftArchiveBundle source)
	{
		...
		// Clear() leaves the empty read snapshots current. Importing rows mutates the
		// backing stores without archive/Codex dirty notifications, so explicitly
		// advance the local read revision once after the full import.
		TouchReadRevision();
	}
	...
	internal static void Clear()
	{
		...
		_cachedTasks = Array.Empty<XjCraftTaskArchiveRecord>();
		_cachedResources = Array.Empty<XjCraftResourceArchiveRecord>();
		_cachedBatches = Array.Empty<XjTalismanBatchArchiveRecord>();
		_cachedCarries = Array.Empty<XjTalismanCarryArchiveRecord>();
		_taskReadRevision = _resourceReadRevision = _batchReadRevision = _carryReadRevision = _readRevision;
	}
	...
	private static void TouchReadRevision()
	{
		_readRevision = _readRevision == long.MaxValue ? 1L : _readRevision + 1L;
	}
