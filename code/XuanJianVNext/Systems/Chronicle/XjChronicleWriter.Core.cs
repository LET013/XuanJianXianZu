...
internal static partial class XjChronicleWriter
{
	...
	internal static bool RecordLingWuAppeared(
		in XjDeathSnapshot snapshot,
		XjLingWuDef definition,
		long recipientFamilyId,
		long recipientActorId,
		string recipientActorName,
		bool claimedByKiller)
	{
		if (!snapshot.Found
			|| recipientFamilyId <= 0L
			|| snapshot.ActorId <= 0L
			|| definition == null
		...
		string victimName = string.IsNullOrWhiteSpace(snapshot.Name) ? "一位紫府修士" : snapshot.Name;
		string recipientName = string.IsNullOrWhiteSpace(recipientActorName) ? "族中修士" : recipientActorName;
		string body = claimedByKiller
			? recipientName + "斩杀" + victimName + "，其" + definition.DaoTuGroup + "道痕凝聚为“" + definition.Name + "”，作为战利品收入本族重宝仓库。"
			: victimName + "身故后，其" + definition.DaoTuGroup + "道痕凝聚为“" + definition.Name + "”，收入家族重宝仓库。";
		XjChronicleEvent chronicleEvent = new XjChronicleEvent(
			true,
			recipientFamilyId,
			Math.Max(0L, recipientActorId),
			XjChronicleEventTypes.LingWuAppeared,
			snapshot.Year,
			claimedByKiller ? "斩敌夺得紫府灵物" : "紫府遗蜕化生灵物",
			body,
			4,
			true,
			true,
			false,
			"Ok",
			claimedByKiller ? "lingwu.zifu_kill_claim" : "lingwu.zifu_death",
			snapshot.RealmId);
		// Keep idempotency bound to the dead ZiFu, not the recipient. Save recovery
		// must never mint the same death LingWu once for each possible owner.
		return XjFamilyChronicleMemory.Shared.Append(chronicleEvent, BuildLingWuAppearedEventKey(snapshot, definition));
	}

	internal static string BuildLingWuAppearedEventKey(in XjDeathSnapshot snapshot, XjLingWuDef definition)
	{
		if (!snapshot.Found || snapshot.ActorId <= 0L || definition == null || string.IsNullOrWhiteSpace(definition.Id))
		{
			return string.Empty;
		}

		return "LingWuAppeared:" + snapshot.ActorId + ":" + snapshot.Year + ":" + definition.Id;
	}
}
