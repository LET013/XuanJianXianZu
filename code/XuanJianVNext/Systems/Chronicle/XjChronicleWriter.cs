...
internal static partial class XjChronicleWriter
{
		...
		string shenTongName)
	{
		...
		string body = actorName + "受他人欺骗练就" + daoTu + "道途神通" + shenTong
			+ "，已入歧途而不自知，自此刻开始金丹前路已断绝。";
		return RecordRich(
			actor,
		...
		string dongTianName)
	{
		...
		string titleName = XjLongShuSystem.IsLongShu(actor) ? "龙君" : "真君";
		string safeGuoWei = string.IsNullOrWhiteSpace(displayGuoWei) ? "金丹果位" : displayGuoWei.Trim();
	...
	internal static bool RecordJieLinSucceeded(Actor actor, int timestamp, int activeCount)
	{
		...
		string countText = activeCount > 0
			...
			: "列于当世结璘仙之中。";
		string body = actorName + "成就结璘，受太阴正位赋予，晋号【" + titleName
			+ "】。其身不占果位，" + countText;
		return RecordRich(
			actor,
