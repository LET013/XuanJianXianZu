...
internal static class XjActorGongFaCollection
{
	...
	private const int CurrentSummaryVersion = 1;
	...
	private readonly struct CollectionSummary
	{
		internal readonly int RecordCount;
		internal readonly int Grade5Count;
		internal readonly int Grade6Count;
		internal readonly int UniqueMappedCount;

		internal CollectionSummary(int recordCount, int grade5Count, int grade6Count, int uniqueMappedCount)
		{
			RecordCount = Math.Max(0, recordCount);
			Grade5Count = Math.Max(0, grade5Count);
			Grade6Count = Math.Max(0, grade6Count);
			UniqueMappedCount = Math.Max(0, uniqueMappedCount);
		}

		internal bool HasFiveRealGrade5 => RecordCount == MaximumRecords
			&& Grade5Count == MaximumRecords
			&& UniqueMappedCount == MaximumRecords;

		internal bool HasJinDanSet => RecordCount == MaximumRecords
			&& Grade5Count == MaximumRecords - 1
			&& Grade6Count == 1
			&& UniqueMappedCount == MaximumRecords;
	}
	...
	internal static bool TryReadStoredPrimary(Actor actor, out Record record)
	{
		...
		bool versionRepair = version < CurrentVersion || store.Version < CurrentVersion;
		string before = versionRepair ? json : string.Empty;
		...
		// JSON 集合才是功法权威源。外部版本键在境界切换、旧档恢复或旁路复制时
		// 偶尔滞后，不应因此把仍然完整的集合判成“无功法”。读取时原地补齐版本，
		// 并把旧单功法镜像恢复，保证玄鉴照录与运行时判定同时可见。
		if (versionRepair || repairs > 0)
		{
			if (Write(actor, store))
			{
				LogRepair(actor, "读取自愈", repairs + (versionRepair ? 1 : 0), before);
			}
			else
			{
				LogInvariant(actor, "有效功法集合版本无法自愈");
			}
		}
		if (!IsLegacyPrimaryMirrored(actor, primary))
		{
	...
	{
		return TryGetOrBuildSummary(actor, out CollectionSummary summary)
			&& summary.HasFiveRealGrade5;
	}

	internal static bool HasJinDanGongFaSet(Actor actor)
	{
		return TryGetOrBuildSummary(actor, out CollectionSummary summary)
			&& summary.HasJinDanSet;
	}
		...
		out int changedCount)
	{
		...
		for (int i = 0; i < store.Records.Count; i++)
		{
			...
			{
				LogInvariant(actor, "神通映射冲突：" + mapped);
				return false;
			}
		}

		string normalizedSource = string.IsNullOrWhiteSpace(source) ? "神通映射校正" : source.Trim();
		for (int i = 0; i < store.Records.Count; i++)
		{
			RecordDto item = store.Records[i];
	...
	/// <summary>
	/// H1 compatibility boundary for raw transaction snapshots. Normal writers update the
	/// summary inside Write(); rollback/import code may call this once after restoring JSON.
	/// </summary>
	internal static void RefreshStoredSummary(Actor actor)
	{
		if (actor?.data == null) return;
		Store store = ReadOrMigrate(actor);
		WriteSummary(actor, BuildSummary(store));
	}

	private static bool TryGetOrBuildSummary(Actor actor, out CollectionSummary summary)
	{
		summary = default;
		if (actor?.data == null) return false;
		if (TryReadSummary(actor, out summary)) return true;

		// Old saves pay one JSON materialization. Every subsequent annual/high-realm
		// eligibility read is scalar-only until the collection is actually mutated.
		Store store = ReadOrMigrate(actor);
		summary = BuildSummary(store);
		WriteSummary(actor, summary);
		return true;
	}

	private static bool TryReadSummary(Actor actor, out CollectionSummary summary)
	{
		summary = default;
		if (actor?.data == null
			|| !XjActorAccessor.TryGetInt(actor, XjActorDataKeys.XjGongFaCollectionSummaryVersion, out int version)
			|| version != CurrentSummaryVersion)
		{
			return false;
		}

		XjActorAccessor.TryGetInt(actor, XjActorDataKeys.XjGongFaCollectionSummaryRecordCount, out int records);
		XjActorAccessor.TryGetInt(actor, XjActorDataKeys.XjGongFaCollectionSummaryGrade5Count, out int grade5);
		XjActorAccessor.TryGetInt(actor, XjActorDataKeys.XjGongFaCollectionSummaryGrade6Count, out int grade6);
		XjActorAccessor.TryGetInt(actor, XjActorDataKeys.XjGongFaCollectionSummaryUniqueMappedCount, out int uniqueMapped);
		summary = new CollectionSummary(records, grade5, grade6, uniqueMapped);
		return true;
	}

	private static CollectionSummary BuildSummary(Store store)
	{
		if (store?.Records == null || store.Records.Count == 0)
		{
			return new CollectionSummary(0, 0, 0, 0);
		}
		...
		int limit = Math.Min(store.Records.Count, MaximumRecords);
		for (int i = 0; i < limit; i++)
		{
			RecordDto item = store.Records[i];
			if (!IsValid(item)) continue;
			recordCount++;
			if (item.Grade == 5) grade5Count++;
			else if (item.Grade == 6) grade6Count++;
			...
			for (int j = 0; j < i; j++)
			{
				RecordDto previous = store.Records[j];
				if (IsValid(previous)
					&& string.Equals(Normalize(previous.MappedXianJi), mapped, StringComparison.Ordinal))
				{
					duplicate = true;
					break;
				}
			}
			if (!duplicate) uniqueMappedCount++;
		}
		return new CollectionSummary(recordCount, grade5Count, grade6Count, uniqueMappedCount);
	}

	private static void WriteSummary(Actor actor, in CollectionSummary summary)
	{
		if (actor?.data == null) return;
		// Version is written last so a partially interrupted projection is never trusted.
		XjActorAccessor.SetInt(actor, XjActorDataKeys.XjGongFaCollectionSummaryRecordCount, summary.RecordCount);
		XjActorAccessor.SetInt(actor, XjActorDataKeys.XjGongFaCollectionSummaryGrade5Count, summary.Grade5Count);
		XjActorAccessor.SetInt(actor, XjActorDataKeys.XjGongFaCollectionSummaryGrade6Count, summary.Grade6Count);
		XjActorAccessor.SetInt(actor, XjActorDataKeys.XjGongFaCollectionSummaryUniqueMappedCount, summary.UniqueMappedCount);
		XjActorAccessor.SetInt(actor, XjActorDataKeys.XjGongFaCollectionSummaryVersion, CurrentSummaryVersion);
	}
	...
	private static int ReconcileStoreWithActor(Actor actor, Store store, bool createMissing)
	{
		...
		// 高境角色在境界写入、仙基缓存失效与金丹预检之间可能短暂读到空集合。
		// 这不是“仙基真实消失”，不得把五部映射功法裁成一部。只在明确退回低境
		// 时执行破坏性清理；高境空窗保留现有集合，待下一次仙基写入再精确对账。
		if (valid.Count == 0 && ShouldPreserveMappedHighRealmCollection(actor, store))
		{
			return repairs;
		}
	...
	private static string[] ResolveReconciliationXianJiIds(Actor actor)
	{
		...
		string[] rawIds = XjXianJiAccessor.ReadRawIds(actor);
		bool highRealm = string.Equals(realmId, XjRealmIds.ZiFu, StringComparison.Ordinal)
		...
		if (highRealm && rawIds != null && rawIds.Length > 0)
		{
			return rawIds;
		}
		...
		bool knownRealm = string.Equals(realmId, XjRealmIds.TaiXi, StringComparison.Ordinal)
			...
			|| highRealm;
		return knownRealm ? Array.Empty<string>() : rawIds;
	}

	private static bool ShouldPreserveMappedHighRealmCollection(Actor actor, Store store)
	{
		string realmId = XjRealmHelper.NormalizeId(
			XjRealmHelper.GetUnifiedId(actor, XjRealmHelper.GetTraitSnapshotForRouter));
		bool highRealm = string.Equals(realmId, XjRealmIds.ZiFu, StringComparison.Ordinal)
			|| string.Equals(realmId, XjRealmIds.JinDan, StringComparison.Ordinal)
			|| string.Equals(realmId, XjRealmIds.ShenDan, StringComparison.Ordinal);
		if (!highRealm || store?.Records == null)
		{
			return false;
		}
		...
		for (int i = 0; i < store.Records.Count; i++)
		{
			if (IsValid(store.Records[i]) && !string.IsNullOrWhiteSpace(store.Records[i].MappedXianJi))
			{
				return true;
			}
		}
	...
	private static string NormalizeName(string value)
	{
		...
		string[] suffixes = { "（一品）", "（二品）", "（三品）", "（四品）", "（五品）", "（六品）", "（七品）" };
	...
	private static string Serialize(Store store)
	{
		...
		catch (System.Exception xjCaught1741_1)
		{
			XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/GongFa/XjActorGongFaCollection.cs:1741", xjCaught1741_1);
	...
	private static Store Deserialize(string json)
	{
		...
		catch (System.Exception xjCaught1757_2)
		{
			XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/GongFa/XjActorGongFaCollection.cs:1757", xjCaught1757_2);
	...
	private static bool Write(Actor actor, Store store)
	{
		...
		try
		{
			...
			bool persistedOk = string.Equals(persisted, json, StringComparison.Ordinal);
			if (persistedOk) WriteSummary(actor, BuildSummary(store));
			return persistedOk;
		}
		catch (System.Exception xjCaught1787_3)
		{
			XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/GongFa/XjActorGongFaCollection.cs:1787", xjCaught1787_3);
			
			return false;
		}
	}

	private static bool IsLegacyPrimaryMirrored(Actor actor, RecordDto primary)
	{
		if (actor?.data == null || primary == null)
		{
			return false;
		}
		return XjActorAccessor.TryGetString(actor, XjActorDataKeys.XjGongFaName, out string name)
			...
			&& SameName(name, primary.Name)
			&& grade == primary.Grade
			&& string.Equals(Normalize(daoTu), Normalize(primary.DaoTu), StringComparison.Ordinal);
	}
