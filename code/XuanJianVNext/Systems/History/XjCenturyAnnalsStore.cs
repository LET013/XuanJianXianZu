...
internal static class XjCenturyAnnalsStore
{
	...
	private const int LedgerTrimBatchSize = 512;
	private static XjCenturyAnnalsStateRecord State = new XjCenturyAnnalsStateRecord();
	// 家族阶段记录最多可达4096条。五岁天资落定/家族扶持属于事件热入口，
	// 不应为了定位一个家族每次线性扫整张阶段表；索引只保存同一 record 引用，
	// 字段原地更新天然可见，只有列表被导入/裁剪/重建时才重建。
	private static readonly Dictionary<long, XjCenturyFamilyStageStateRecord> FamilyStageStateByFamilyId =
		new Dictionary<long, XjCenturyFamilyStageStateRecord>();
	private static bool TimelineRepairPending;
	private static bool AnnalsCalendarRepairPending;
	...
	internal static int AnnalsBaseWorldYear => Math.Max(0, State?.BaseWorldYear ?? 0);
	...
	/// <summary>
	/// 家族阶段状态的 O(1) 读取入口。只返回现存持久记录引用，不创建快照、不扫描列表。
	/// </summary>
	internal static bool TryGetFamilyStageState(long familyStableId, out XjCenturyFamilyStageStateRecord state)
	{
		state = null;
		return familyStableId > 0L
			&& FamilyStageStateByFamilyId.TryGetValue(familyStableId, out state)
			&& state != null;
	}
	...
	{
		State ??= new XjCenturyAnnalsStateRecord();
		// BaseWorldYear 是玄鉴历唯一纪年基准：进入世界并完成玄鉴起录的当年即玄鉴历1年。
		// 百年世谱、固定里程碑和玩家可见纪年均从此换算；冷却/持续时间仍使用绝对世界年。
		// 只有首次建立、旧时间线恢复或世谱日历迁移时才进入重恢复。
		int runtimeBaselineYear = XjRuntimeCadence.LoadBaselineYear;
		bool hasRuntimeBaseline = runtimeBaselineYear > 0
			&& (currentYear <= 0 || runtimeBaselineYear <= currentYear);
		if (!TimelineRepairPending
			&& !AnnalsCalendarRepairPending
			&& State.BaseWorldYear > 0
			&& State.SchemaVersion == XjCenturyAnnalsSchema.CurrentVersion
			&& (!hasRuntimeBaseline || State.BaseWorldYear == runtimeBaselineYear))
		{
		...
		if (AnnalsCalendarRepairPending)
		{
			changed |= RepairAnnalsRelativeCalendar(resolvedBaseWorldYear, referenceYear);
			AnnalsCalendarRepairPending = false;
		}
	...
	{
		// RuntimeCadence 的持久基准是在玄鉴第一次进入该存档时写下的世界绝对年，
		// 它比 Schema10/11 被错误写成1的世谱 BaseWorldYear 更可信。Schema12 起
		// 仍把 CenturyAnnals.State.BaseWorldYear 作为公开唯一纪年基准，但每次迁移/自检
		// 都先与这份既有起录证据对齐，防止三千年旧地图再次被认作从世界1年起录。
		int runtimeBaselineYear = XjRuntimeCadence.LoadBaselineYear;
		if (runtimeBaselineYear > 0 && (referenceYear <= 0 || runtimeBaselineYear <= referenceYear))
		{
			return runtimeBaselineYear;
		}
		...
		// Schema10/11 的卷首本身就是错误的绝对百年，不能再反过来拿它推导起录年。
		// 若极少数旧档丢了 BaseWorldYear，则从已经导入的玄鉴世界史取最早可确证年份；
		// 若连世界史也为空，才以本次载入世界年作为新的起录基准。
		if (AnnalsCalendarRepairPending && ImportedSchemaVersion >= 10 && ImportedSchemaVersion < 12)
		{
			int historyBase = RecoverBaseWorldYearFromWorldHistory(referenceYear);
			return historyBase > 0 ? historyBase : Math.Max(1, referenceYear);
		}
	...
	private static int RecoverBaseWorldYearFromWorldHistory(int referenceYear)
	{
		IReadOnlyList<XjWorldHistoryArchiveRecord> history = XjWorldHistoryStore.ReadSnapshot();
		int earliest = 0;
		for (int i = 0; i < history.Count; i++)
		{
			XjWorldHistoryArchiveRecord record = history[i];
			if (record == null || record.Year <= 0 || referenceYear > 0 && record.Year > referenceYear) continue;
			if (earliest <= 0 || record.Year < earliest) earliest = record.Year;
		}
		return earliest;
	}
	...
	/// <summary>
	/// Schema 10/11 曾错误地把 WorldBox 绝对世界年当成“玄鉴历”，导致载入三千年旧地图时
	/// 立即补出三十余卷假世谱。Schema 12 重新以持久 BaseWorldYear 为唯一卷首；不与新卷首
	/// 对齐的旧卷直接舍弃，仍可确证的世界史和账本保留并重新归卷，等真正满百年后再成卷。
	/// </summary>
	private static bool RepairAnnalsRelativeCalendar(int baseWorldYear, int currentYear)
	{
		...
		int latestCompletedCenturyId = currentYear >= baseWorldYear
			...
			: 0;
		Dictionary<int, XjCenturyAnnalsArchiveRecord> repaired = new Dictionary<int, XjCenturyAnnalsArchiveRecord>();
		bool discardedMisalignedRecord = false;
		for (int i = 0; i < Records.Count; i++)
		{
			XjCenturyAnnalsArchiveRecord record = Records[i];
			if (record == null || record.StartYear <= 0)
			{
				changed = true;
				discardedMisalignedRecord = true;
				continue;
			}
			int centuryId = ResolveCenturyIdFromBase(record.StartYear, baseWorldYear);
			int expectedStartYear = centuryId > 0
				? baseWorldYear + (centuryId - 1) * XjCenturyAnnalsSchema.YearsPerCentury
				: 0;
			int expectedEndYear = expectedStartYear > 0
				? expectedStartYear + XjCenturyAnnalsSchema.YearsPerCentury - 1
				: 0;
			bool aligned = centuryId > 0
				&& centuryId <= latestCompletedCenturyId
				&& record.StartYear == expectedStartYear
				&& record.EndYear == expectedEndYear;
			if (!aligned)
			{
			...
			if (record.CenturyId != centuryId
				|| record.SchemaVersion != XjCenturyAnnalsSchema.CurrentVersion
				|| !string.Equals(record.Title, "第" + centuryId + "世谱", StringComparison.Ordinal))
			{
				changed = true;
			}
			record.SchemaVersion = XjCenturyAnnalsSchema.CurrentVersion;
		...
		if (repaired.Count != RecordsByCenturyId.Count) changed = true;
		RecordsByCenturyId.Clear();
		foreach (KeyValuePair<int, XjCenturyAnnalsArchiveRecord> pair in repaired)
		{
			RecordsByCenturyId[pair.Key] = pair.Value;
		...
		{
			XjCenturyAnnalsLedgerRecord record = Ledger[i];
			if (record == null || record.Year < baseWorldYear)
			{
				Ledger.RemoveAt(i);
				changed = true;
				continue;
		...
		// 只有 Schema10/11 的绝对纪年卷确实被丢弃时，才清掉由错误卷次推进出的家族阶段状态。
		// 新世界从1年起录、或旧相对卷本就对齐时不做无谓重置。
		if (discardedMisalignedRecord && ImportedSchemaVersion >= 10 && ImportedSchemaVersion < 12
			&& State?.FamilyStageStates != null && State.FamilyStageStates.Count > 0)
		{
			State.FamilyStageStates.Clear();
			FamilyStageStateByFamilyId.Clear();
			changed = true;
		}
		State.BaseWorldYear = baseWorldYear;
		State.GeneratedCenturyIds = new List<int>();
	...
	/// <summary>
	/// 百年卷只读取目标卷自己的账本。旧实现固定拿“最近320条”，世界运行
	/// 数百年后再补前卷时，目标世纪很容易已经不在这320条中，最终显示为
	/// 家族/宗门/器物全空。这里扫描最多8000条常驻账本，但只克隆目标卷。
	/// </summary>
	internal static IReadOnlyList<XjCenturyAnnalsLedgerRecord> ReadLedgerSnapshotForCentury(int centuryId, int maximumEntries = 0)
	{
		if (centuryId <= 0 || Ledger.Count == 0) return Array.Empty<XjCenturyAnnalsLedgerRecord>();
		int cap = maximumEntries > 0 ? maximumEntries : int.MaxValue;
		List<XjCenturyAnnalsLedgerRecord> result = new List<XjCenturyAnnalsLedgerRecord>(Math.Min(cap, 128));
		for (int i = Ledger.Count - 1; i >= 0 && result.Count < cap; i--)
		{
			XjCenturyAnnalsLedgerRecord item = Ledger[i];
			if (item == null || item.CenturyId != centuryId) continue;
		...
		IReadOnlyList<XjCenturyAnnalsLedgerRecord> ledger)
	{
		...
		// Schema 7以前先修复旧的起录时间轴；Schema 12再把10/11错误的“世界绝对百年”
		// 迁回同一个 BaseWorldYear 相对纪年。
		TimelineRepairPending = importedSchemaVersion < 7;
		AnnalsCalendarRepairPending = importedSchemaVersion < 12;
		State = Normalize(Clone(state) ?? new XjCenturyAnnalsStateRecord());
		RebuildFamilyStageStateIndex();
		if (records != null)
		{
		...
		if (currentYear > 0)
		{
			// Schema <12 必须先把世谱卷次统一迁回起录年相对纪年，再按目标世纪取证；
			// 否则账本 CenturyId 仍是旧口径，后续补录会再次读空。
			TryEnsureBaseWorldYear(currentYear, out _);
		}

		// Schema 11：旧卷曾只保存事件ID且补卷只读取“最近320条账本”，导致
		// 高年份读档后早期卷几乎全空。世界史在 WorldArchive 导入顺序中已先恢复，
		// 因此这里可用目标世纪的留存史料一次性补回“本卷大事”以及可确证的
		// 家族、宗门、境界、道途、器物、人物证据；绝不拿当前世界状态伪造旧史。
		bool rehydrated = false;
		if (importedSchemaVersion < 11)
		{
			for (int i = 0; i < Records.Count; i++)
			{
				XjCenturyAnnalsArchiveRecord record = Records[i];
				if (record == null) continue;
				rehydrated |= XjCenturyAnnalsBuilder.TryRehydrateArchivedRecord(record);
				Normalize(record);
			}
		}
		requiresRewrite |= rehydrated;
		if (requiresRewrite)
		{
			XjWorldArchiveSystem.MarkChanged();
			XjWorldArchiveSystem.RequestProtectedCommit();
	...
	internal static bool TryStoreRecord(XjCenturyAnnalsArchiveRecord record)
	{
		...
		RebuildOrderedRecords();
		State ??= new XjCenturyAnnalsStateRecord();
		// 百年卷只消费已固化的 BaseWorldYear，不反向改写纪元起点。
	...
	internal static void ObserveDeathSnapshot(in XjDeathSnapshot snapshot)
	{
		...
		AppendLedgerRecord(new XjCenturyAnnalsLedgerRecord
		{
			...
			EventType = snapshot.IsYuYiXian ? "YuYiDeath" : snapshot.IsJieLinXian ? "JieLinDeath" : "ActorDeath",
			Year = snapshot.Year,
	...
	internal static void AppendLedgerRecord(XjCenturyAnnalsLedgerRecord record)
	{
		...
		{
			// Avoid shifting an 8k List for every ledger append once saturated. A
			// batched FIFO trim keeps the same retention semantics with a long hysteresis
			// window; completed century rolls already carry their own immutable snapshots.
			int target = Math.Max(0, XjCenturyAnnalsSchema.MaxLedgerRecords - LedgerTrimBatchSize);
			int removeCount = Math.Min(Ledger.Count, Math.Max(1, Ledger.Count - target));
			Ledger.RemoveRange(0, removeCount);
		}
		XjWorldArchiveSystem.MarkChanged();
	}
	...
	{
		if (year <= 0) return 0;
		int baseWorldYear = Math.Max(0, State?.BaseWorldYear ?? 0);
		if (baseWorldYear <= 0)
		{
			TryEnsureBaseWorldYear(year, out baseWorldYear);
		}
		...
		IReadOnlyList<XjCenturyAnnalsLedgerRecord> ledger)
	{
		...
		if (records != null)
		{
			for (int i = 0; i < records.Count; i++)
			{
				...
				if (record.NotableEvents == null) return true;
	...
	private static void RebuildFamilyStageStateIndex()
	{
		FamilyStageStateByFamilyId.Clear();
		IReadOnlyList<XjCenturyFamilyStageStateRecord> states = State?.FamilyStageStates;
		if (states == null) return;
		for (int i = 0; i < states.Count; i++)
		{
			XjCenturyFamilyStageStateRecord item = states[i];
			if (item == null || item.FamilyStableId <= 0L) continue;
			FamilyStageStateByFamilyId[item.FamilyStableId] = item;
		}
	}
	...
	private static void NormalizeFamilyStageStates(List<XjCenturyFamilyStageStateRecord> states)
	{
		...
		for (int i = 0; i < states.Count; i++)
		{
			...
			// 扶持对象与家族志向解耦：志向消失/变化都不能清空长期培养对象。
			if (item.SupportedActorId <= 0L)
			{
			...
			{
				item.SupportedSinceYear = item.LastUpdatedYear > 0
					? item.LastUpdatedYear
					: Math.Max(item.AspirationSinceYear, item.FirstRecordedYear);
			}
	...
	private static XjCenturyAnnalsArchiveRecord Normalize(XjCenturyAnnalsArchiveRecord record)
	{
		...
		NormalizeNotableEvents(record.NotableEvents);
	...
	private static void NormalizeNotableEvents(List<XjCenturyEventRecord> items)
	{
		if (items == null) return;
		HashSet<string> seen = new HashSet<string>(StringComparer.Ordinal);
		for (int i = items.Count - 1; i >= 0; i--)
		{
			XjCenturyEventRecord item = items[i];
			if (item == null || item.Year <= 0)
			{
				items.RemoveAt(i);
				continue;
			}
			item.EventId = Math.Max(0L, item.EventId);
			item.Year = Math.Max(1, item.Year);
			item.Category = item.Category?.Trim() ?? string.Empty;
			item.EventType = item.EventType?.Trim() ?? string.Empty;
			item.Title = item.Title?.Trim() ?? string.Empty;
			item.Summary = item.Summary?.Trim() ?? string.Empty;
			item.Importance = Math.Max(1, item.Importance);
			item.ActorId = Math.Max(0L, item.ActorId);
			item.ActorName = item.ActorName?.Trim() ?? string.Empty;
			item.FamilyId = Math.Max(0L, item.FamilyId);
			item.FamilyName = item.FamilyName?.Trim() ?? string.Empty;
			item.SectId = Math.Max(0L, item.SectId);
			item.SectName = item.SectName?.Trim() ?? string.Empty;
			if (item.Title.Length == 0 && item.Summary.Length == 0)
			{
				items.RemoveAt(i);
				continue;
			}
			string signature = item.EventId > 0L
				? "id:" + item.EventId.ToString()
				: item.Year.ToString() + "|" + item.EventType + "|" + item.Title + "|" + item.ActorId.ToString();
			if (!seen.Add(signature)) items.RemoveAt(i);
		}
		items.Sort((left, right) =>
		{
		...
		if (items.Count > XjCenturyAnnalsSchema.MaxNotableEventsPerCentury)
		{
			items.RemoveRange(XjCenturyAnnalsSchema.MaxNotableEventsPerCentury,
				items.Count - XjCenturyAnnalsSchema.MaxNotableEventsPerCentury);
		}
	}
	...
	private static string BuildDeathSummary(in XjDeathSnapshot snapshot)
	{
		...
		if (snapshot.IsYuYiXian) parts.Add("郁仪仙陨落");
		else if (snapshot.IsJieLinXian) parts.Add("结璘仙陨落");
	...
	private static XjCenturyAnnalsArchiveRecord Clone(XjCenturyAnnalsArchiveRecord source)
	{
		...
		XjCenturyAnnalsArchiveRecord copy = new XjCenturyAnnalsArchiveRecord
		{
			...
			NotableEvents = new List<XjCenturyEventRecord>(),
		...
		if (source.NotableEvents != null) for (int i = 0; i < source.NotableEvents.Count; i++) copy.NotableEvents.Add(Clone(source.NotableEvents[i]));
	...
	private static XjCenturyEventRecord Clone(XjCenturyEventRecord source)
	{
		if (source == null) return null;
		return new XjCenturyEventRecord
		{
