...
internal static class XjThreeBookDeferredFamilyFacts
{
	...
	internal static int TickYear(int currentYear)
	{
		...
		int budget = XjRuntimeWorkBudget.StressTier >= XjRuntimeStressTier.Mild ? 32 : 96;
		int processed = 0;
		int inspected = 0;
		int initialCount = Records.Count;
		bool changed = false;
		while (Records.Count > 0 && processed < budget && inspected < initialCount)
		{
		...
		XjRuntimeDiagnostics.SampleQueue("threebook-family-pending", Records.Count);
		return processed;
	}
