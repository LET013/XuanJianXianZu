...
/// <summary>
/// 修士年度社交纪事。复用现有宗门/城市增量索引，每名修士五年最多检查12名候选，
...
internal static class XjThreeBookSocialObserver
{
	...
	internal static void TickActor(Actor actor, int currentYear)
	{
		...
		if (XjNativeRelationshipBridge.TryResolvePartner(actor, out Actor partner)
			&& partner?.data != null
	...
	private static bool TrySelectCandidate(Actor actor, long actorId, long excludedActorId, int currentYear, out Actor selected, out string basis)
	{
		...
		IReadOnlyList<long> ids = sectId > 0L ? XjZongMenCultivatorCityIndex.GetActorIdsForSect(sectId) : null;
	...
	{
		if (city != null && XjZongMenCultivatorCityIndex.GetCityIndex().TryGetValue(city, out List<long> ids)) return ids;
