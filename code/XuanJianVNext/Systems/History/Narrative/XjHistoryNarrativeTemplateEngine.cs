...
internal static class XjHistoryNarrativeTemplateEngine
{
		...
		string subjectName)
	{
		...
		string result = huiGuangGain > 0
			? "闭关结束后，他的慧光增长了" + huiGuangGain + "，许多从前晦涩的关隘也随之清晰了几分。"
		...
		string text = visibility switch
		{
			XjHistoryVisibility.Family => sect + "举行十年大比，" + family + "族人" + actor + "力压同辈，列第" + rank + "。最终共有" + total + "名弟子取得【" + realmName + "】名额，他也在其中。" + result,
			XjHistoryVisibility.Sect => sect + "十年大比再开。诸峰弟子依次登场，" + actor + "最终列第" + rank + companionText + result,
			_ => sect + "十年大比开场时，" + actor + "也在台下候名。数轮考校之后，他列第" + rank + "，从门中诸弟子里争得一席，与共计" + total + "名胜者一同进入【" + realmName + "】。" + result
		};
		...
		string subjectName)
	{
		...
		List<string> bracketed = ExtractDelimitedValues(sourceText, '【', '】');
		string realmName = bracketed.Count > 0 ? bracketed[0] : realmLabel;
		string qualification = ContainsAny(sourceText, "真人退席")
			? "门中真人不与弟子争席，只许紫府以下修士入场。"
			: ContainsAny(sourceText, "真君退席")
				? "门中真君退席，紫府真人与其下弟子方可争名。"
				: string.Empty;
		string text = sect + "为有限的【" + realmName + "】修炼名额重开十年大比。" + qualification
			+ "诸峰弟子依境界、真元与道行逐一考校，最终只有前" + total + "人得以入内。大比散场之后，胜者结伴踏入洞门，余者则各自回峰再候下次。";
	...
	private static XjHistoryNarrativeEntry ComposeRenDan(XjCodexHistoryItem item, XjHistoryVisibility visibility, long subjectId, string subjectName)
	{
		...
		if (EqualsType(eventType, "RenDanMarked"))
		{
			...
			{
				tag = "筑基奇遇";
				text = victim + "忽得来历不明的修行机缘，真元与慧光皆有增益；其人与" + victimFamily + "当时并不知道，这份机缘来自" + source + "的人丹布置。";
			}
			accent = "#B7A7FF";
		}
	...
	private static string StripRealmSuffix(string value)
	{
		...
		if (!ContainsAny(suffix, "胎息", "炼气", "筑基", "紫府", "金丹", "神丹", "结璘", "真人", "真君", "玄君", "龙王")) return text;
