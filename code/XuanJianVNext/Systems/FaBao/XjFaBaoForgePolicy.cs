...
internal static class XjFaBaoForgePolicy
{
	...
	internal const int WarehouseAttemptIntervalYears = 10;
	internal const int ZhuJiWarehouseLifetimeLimit = 5;
	...
	internal static string ResolvePracticeRealmId(Actor actor, string actualRealmId)
	{
		...
		bool isDaoTaiArtifactRealm = string.Equals(normalizedRealmId, XjRealmIds.DaoTai, StringComparison.Ordinal)
			|| string.Equals(normalizedRealmId, XjRealmIds.FuQiDaoTai, StringComparison.Ordinal);
		bool isFuQiJinDanEquivalent = isFuQi
			&& (string.Equals(normalizedRealmId, XjRealmIds.ZhenJunYuShi, StringComparison.Ordinal)
				|| string.Equals(normalizedRealmId, XjRealmIds.FuQiDaoTai, StringComparison.Ordinal));
		int rank = XjCraftProficiencySystem.GetArtifactRank(actor);
		if (isFuQi && !isFuQiJinDanEquivalent)
		{
			// 服气修士的本命核心属于自身性命工程，不把炼器师身份解释为可批量
			// 炼制灵宝、法宝。黄冠以上的炼器师以自身境界承载手艺，但实际对外
			// 产出保持在可传承法器层次。
			int tier = XjRealmSuppression.GetRealmTier(actor);
			if (tier >= XjRealmSuppression.TierZhuJi
				&& rank >= XjCraftProficiencySystem.RankZhuJi) return XjRealmIds.ZhuJi;
			return rank >= XjCraftProficiencySystem.RankLianQi ? XjRealmIds.LianQi : string.Empty;
		}

		bool isJinDan = string.Equals(normalizedRealmId, XjRealmIds.JinDan, StringComparison.Ordinal)
			|| isFuQiJinDanEquivalent
			|| isDaoTaiArtifactRealm;
	...
	/// <summary>
	/// 真人本命灵宝是个人高境器物，不属于百艺炼器生产。
	/// 紫金紫府以三门神通为成熟节点；服气真人以本命神妙圆满为对应节点。
	/// 两路均只消耗同道途紫府灵物，本命灵宝写入原生武器槽。
	/// </summary>
	internal static bool NeedsPersonalZiFuLingBao(Actor actor)
	{
		...
		bool hasMatureRealmFoundation;
		if (string.Equals(realmId, XjRealmIds.ZiFu, StringComparison.Ordinal))
		{
			hasMatureRealmFoundation = XjXianJiAccessor.GetEffectiveShenTongCount(actor) >= 3;
		}
		else if (string.Equals(realmId, XjRealmIds.FuQiZhenRen, StringComparison.Ordinal))
		{
			hasMatureRealmFoundation = XjActorAccessor.TryGetInt(
				actor, XjActorDataKeys.FuQiShenMiaoPerfectionYear, out int perfectedYear)
				&& perfectedYear > 0;
		}
		else
		{
		...
		if (!hasMatureRealmFoundation) return false;
	...
	internal static int ResolveChancePercent(Actor actor, string realmId)
	{
		...
		bool personalJinDanForge = IsPersonalJinDanForge(actor, realmId);
		if (!personalJinDanForge && !XjCraftTraitRules.CanRefineArtifacts(actor)) return 0;
		...
		{
			baseChance = XjXianJiAccessor.GetEffectiveShenTongCount(actor) >= 3
	...
	private static bool IsPersonalJinDanForge(Actor actor, string realmId)
	{
		if (actor?.data == null
		...
		// 道胎仍沿用金丹级器物体系，不另造一套“道胎法宝”炼制生产线；
		// 因此在法宝炼制策略内按金丹及以上承载，但不改全局修炼等价关系。
		return XjCultivationPathRules.IsJinDanEquivalent(actor)
	...
	internal static int ResolveManagedItemLimit(string realmId)
	{
		...
		if (XjHighRealmIdentity.IsZhenRen(realmId)) return ZiFuMaxManagedItems;
		if (XjHighRealmIdentity.IsZhenJun(realmId)) return JinDanMaxManagedItems;
		return 0;
	}
	...
	{
		if (string.Equals(realmId, XjRealmIds.ZhuJi, StringComparison.Ordinal)
			|| string.Equals(realmId, XjRealmIds.HuangGuan, StringComparison.Ordinal)) return ZhuJiWarehouseLifetimeLimit;
		if (XjCultivationPathRules.IsZhenRenEquivalentRealm(realmId)) return ZiFuWarehouseLifetimeLimit;
		if (XjCultivationPathRules.IsJinDanEquivalentRealm(realmId)) return JinDanWarehouseLifetimeLimit;
		return 0;
	}
	...
	{
		if (string.Equals(realmId, XjRealmIds.ZhuJi, StringComparison.Ordinal)
			|| string.Equals(realmId, XjRealmIds.HuangGuan, StringComparison.Ordinal)) return XjActorDataKeys.XjFaBaoWarehouseFaQiCount;
		if (XjCultivationPathRules.IsZhenRenEquivalentRealm(realmId)) return XjActorDataKeys.XjFaBaoWarehouseLingBaoCount;
		if (XjCultivationPathRules.IsJinDanEquivalentRealm(realmId)) return XjActorDataKeys.XjFaBaoWarehouseFaBaoCount;
		return string.Empty;
	}
	...
	private static string ReadGuoWei(Actor actor)
	{
		if (XjGuoWeiQuanBingRegistry.TryGetForLiveDisplay(actor, out XjGuoWeiQuanBingState authority)
			&& authority.Found
			&& !string.IsNullOrWhiteSpace(authority.GuoWei))
		{
			return authority.GuoWei.Trim();
		}

		XjJinDanState state = XjJinDanAccessor.BuildPositionCarrierState(actor);
		return state.Found && !string.IsNullOrWhiteSpace(state.GuoWei)
			? state.GuoWei.Trim()
			: string.Empty;
	}
}
