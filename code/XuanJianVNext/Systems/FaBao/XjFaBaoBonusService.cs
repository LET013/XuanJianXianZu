...
{
	private static readonly Dictionary<long, CachedProfile> ProfileCache = new Dictionary<long, CachedProfile>();
	private static readonly object ProfileCacheLock = new object();
	private const string NoFaBaoSignature = "<none>";
	// 法宝加成会被战斗、寿元、突破和面板同时读取。状态写入会主动 Forget，
	// 因此同一短帧窗口内可以复用结果，避免反复拼接装备签名造成 GC 峰值。
	private const int ProfileValidationFrames = 12;
	...
	private static string BuildBonusText(in XjFaBaoBonusProfile profile)
	{
		...
		AddPercent(parts, "慧光加成", profile.HuiGuangBonus);
	...
	internal static string BuildDescription(in XjFaBaoState state)
	{
		...
		string classText = XjFaBaoCatalog.IsJinDanFaBao(state.ClassName)
	...
	internal static int GetOverviewPower(in XjFaBaoState state)
	{
		...
		return XjFaBaoCatalog.IsJinDanFaBao(state.ClassName) ? 30
