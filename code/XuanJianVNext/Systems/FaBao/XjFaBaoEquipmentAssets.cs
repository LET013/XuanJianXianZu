...
internal static class XjFaBaoEquipmentAssets
{
	...
	private static void AddDaoTuSlotDefinitions(List<XjFaBaoEquipmentDefinition> result)
	{
		...
		AddDaoTuSlotSeries(result, "amulet", "灵符", "Amulet", "Amulet", EquipmentType.Amulet, XjAmuletGroupId);
	}
	...
	private static void RegisterEquipment(in XjFaBaoEquipmentDefinition definition)
	{
		...
		try
		{
			...
			((BaseUnlockableAsset)asset).base_stats = BuildNativeBaseStats(in definition);
	...
	private static BaseStats BuildNativeBaseStats(in XjFaBaoEquipmentDefinition definition)
	{
		...
		try
		{
			...
			{
				stats.set("damage", 1000f);
				stats.set("damage_range", 0.6f);
				stats.set("attack_speed", 8f);
				stats.set("crit", 0.03f);
				return stats;
			}
			if (string.Equals(definition.Role, XjFaBaoCatalog.RoleDefense, StringComparison.Ordinal))
			{
				stats.set("health", 200000f);
				switch (definition.EquipmentType)
				{
					case EquipmentType.Helmet: stats.set("armor", 6f); break;
					case EquipmentType.Armor: stats.set("armor", 12f); break;
					case EquipmentType.Boots: stats.set("speed", 8f); stats.set("dodge", 0.04f); break;
					case EquipmentType.Ring: stats.set("crit", 0.02f); break;
					case EquipmentType.Amulet: stats.set("armor", 4f); break;
				}
			}
			// 辅助类法宝只承载法宝词条，不写入攻击类基础伤害。
		}
		catch { }
		return stats;
	}
