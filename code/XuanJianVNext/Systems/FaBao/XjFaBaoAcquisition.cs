...
internal static class XjFaBaoAcquisition
{
	...
	internal const string SourceDaoTaiXianQi = "DaoTaiXianQi";
	internal const string SourceDaoTaiAscensionGrant = "DaoTaiAscensionGrant";
	...
	/// <summary>
	/// 成胎成功的固定器物结算：本命武器至少提升到金丹法宝品质。
	/// 这是境界奖励，不消耗金性、不走炼器概率，也不增加炼器熟练度；
	/// 已经是金丹法宝或仙器时绝不重铸。
	/// </summary>
	internal static bool EnsurePrimaryJinDanForDaoTaiAscension(Actor actor, string daoTu, int currentYear)
	{
		if (actor?.data == null
			|| XjCultivationPathRules.IsShi(actor)
			|| XjRealmSuppression.GetRealmTier(actor) < XjRealmSuppression.TierDaoTai
		...
		XjFaBaoState existing = XjFaBaoAccessor.BuildState(actor);
		if (existing.Found
			&& (XjFaBaoCatalog.IsJinDanFaBao(existing.ClassName) || XjFaBaoCatalog.IsXianQi(existing.ClassName)))
		{
			return true;
		}
		if (existing.Found && XjFaBaoCatalog.IsZiFuLingBao(existing.ClassName))
		{
			return UpgradePrimaryLingBao(actor, existing, daoTu, Math.Max(1, currentYear), SourceDaoTaiAscensionGrant);
		}

		long actorId = GetActorId(actor);
		int ordinal = XjDeterministicHash.PositiveIndex(actorId + Math.Max(1, currentYear),
			"daotai_ascension_primary|" + daoTu, 997);
		if (!TryCreateGeneratedState(
			actor, daoTu, XjFaBaoCatalog.JinDanFaBaoClass, SourceDaoTaiAscensionGrant,
			Math.Max(1, currentYear), ordinal, out XjFaBaoState created,
			forcedRole: XjFaBaoCatalog.RoleAttack))
		{
			return false;
		}

		WriteAndPublish(actor, created);
		return XjFaBaoAccessor.BuildState(actor).Found;
	}

	/// <summary>
	/// 道胎仙器只允许由本人现有的“金丹本命法宝”同器升格。
	/// 不重新抽器型、不改名字、不换ID，因此它仍是同一件本命法宝，只是位格和数值跃升。
	/// </summary>
	internal static bool TryUpgradePrimaryJinDanToXianQi(Actor actor, int currentYear)
	{
		if (actor?.data == null
			|| currentYear <= 0
			|| XjCultivationPathRules.IsShi(actor)
		...
		XjFaBaoState existing = XjFaBaoAccessor.BuildState(actor);
		if (!existing.Found
			|| !string.Equals(existing.ClassName, XjFaBaoCatalog.JinDanFaBaoClass, StringComparison.Ordinal))
		{
			return false;
		}

		string role = XjFaBaoCatalog.NormalizeRole(existing.Kind, existing.Role);
		string promotedAffixes = PromoteAffixesToXianQi(existing.Affixes, role);
		string upgradeAffixes = MergeUpgradeAffixes(
			actor, promotedAffixes, XjFaBaoCatalog.XianQiClass, role, currentYear, 71);
		string description = XjFaBaoDescriptionFormatter.BuildGeneratedDescription(
			actor, existing.Name, existing.DaoTu, XjFaBaoCatalog.XianQiClass,
			existing.Kind, role, SourceDaoTaiXianQi);
		XjFaBaoState upgraded = new XjFaBaoState(
			true,
			string.IsNullOrWhiteSpace(existing.Id)
				? BuildFaBaoId(actor, existing.DaoTu, XjFaBaoCatalog.XianQiClass, currentYear, 71)
				: existing.Id,
			existing.Name, existing.DaoTu, XjFaBaoCatalog.XianQiClass,
			existing.Kind, role, upgradeAffixes, description,
			SourceDaoTaiXianQi, currentYear, "Ok");
		WriteAndPublish(actor, upgraded);
		return true;
	}

	private static string PromoteAffixesToXianQi(string affixes, string role)
	{
		if (string.IsNullOrWhiteSpace(affixes)) return string.Empty;
		string[] raw = affixes.Split(new[] { '/', '-' }, StringSplitOptions.RemoveEmptyEntries);
		List<string> promoted = new List<string>(raw.Length);
		for (int i = 0; i < raw.Length; i++)
		{
			...
			// 原法宝词条进入仙器后直接保留并提高75%，不通过重铸洗词条。
			// “每秒回血”本身是小数百分比，统一倍率不会膨胀到异常值。
			float upgraded = Math.Clamp(value * 1.75f, 0f, 50f);
			string formatted = upgraded.ToString(upgraded < 2f ? "0.0" : "0.#",
				System.Globalization.CultureInfo.InvariantCulture);
			promoted.Add(label + "+" + formatted + "%");
		}
		return XjFaBaoCatalog.NormalizeAffixesForClass(
			string.Join("/", promoted), role, XjFaBaoCatalog.XianQiClass);
	}
	...
	private static void TryRefineJieLinFaBao(Actor actor, int currentYear, bool promotionAttempt)
	{
		...
		string daoTu = XjXuanJianShenTongSpecials.IsYuYiXian(actor) ? "太阳" : "太阴";
		if (!(promotionAttempt
	...
	private static string BuildAffixSummary(Actor actor, string className, string role, int year, int ordinal)
	{
		...
		for (int attempt = 0; parts.Count < count && attempt < labels.Length * 2; attempt++)
		{
			...
			float valueCap = isXianQi ? 50f : isJinDan ? 30f : isFaQi ? 5f : 10f;
	...
	internal static void WriteAndPublish(Actor actor, in XjFaBaoState state)
	{
		...
		bool isUpgrade = string.Equals(state.Source, SourceLingBaoUpgrade, System.StringComparison.Ordinal)
			...
			|| string.Equals(state.Source, SourceDaoTaiXianQi, System.StringComparison.Ordinal);
		long familyStableId = ResolveLiveFaBaoFamilyId(actor, state);
		...
		{
			XuanJianVNext.Systems.Broadcast.XjBroadcastSystem.ShowRecordedCategorizedWorldTip(
	...
	/// 也不生成临时装备对象。本命主器始终留在人物身上；只有真实余器在家族与所属宗门之间低频分流。
	/// </summary>
	internal static bool TryStoreSurplusCraft(Actor actor, in XjFaBaoState state)
	{
		...
		if (routeToSect)
		{
			stored = XjFamilyFaBaoWarehouse.AddFaBaoToSect(
				actorId,
				XjStringHelper.ActorName(actor, "未名炼器师"),
				sectId,
				sect.Name,
				state.Id,
				state.Name,
	...
	private static bool ShouldRouteSurplusCraftToSect(long familyStableId, long sectId)
	{
		...
		// 这里只有“角色装备位已满后炼出的余器”。本命主器永远留在人物身上，不参与家族/宗门器库分流。
		return sectOwnCount * 3 <= familyCount;
	}

	private static long ResolveLiveFaBaoFamilyId(Actor actor, in XjFaBaoState state)
	{
		long actorId = GetActorId(actor);
		if (actorId <= 0L || !state.Found) return 0L;

		// 本命主器的唯一所有权在人物自身。旧版本会把同一件器物同时写进家族器库，
		// 这会让去重、借用和宗门转库把“活人的本命器”当成仓储资产。新写入时先
		// 清除旧错误映射，历史记录仍可附带家族编号，但器物本身不进入器库。
		if (!string.IsNullOrWhiteSpace(state.Id) && !string.IsNullOrWhiteSpace(state.DaoTu))
		{
			XjFamilyFaBaoWarehouse.RemoveLiveBoundPrimaryEntries(actorId, state.Id, state.DaoTu, state.Source);
		}

		return XjFamilyReadModel.Shared.TryGetConfirmedFamilyStableId(actorId, out long familyStableId)
			&& familyStableId > 0L
			? familyStableId
			: 0L;
	}
		...
		long sectId = 0L)
	{
		...
		long actorId = GetActorId(actor);
		string actorName = XjStringHelper.ActorName(actor, "未名炼器师");
	...
	private static bool ShouldAnnounceFaBaoResult(string source)
	{
		...
		return string.Equals(value, SourceJinDan, StringComparison.Ordinal)
			...
			|| string.Equals(value, SourceDaoTaiXianQi, StringComparison.Ordinal);
	}
	...
	private static string BuildFaBaoId(Actor actor, string daoTu, string className, int year, int ordinal)
	{
		...
		string tier = XjFaBaoCatalog.IsXianQi(className) ? "xianqi"
			: XjFaBaoCatalog.IsJinDanFaBao(className) ? "jindan"
