...
internal static class XjFaBaoEquipmentSync
{
	...
	private const int CurrentItemSchemaVersion = 8;
	...
	internal static bool TryBorrowFamilyFaBao(Actor actor, string realmId, int currentYear)
	{
		...
		if (current.Found)
		{
			// 家族借器只能填补“没有个人本命主器”的空缺，绝不能为了借一件更高品阶器物
			// 覆盖人物自己的本命灵宝/法宝。已有借器可以在家族器库中换成更高一档，
			// 但个人本命链始终优先且不可被借器状态取代。
			if (!string.Equals((current.Source ?? string.Empty).Trim(), FamilyBorrowSource, StringComparison.Ordinal)) return false;
			if (ResolveClassRank(current.ClassName) >= entryRank) return false;
		}
	...
	private static bool TryFindBestFamilyFaBao(Actor actor, out XjFamilyFaBaoWarehouseEntry best, out int bestRank)
	{
		...
		for (int i = 0; i < entries.Count; i++)
		{
			...
			// 兼容旧档：活着原主人的本命主器即使尚未来得及年度清库，也绝不能被
			// 当作“家族可借器物”。这样不会出现同一件本命器同时挂在两个人身上。
			if (XjFamilyFaBaoWarehouse.IsLiveBoundPrimaryEntry(in candidate)) continue;
	...
	internal static void TrySyncGeneratedEquipment(Actor actor, in XjFaBaoState state)
	{
		...
		EnsureBoundPrimaryOwnership(item, actor, state);
		XjPresentationHooks.RegisterTooltipTexts(state.Name, state.ClassName, state.Description, state.Affixes);
	}
	...
	/// <summary>
	/// AVBS replacement creates a new Actor id while copying equipped items. Rebind every
	/// equipped managed XuanJian item that still names the disposable source actor so the
	/// replacement is not treated as a foreign borrower of its own FaBao/LingBao.
	/// </summary>
	internal static void RebindEquippedOwnersAfterExternalReplacement(Actor actor, long oldActorId)
	{
		if (actor?.data == null || actor.equipment == null || oldActorId <= 0L) return;
		...
		foreach (ActorEquipmentSlot slot in actor.equipment)
		{
			...
			item.data.get("xuanjian.fabao.owner_id", out long ownerId, 0L);
			if (ownerId != oldActorId) continue;
			item.data.set("xuanjian.fabao.owner_id", newActorId);
			changed = true;
		}

		if (!changed) return;
		XjFaBaoBonusService.Forget(oldActorId);
		XjFaBaoBonusService.Forget(newActorId);
		actor.setStatsDirty();
	}
	...
	internal static void ApplyFaBaoItemData(Item item, in XjFaBaoState state, bool clearNativeModifiers = false)
	{
		...
		string normalizedAffixes = XjFaBaoCatalog.NormalizeAffixesForClass(
			state.Affixes ?? string.Empty,
			XjFaBaoCatalog.NormalizeRole(state.Kind, state.Role),
			state.ClassName ?? string.Empty);
		item.data.set(ItemKeyAffixes, normalizedAffixes);
		...
		catch (System.Exception xjCaught789) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/FaBao/XjFaBaoEquipmentSync.cs:789", xjCaught789); }
	}
	...
	internal static void ApplyNativeClassStats(Item item, string className)
	{
		...
		// 模板或旧档残留的攻击数值必须清零；防御生命按器物层级结算，
		// 禁止筑基法器沿用紫府灵宝的20万生命。
		stats.set("damage", 0f);
		stats.set("damage_range", 0f);
		if (string.Equals(definition.Role, XjFaBaoCatalog.RoleDefense, StringComparison.Ordinal))
		{
			stats.set("health", ResolveNativeClassHealth(className));
		}
	}

	private static int ResolveNativeClassDamage(string className)
	{
		if (XjFaBaoCatalog.IsXianQi(className)) return 50000;
		if (XjFaBaoCatalog.IsJinDanFaBao(className)) return 10000;
		if (XjFaBaoCatalog.IsZiFuLingBao(className)) return 4500;
		if (XjFaBaoCatalog.IsZhuJiFaQi(className)) return 300;
		return 0;
	}

	private static int ResolveNativeClassHealth(string className)
	{
		if (XjFaBaoCatalog.IsXianQi(className)) return 2000000;
		if (XjFaBaoCatalog.IsJinDanFaBao(className)) return 500000;
		if (XjFaBaoCatalog.IsZiFuLingBao(className)) return 200000;
		if (XjFaBaoCatalog.IsZhuJiFaQi(className)) return 12000;
		return 0;
	}
	...
	private static void RemoveLooseItem(Item item)
	{
		...
		catch (System.Exception xjCaught898) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/FaBao/XjFaBaoEquipmentSync.cs:898", xjCaught898); }
	}
	...
	private static void MarkActorStatsDirty(Actor actor)
	{
		...
		catch (System.Exception xjCaught909) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/FaBao/XjFaBaoEquipmentSync.cs:909", xjCaught909); }
	}
