...
{		private static void TrySoftSplitSect(long sectId, int currentYear)
		...
		private static List<long> SelectRemoteCityCluster(XjSectArchiveRecord sect, City capital)
		{
			...
			bool hasCapitalPosition = XjNativeMapPositionInterop.TryResolve(capital, out Vector3 capitalPosition);
			for (int i = 0; i < sect.CityIds.Count; i++)
			{
				...
				if (hasCapitalPosition && XjNativeMapPositionInterop.TryResolve(city, out Vector3 position))
				{
		...
		private static bool TrySelectSoftSplitFounder(XjSectArchiveRecord sect, IReadOnlyList<long> remoteCities, out Actor founder, out long familyId)
		{
			...
			IReadOnlyDictionary<long, List<long>> index = XjSectCultivatorCityIndex.GetCityIndex();
			foreach (KeyValuePair<long, List<long>> pair in index)
			{
				if (pair.Key <= 0L || pair.Value == null || !citySet.Contains(pair.Key)) continue;
		...
		{
			if (sect == null || cityIds == null || !XjSectOwnership.TryResolvePrimaryCity(sect, out City zongMenCity)
			...
			foreach (KeyValuePair<long, List<long>> pair in index)
			{
				...
				for (int i = 0; i < pair.Value.Count; i++)
				{
					...
					XjSectMembershipService.EnsureMember(zongMenCity, actor, currentYear, "SoftSplitMember");
				}
			}
