...
internal static partial class XjSectGovernanceRuntimeLane
{
		...
		out string actorName)
	{
		...
		for (int i = 0; i < seats.Count; i++)
		{
			...
			{
				if (XjZongMenQiuJinFaBorrow.TryBorrowForActor(actor)) granted = "求金法";
				else if (XjZongMenGongFaBorrow.TryBorrowForActor(actor)) granted = "五品功法";
			}
			else if (string.Equals(state.SupportPurpose, XjCenturyFamilySupportPurpose.FuChiZiFu, StringComparison.Ordinal))
			{
				if (XjZongMenGongFaBorrow.TryBorrowForActor(actor)) granted = "合法功法";
			}
			else if (string.Equals(state.SupportPurpose, XjCenturyFamilySupportPurpose.FuChiZhuJi, StringComparison.Ordinal))
			{
				if (XjZongMenCaiQiFaBorrow.TryBorrowForActor(actor)) granted = "采气法";
				else if (XjZongMenGongFaBorrow.TryBorrowForActor(actor)) granted = "合法功法";
			}
