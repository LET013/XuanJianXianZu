...
internal static partial class XjSectWarSystem
{
	...
	private const int MaxIndexedActorsPerSect = 128;
	...
	private static bool CreateWar(Candidate attacker, Candidate defender, int currentYear, string reason)
	{
		...
		XjSectWarArchiveRecord record = new XjSectWarArchiveRecord
		{
			...
			WarGoal = "破阵夺势",
			Reason = string.IsNullOrWhiteSpace(reason) ? "宗门高境冲突" : reason.Trim(),
	...
	private static float ComputeHighRealmScore(long sectId, List<long> participantIds)
	{
		...
		float score = 0f;
		int scanned = 0;
		for (int i = 0; i < actorIds.Count && scanned < MaxIndexedActorsPerSect; i++)
		{
		...
		string eventType)
	{
		...
		if (importance >= 5)
		{
			XjBroadcastSystem.ShowWorldTipCritical(summary, color: "#D94C4C", duration: 9f);
		}
		else if (importance >= 4)
		{
			XjBroadcastSystem.ShowWorldTipCritical(summary, color: "#F3961F", duration: 8f, delayFrames: 1);
		}
	...
	private static XjSectWarArchiveRecord Normalize(XjSectWarArchiveRecord source)
	{
		...
		source.Reason = source.Reason ?? string.Empty;
