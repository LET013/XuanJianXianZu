...
{		internal static bool TryBindSecretRealm(long sectId, long realmId, string status)
		...
		internal static int CountEstablishedSects()
		{
			if (BySectId.Count == 0) return 0;
			int count = 0;
			foreach (XjSectArchiveRecord record in BySectId.Values)
			{
				if (IsEstablishedSect(record)) count++;
			}
			return count;
		}

		internal static void CopyEstablishedSectIdsTo(List<long> target)
		{
			if (target == null) return;
			target.Clear();
			if (BySectId.Count == 0) return;
			foreach (XjSectArchiveRecord record in BySectId.Values)
			{
				if (IsEstablishedSect(record) && record.SectId > 0L) target.Add(record.SectId);
			}
			target.Sort();
		}
