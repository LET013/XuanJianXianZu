...
internal static partial class XjSectWarSystem
{
	...
	private const int MaximumReasonInputLength = 4096;
	private const int MaximumReasonDisplayLength = 240;
	...
	internal static bool AddHostility(long aggressorSectId, long targetSectId, int amount, int currentYear, string reason)
	{
		...
		record.LastReason = NormalizeReasonForDisplay(reason, "宗门冲突");
	...
	internal static void ObserveCrossSectHighRealmDeath(in XjDeathSnapshot snapshot)
	{
		if (!snapshot.Found
			...
			|| !XjHighRealmIdentity.IsHighRealm(snapshot.RealmId)
		...
		int hostility = string.Equals(realmId, XjRealmIds.DaoTai, StringComparison.Ordinal)
			|| string.Equals(realmId, XjRealmIds.FuQiDaoTai, StringComparison.Ordinal)
				? 70
				: XjHighRealmIdentity.IsZhenJun(realmId) ? 45 : 25;
		string guoWei = snapshot.GuoWei ?? string.Empty;
		if (guoWei.Contains(XjGuoWeiCalculator.ZhengWei, StringComparison.Ordinal)) hostility += 10;
		else if (guoWei.Contains(XjGuoWeiCalculator.YuWei, StringComparison.Ordinal)) hostility += 5;
		AddHostility(
			attackerSectId,
			victimSectId,
			hostility,
	...
	private static bool GenerateConflictEvents(int currentYear)
	{
		...
		int validSectCount = XjSectRepository.CountEstablishedSects();
	...
	private static bool TryStartThresholdWars(int currentYear)
	{
		...
		for (int i = 0; i < HostilityBuffer.Count && CountActiveWars() < MaxActiveWars; i++)
		{
			...
			{
				// 敌意可以很深，但没有足以承担后果的高境战力就不会发动
				// 自杀式宗战。退回临界线下，后续有新血债或实力变化再评估。
	...
	private static bool CanGenerateConflict(XjSectHostilityArchiveRecord record, int currentYear)
	{
		return record != null
			&& record.LeftSectId > 0L
			&& record.RightSectId > 0L
			&& TryBuildCandidate(record.LeftSectId, out _)
			&& TryBuildCandidate(record.RightSectId, out _)
	...
	private static long ResolveConflictAggressor(XjSectHostilityArchiveRecord record, int currentYear)
	{
		...
		if ((record.LastAggressorSectId == record.LeftSectId || record.LastAggressorSectId == record.RightSectId)
			&& CanSectInitiateWar(
				...
				record.LastAggressorSectId == record.LeftSectId ? record.RightSectId : record.LeftSectId))
		{
			return record.LastAggressorSectId;
		}

		bool leftCan = CanSectInitiateWar(record.LeftSectId, record.RightSectId);
		bool rightCan = CanSectInitiateWar(record.RightSectId, record.LeftSectId);
		if (leftCan && !rightCan) return record.LeftSectId;
		if (rightCan && !leftCan) return record.RightSectId;
	...
	private static void PublishConflictEvent(XjSectHostilityArchiveRecord record, int amount, int currentYear)
	{
		...
		{
			XjBroadcastSystem.ShowRecordedWorldTipCritical(summary, color: "#C96A3D", duration: 7f, delayFrames: 1);
		}
	}
	...
	private static void PublishHostilityThreshold(XjSectHostilityArchiveRecord record, int previous, int currentYear)
	{
		...
		string leftName = ResolveSectName(record.LeftSectId);
		string rightName = ResolveSectName(record.RightSectId);
		bool hasViableAggressor = CanSectInitiateWar(record.LeftSectId, record.RightSectId)
			|| CanSectInitiateWar(record.RightSectId, record.LeftSectId);
		string summary = threshold == 100
			? hasViableAggressor
				? leftName + "与" + rightName + "敌意已至开战临界；只有攻方高境足以承受对手、双方护宗大阵可用时，宗门大战才会爆发。"
				: leftName + "与" + rightName + "积怨虽已至极深，但当前没有足以承受宗战的高境力量，双方仍不敢越界。"
			: threshold == 80
				? hasViableAggressor
					? leftName + "与" + rightName + "敌意深重，高境力量已足以令冲突逼近宗门大战。"
					: leftName + "与" + rightName + "敌意深重，但弱方高境不足，积怨暂不会演变为主动寻仇或宗门大战。"
		...
		if (threshold >= 80)
		{
			XjBroadcastSystem.ShowRecordedWorldTipCritical(summary, color: threshold == 100 ? "#D94C4C" : "#C96A3D", duration: 8f, delayFrames: 1);
		}
	}
	...
	private static void RegisterWarStarted(long attackerSectId, long defenderSectId, int currentYear, string reason)
	{
		...
		record.LastReason = NormalizeReasonForDisplay(reason, "宗门敌意达到临界");
	}
	...
	internal static string NormalizeReasonForDisplay(string reason, string fallback)
	{
		string normalized = NormalizeHostilityReason(reason);
		if (normalized.Length == 0) normalized = NormalizeHostilityReason(fallback);
		return normalized;
	}

	private static string NormalizeHostilityReason(string source)
	{
		if (string.IsNullOrWhiteSpace(source)) return string.Empty;
		string compact = CompactReasonWhitespace(source);
		...
		{
			char c = compact[i];
			if (IsReasonSeparator(c))
			{
				string segment = CollapseRepeatedWholeBlock(current.ToString().Trim());
				current.Clear();
				if (segment.Length > 0)
				{
		...
		string tail = CollapseRepeatedWholeBlock(current.ToString().Trim());
		if (tail.Length > 0)
		{
			segments.Add(tail);
			separators.Add('。');
		...
		StringBuilder result = new StringBuilder(Math.Min(compact.Length, MaximumReasonDisplayLength + 16));
		for (int i = 0; i < segments.Count; i++)
		{
			...
			result.Append(key);
			char separator = i < separators.Count ? separators[i] : '。';
			if (separator == ',' || separator == '，') separator = '，';
			else if (separator == ';' || separator == '；') separator = '；';
			else if (separator != '！' && separator != '？') separator = '。';
			result.Append(separator);
			if (result.Length >= MaximumReasonDisplayLength) break;
		}

		string output = CollapseRepeatedWholeBlock(result.ToString().Trim());
		if (output.Length > MaximumReasonDisplayLength)
		{
			int cut = MaximumReasonDisplayLength - 1;
			for (int i = cut; i >= Math.Max(0, cut - 36); i--)
			{
				if (IsReasonSeparator(output[i]))
				{
					cut = i + 1;
					break;
				}
			}
			output = output.Substring(0, Math.Min(cut, output.Length)).TrimEnd() + "…";
		}
		return output;
	}

	private static string CompactReasonWhitespace(string source)
	{
		int length = Math.Min(source.Length, MaximumReasonInputLength);
		...
		for (int i = 0; i < length; i++)
		{
			...
			if (pendingSpace && !IsReasonSeparator(c) && builder.Length > 0 && !IsReasonSeparator(builder[builder.Length - 1])) builder.Append(' ');
			pendingSpace = false;
			builder.Append(c);
		}
	...
	private static bool IsReasonSeparator(char c)
	{
		return c == '。' || c == '！' || c == '？' || c == '；' || c == ';' || c == '，' || c == ',';
	}

	private static string CollapseRepeatedWholeBlock(string value)
	{
		if (string.IsNullOrEmpty(value) || value.Length < 8) return value ?? string.Empty;
		...
		if (period > 0 && period < length && length % period == 0)
		{
			return value.Substring(0, period).Trim();
		}
		return value;
	}
	...
	private static XjSectHostilityArchiveRecord NormalizeHostility(XjSectHostilityArchiveRecord source)
	{
		...
		source.LastReason = NormalizeReasonForDisplay(source.LastReason, string.Empty);
		return source;
	}
	...
	private static XjSectHostilityArchiveRecord CloneHostility(XjSectHostilityArchiveRecord source)
	{
		...
		return new XjSectHostilityArchiveRecord
		{
			...
			LastReason = NormalizeReasonForDisplay(source.LastReason, string.Empty)
		};
	}
}
