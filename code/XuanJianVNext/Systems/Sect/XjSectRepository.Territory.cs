...
internal static partial class XjSectRepository
{
		...
		{
			if (city?.data != null && sectId > 0L && XjZongMenCityData.GetZongMenId(city) == sectId)
			{
				XjZongMenCityData.Clear(city);
			}
		...
		internal static bool TryDefeatSectByFormationBreak(long defeatedSectId, long victorSectId, int currentYear, string reason)
		{
			...
			for (int i = 0; i < cityIds.Count; i++)
			{
				...
				if (city?.data != null)
				{
					XjZongMenCityData.RebindSectMirror(city, victor.SectId, victor.Name);
				}
		...
		private static void CollectDefeatedSectMemberIds(XjSectArchiveRecord defeated, List<long> cityIds)
		{
			...
			IReadOnlyList<long> indexedIds = XjZongMenCultivatorCityIndex.GetActorIdsForSect(defeated.SectId);
			...
			for (int i = 0; i < cityIds.Count; i++)
			{
				...
				if (city?.data == null
					...
					|| XjZongMenCityData.GetZongMenId(city) != defeated.SectId)
				{
					continue;
				}
				List<long> memberIds = XjZongMenCityData.GetMemberIds(city);
		...
		private static int AbsorbDefeatedSectMembers(XjSectArchiveRecord victor, int currentYear)
		{
			...
			for (int i = 0; i < SectMemberMirrorBuffer.Count; i++)
			{
				...
				bool added = XjZongMenMembershipWriter.EnsureMember(victorCity, actor, currentYear, "SectDestroyedAbsorbMember");
				if (added || previousSectId != victor.SectId)
				{
					if (XjZongMenCityData.IsMember(victorCity, actor))
					{
						absorbed++;
					}
				}
			...
			SectMemberMirrorBuffer.Clear();
			SectMemberTransferSeen.Clear();
			XjZongMenCityData.AssignRolesFromScratch(victorCity, currentYear);
		...
		private static bool TryResolvePrimarySectCity(XjSectArchiveRecord record, out City city)
		{
			...
			if (record.CapitalCityId > 0L)
			{
				city = ResolveCityById(record.CapitalCityId);
				if (city?.data != null
					&& XjZongMenCityData.HasZongMen(city)
					&& XjZongMenCityData.GetZongMenId(city) == record.SectId)
				{
					return true;
				}
			}
			if (record.CityIds != null)
			{
				for (int i = 0; i < record.CityIds.Count; i++)
				{
					city = ResolveCityById(record.CityIds[i]);
					if (city?.data != null
						&& XjZongMenCityData.HasZongMen(city)
						&& XjZongMenCityData.GetZongMenId(city) == record.SectId)
					{
			...
			if (XjZongMenCityData.TryResolveZongMenCity(record.SectId, out city)
				&& city?.data != null
				&& XjZongMenCityData.HasZongMen(city)
				&& XjZongMenCityData.GetZongMenId(city) == record.SectId)
			{
		...
		private static void ClearActorMirrorsForSect(long sectId)
		{
			...
			IReadOnlyList<long> indexedIds = XjZongMenCultivatorCityIndex.GetActorIdsForSect(sectId);
			SectMemberMirrorBuffer.Clear();
			...
			for (int i = 0; i < SectMemberMirrorBuffer.Count; i++)
			{
				...
				if (ReadActorSectId(actor) == sectId) XjZongMenAccessor.WriteIdentity(actor, XjZongMenIdentitySnapshot.Empty);
			}
			SectMemberMirrorBuffer.Clear();
		}
		...
		private static void ClearCityZongMenForSect(XjSectArchiveRecord record)
		{
			...
			for (int i = 0; i < record.CityIds.Count; i++)
			{
				...
				if (city?.data == null || !XjZongMenCityData.HasZongMen(city) || XjZongMenCityData.GetZongMenId(city) != record.SectId) continue;
				XjZongMenCityData.Clear(city);
			}
		}

		private static bool IsFunctionallyExtinct(XjSectArchiveRecord record)
		{
			if (record == null || record.SectId <= 0L) return false;
			if (record.CityIds != null && record.CityIds.Count > 0) return false;
			if (record.SovereignActorId > 0L
				&& XjScheduler.ResolveActor(record.SovereignActorId, out Actor sovereign)
				&& sovereign?.data != null
				&& sovereign.isAlive()
				&& ReadActorSectId(sovereign) == record.SectId)
			{
				return false;
			}
	
			IReadOnlyList<long> ids = XjZongMenCultivatorCityIndex.GetActorIdsForSect(record.SectId);
