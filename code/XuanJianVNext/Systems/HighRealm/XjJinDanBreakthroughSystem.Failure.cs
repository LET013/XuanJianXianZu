...
{		internal static void ReconcileFailureDemonization(Actor actor)
		...
		private static void ResolveTerminalFailure(Actor actor, int failureYear, string reason)
		{
			...
			if (TryCreateJinXingYaoXieOnFailure(actor, safeYear))
			{
				return;
			}
			...
			bool died = XuanJianVNext.Systems.Combat.XjVanillaDeathGuard.TryExecuteForceDeath(actor, (AttackType)5, true);
			if (died)
			{
		...
		private static string BuildZhengWeiSchemerFailureNarrative(Actor actor, in XjGuoWeiRegistryEntry schemer)
		{
			...
			return "因正位" + schemerName + "暗中算计，" + actorName + "道势失衡，求金失败。";
		}
		...
		private static string StripRealmSuffix(string value)
		{
			...
			string[] suffixes = { "-金丹", "-神丹", "-结璘仙", "-紫府", "-筑基", "-炼气", "-胎息" };
		...
		private static bool IsHiddenYinSiZhengWeiAttempt(string daoTu, string guoWeiType)
		{
			string candidate = XjGuoWeiCalculator.BuildGuoWeiSlotName(daoTu, guoWeiType, 1);
			return XjGuoWeiRegistry.IsHiddenYinSiZhengWei(daoTu, guoWeiType, candidate);
		}

		private static void BroadcastHiddenZhengWeiOccupied(Actor actor)
		{
			if (!XjSafeCore.IsAliveActor(actor))
			{
				return;
			...
			string text = actorName + "欲证谪炁正位，却只觉冥冥中正位已有主，金性不得落定。";
		...
		private static string ResolveShenTongDisplayName(string raw)
		{
			...
			if (string.Equals(value, "结璘章", StringComparison.Ordinal)
				|| string.Equals(value, XjXuanJianShenTongSpecials.JieLinZhangTraitId, StringComparison.OrdinalIgnoreCase)
		...
		private static void ApplyJinXingYaoXieName(Actor actor, string shenTong)
		{
			...
			if (!string.Equals(actor.name, fullName, StringComparison.Ordinal))
			{
				actor.name = fullName;
			}
