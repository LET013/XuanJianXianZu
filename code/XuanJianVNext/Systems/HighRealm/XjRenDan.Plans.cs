...
internal static partial class XjRenDan
{
	...
	/// <summary>
	/// 在第4/第5仙基尝试中，紫府从其他小族中暗定一名同道途筑基作为人丹。
		...
		int currentYear)
	{
		...
		if (entriesByActorId.TryGetValue(actorId, out XjRenDanState existing)
			&& existing.Found
			&& (string.Equals(existing.Stage, XjRenDanStages.Prepared, StringComparison.Ordinal)
		...
		if (XjDeterministicHash.PositiveIndex(actorId + currentYear + targetCount, snapshot.DaoTu + "|rendan_plan", 100) >= chanceThreshold)
		{
			return false;
		}

		if (!TryResolveMappedGongFaForRenDan(actor, snapshot.DaoTu, state, targetCount, out string plannedXianJi, out _))
		{
			return false;
		}
		...
		int currentYear)
	{
		...
		if (string.Equals(outcome, XjRenDanOutcomes.Success, StringComparison.Ordinal))
		{
			...
			if (!TryResolveMappedGongFaForRenDan(actor, snapshot.DaoTu, state, plan.ShenTongCount, out string verifiedXianJi, out string gongFaName)
			...
			((BaseSystemData)actor.data).set(XjRenDanRules.RenDanShenTongTagKey, 1);
	...
	private static bool TryResolveMappedGongFaForRenDan(
		Actor actor,
		string daoTu,
		in XjXianJiState state,
		int ordinal,
	...
	private static void ApplyPreparationAid(Actor victim)
	{
		...
		XjActorAccessor.SetFloat(victim, XjActorDataKeys.HuiGuang, (float)Math.Floor(Math.Max(0f, rawHuiGuang + PreparationHuiGuangAid)));
	}
	...
	private static void ApplyTaintedBacklash(Actor source)
	{
		...
		((BaseSystemData)source.data).set(XjRenDanRules.RenDanPollutionTagKey, 1);
	}
	...
	private static void RecordPlanPreparedHistory(Actor source, Actor victim, long victimFamilyId, int year, string plannedXianJi)
	{
		...
		string body = victimName + "得一桩来历不明的修行扶持，真元与慧光有所增长；实则" + sourceName
