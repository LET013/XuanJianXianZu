...
internal static class XjYinSiExposurePursuitSystem
{
	...
	private static readonly HashSet<long> PendingDirectPursuitMissionIds = new HashSet<long>();
	...
	internal static void TickYear(int currentYear)
	{
		...
		if (PendingKnownTargetIds.Count > 0)
		{
			...
			for (int i = 0; i < PendingTargetBuffer.Count; i++)
			{
				long actorId = PendingTargetBuffer[i];
				...
				if (currentYear - record.LastKnownYear < TargetMissionCooldownYears) continue;
				if (!XjScheduler.ResolveActor(actorId, out Actor target) || target?.data == null || !target.isAlive()) continue;
		...
		foreach (KeyValuePair<long, XjYinSiMissionArchiveRecord> pair in ByMissionId)
		{
			...
			if (mission == null || IsTerminal(mission.Stage) || mission.NextActionYear > currentYear) continue;
			DueMissionBuffer.Add(pair.Key);
		}
		DueMissionBuffer.Sort();
		for (int i = 0; i < DueMissionBuffer.Count; i++)
		{
			if (ByMissionId.TryGetValue(DueMissionBuffer[i], out XjYinSiMissionArchiveRecord mission))
			{
				AdvanceMission(mission, currentYear);
			}
		}
		DueMissionBuffer.Clear();
	}
	...
	internal static void Clear()
	{
		...
		PendingDirectPursuitMissionIds.Clear();
	...
	private static void TryTriggerDaoTaiGaze(Actor actor, XjJinDanImmortalityArchiveRecord record, string reason, int currentYear)
	{
		...
		XjBroadcastSystem.ShowWorldTipCritical("【道胎垂眸】北天无声开眼，" + name + "命痕已被阴司照见。", false, "top", 8f, "#8FD7FF");
	...
	private static void ResolveDirectPursuit(XjYinSiMissionArchiveRecord mission, Actor target, int currentYear, bool sheltered)
	{
		...
		if (PendingDirectPursuitMissionIds.Contains(mission.MissionId))
		{
		...
		PendingDirectPursuitMissionIds.Add(mission.MissionId);
		...
		if (!XjJinDanCombatApi.TryScheduleImpact(delay,
			() => ExecuteDelayedDirectPursuit(mission.MissionId, mission.TargetActorId, currentYear, killChance, roll),
			"xj_yinsi_direct_pursuit",
			out _))
		{
			ExecuteDelayedDirectPursuit(mission.MissionId, mission.TargetActorId, currentYear, killChance, roll);
		}
	...
	{
		PendingDirectPursuitMissionIds.Remove(missionId);
		...
		XjBroadcastSystem.BroadcastSLevelWorldEvent(retreatText, retreatText, "#73506E", 7f, XjEventIconCatalog.YinSiLeave);
	...
	private static bool TryExecuteDirectPursuitDeath(Actor target, XjYinSiMissionArchiveRecord mission, int currentYear, float killChance)
	{
		...
		try
		{
			...
			bool died = XjVanillaDeathGuard.TryExecuteForceDeath(target, (AttackType)YinSiDirectPursuitAttackType, false);
			if (!died)
			{
				XjActorAccessor.SetString(target, XjActorDataKeys.XjDeathAnnouncementReason, string.Empty);
				return false;
			}
			string percent = Math.Round(killChance * 100f).ToString("0", CultureInfo.InvariantCulture) + "%";
			...
			XjBroadcastSystem.BroadcastSLevelWorldEvent(text, text, "#73506E", 8f, XjEventIconCatalog.YinSiLeave);
			return true;
		}
		catch
		{
			try { XjActorAccessor.SetString(target, XjActorDataKeys.XjDeathAnnouncementReason, string.Empty); } catch { }
			return false;
		}
	}
	...
	private static void PruneAndAggro(XjYinSiMissionArchiveRecord mission, Actor target)
	{
		for (int i = mission.ManifestActorIds.Count - 1; i >= 0; i--)
		{
			long manifestId = mission.ManifestActorIds[i];
			if (!XjScheduler.ResolveActor(manifestId, out Actor manifest) || manifest?.data == null || !manifest.isAlive())
			{
				mission.ManifestActorIds.RemoveAt(i);
				RemoveManifestIndex(manifestId, mission.MissionId);
				continue;
			}
			XjYinSiTraitLifecycle.ForceMissionAggro(manifest, target);
		}
	}
	...
	private static bool IsSheltered(long actorId)
	{
		...
		{
			try { return XuanJianVNext.Systems.ZongMen.XjZongMenDongTianLifecycle.IsRetreatActive(actor); } catch { }
		}
		return false;
	}
	...
	{
		try { return actor?.getName() ?? "未名真君"; } catch { return "未名真君"; }
	}
