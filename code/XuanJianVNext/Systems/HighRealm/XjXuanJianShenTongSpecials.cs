...
internal static class XjXuanJianShenTongSpecials
{
	...
	internal const string JieLinXianTraitId = "XjJieLinXian";
	internal const string YuYiXianTraitId = "XjYuYiXian";
	internal const string LegacyJieLinZhangTraitId = "XjJieLinZhang";
	internal const string LegacyYuYiWenTraitId = "XjYuYiWen";
	...
	private const string YuYiWenXianJi = "郁仪文";
	...
	private static int _specialIdentityRemovalDepth;
	private static readonly HashSet<int> YiDuiYingMirrorActorIds = new HashSet<int>();
	private static readonly List<int> YiDuiYingMirrorActorIdScratch = new List<int>();
	internal static int KnownYiDuiYingMirrorCount => YiDuiYingMirrorActorIds.Count;
	...
	internal static bool IsJieLinXianTrait(string traitId)
	{
		return string.Equals(traitId, JieLinXianTraitId, StringComparison.Ordinal);
	}

	internal static bool IsYuYiXianTrait(string traitId)
	{
		return string.Equals(traitId, YuYiXianTraitId, StringComparison.Ordinal);
	}

	internal static bool ShouldAllowSpecialIdentityTraitRemoval(string traitId)
	{
		return _specialIdentityRemovalDepth > 0
			|| (!IsJieLinXianTrait(traitId) && !IsYuYiXianTrait(traitId));
	}

	private static void RemoveSpecialIdentityTrait(Actor actor, string traitId)
	{
		...
		{
			_specialIdentityRemovalDepth = Math.Max(0, _specialIdentityRemovalDepth - 1);
		}
	}
	...
	internal static bool IsYuYiXian(Actor actor)
	{
		return actor?.data != null
			&& XjActorAccessor.TryGetInt(actor, XjActorDataKeys.XjYuYiXian, out int value)
			&& value > 0;
	}
	...
	/// <summary>
	/// 郁仪仙/结璘仙只是求金失路后的过渡高境。正式取得余位或闰位后，
	/// 必须退出特殊仙身名册，清理特殊特质，并恢复正式金丹/真君羽士姓名投影。
	/// 年份字段保留为人物历史，不再作为活动身份判断。
	/// </summary>
	internal static void FinalizeSpecialXianPositionIdentity(Actor actor, string daoTu)
	{
		if (actor?.data == null) return;
		long actorId = ((BaseSystemData)actor.data).id;
		bool hadYuYi = IsYuYiXian(actor) || actor.hasTrait(YuYiXianTraitId);
		bool hadJieLin = IsJieLinXian(actor) || actor.hasTrait(JieLinXianTraitId);
		if (!hadYuYi && !hadJieLin) return;

		XjActorAccessor.SetInt(actor, XjActorDataKeys.XjYuYiXian, 0);
		XjActorAccessor.SetInt(actor, XjActorDataKeys.XjJieLinXian, 0);
		RemoveSpecialIdentityTrait(actor, YuYiXianTraitId);
		RemoveSpecialIdentityTrait(actor, JieLinXianTraitId);
		if (actor.hasTrait(LegacyYuYiWenTraitId)) actor.removeTrait(LegacyYuYiWenTraitId);
		if (actor.hasTrait(LegacyJieLinZhangTraitId)) actor.removeTrait(LegacyJieLinZhangTraitId);
		if (actorId > 0L)
		{
		...
		string manifestDaoTu = (daoTu ?? string.Empty).Trim();
		if (manifestDaoTu.Length == 0)
		{
			XjActorAccessor.TryGetString(actor, XjActorDataKeys.DaoTu, out manifestDaoTu);
			manifestDaoTu = (manifestDaoTu ?? string.Empty).Trim();
		}
		string formalRealm = XjCultivationPathRules.IsFuQiYangXing(actor)
			? XjRealmIds.ZhenJunYuShi
			: XjRealmIds.JinDan;
		XjActorAccessor.SetString(actor, XjActorDataKeys.XjNameRealmDisplay, string.Empty);
		XjRealmTitleApplyService.ApplyOnPromotion(actor, formalRealm, manifestDaoTu);
	...
	/// <summary>
	/// 旧档若已经让郁仪仙/结璘仙取得正式果余闰席位，却仍残留特殊仙身标记，
	/// 在世界冷启动重建时立即收口为正式金丹真君/真君羽士。
	/// </summary>
	internal static void ReconcileSpecialXianPositionIdentityOnLoad(Actor actor)
	{
		if (actor?.data == null
			|| (!IsYuYiXian(actor) && !IsJieLinXian(actor))
		...
		FinalizeSpecialXianPositionIdentity(actor, daoTu);
	}
	...
	internal static bool ReconcileYiDuiYingMirrorOnLoad(Actor actor)
	{
		...
		// 仪对影只能由原本体生成一层。旧档若已经出现“影生影”，
		// 其来源本身也是仪对影时，直接清理该递归产物，绝不允许形成链式复制。
		if (TryResolveActor(sourceActorId, out Actor recursiveSource)
			&& IsYiDuiYingMirror(recursiveSource))
		{
			int invalidActorId = SafeActorId(actor);
			ForgetYiDuiYingMirror(invalidActorId);
			if (XjActorAccessor.TryGetInt(recursiveSource, XjActorDataKeys.XjYiDuiYingMirrorActorId, out int linkedId)
				&& linkedId == invalidActorId)
			{
				XjActorAccessor.SetInt(recursiveSource, XjActorDataKeys.XjYiDuiYingMirrorActorId, 0);
			}
		...
		XjActorAccessor.SetFloat(actor, XjActorDataKeys.XjYiDuiYingCooldownEndTime, 0f);
		XjXianJiAccessor.RepairForbiddenYiDuiYingForMirror(actor, XjYearTracker.CurrentYear);
	...
	internal static void TickActor(Actor actor, in XjActorCultivationSnapshot snapshot, int currentYear)
	{
		...
		if (IsYiDuiYingMirror(actor))
		{
			...
			XjActorAccessor.SetInt(actor, XjActorDataKeys.XjYiDuiYingMirrorActorId, 0);
			XjActorAccessor.SetFloat(actor, XjActorDataKeys.XjYiDuiYingCooldownEndTime, 0f);
			XjXianJiAccessor.RepairForbiddenYiDuiYingForMirror(actor, currentYear);
		}

		if (IsYuYiXian(actor))
		{
			EnsureTrait(actor, YuYiXianTraitId);
			XjYuYiXianRegistry.ReconcileLiveActor(actor);
			XjRealmTitleApplyService.EnsureYuYiTitle(actor);
		}
		else if (IsJieLinXian(actor))
		{
			EnsureTrait(actor, JieLinXianTraitId);
		...
		XjSpecialXianPositionPromotionSystem.TickAnnual(actor, currentYear);
		TryCreateYiDuiYingMirror(actor, snapshot, xianJiState);
		// 执孛「形渡阡」是独立幻身，不是一个视觉占位。它只在本体年度流水线
		// 中维护一名真实 Actor，幻身自身不再进入修炼年度流水线。
		XjXingDuQianPhantomSystem.TickSource(actor, snapshot, xianJiState, currentYear);
		...
		int currentYear)
	{
		...
		XjJinDanAccessor.WriteSpecialXianPayload(actor, jinXing, safeYear);
		...
		if (!XjJieLinXianRegistry.TryRegister(actor, safeYear))
		{
			...
			XjJinDanAccessor.ClearSpecialXianPayload(actor);
			XjActorAccessor.SetInt(actor, XjActorDataKeys.XjJieLinXian, 0);
		...
		XjBroadcastSystem.BroadcastBLevelActorEvent(
			...
			iconId: XjEventIconCatalog.Jielin,
	...
	/// <summary>
	/// 服气太阴求证失败时进入与紫府金丹完全相同的结璘容量表。
	/// 修法只决定此前如何温养性命，不额外增加六个结璘名额。
	/// </summary>
	internal static bool TryResolveJieLinXianOnFuQiFailure(Actor actor, int currentYear)
	{
		if (actor?.data == null
			|| !XjCultivationPathRules.IsFuQiYangXing(actor)
			|| !XjActorAccessor.TryGetString(actor, XjActorDataKeys.DaoTu, out string daoTu)
			|| !string.Equals(daoTu, TaiYinDaoTu, StringComparison.Ordinal)
			|| !XjGuoWeiRegistry.HasManifestedZhengWeiDaoTu(TaiYinDaoTu)
			|| !XjJieLinXianRegistry.HasCapacityFor(actor))
		{
			return false;
		...
		string jinXing = XjJinXingCalculator.Calculate(TaiYinDaoTu, actorId);
		...
		{
			XjActorAccessor.SetInt(actor, XjActorDataKeys.XjJinDanYiXiang,
				XjDeterministicHash.PositiveIndex(actorId + safeYear, "fuqi_jielinxian_yixiang", 300) + 1);
		}

		if (!XjJieLinXianRegistry.TryRegister(actor, safeYear))
		{
			ClearFuQiJieLinAttempt(actor);
		...
		XjActorAccessor.SetInt(actor, XjActorDataKeys.FuQiJinDanSuccessYear, safeYear);
		XjActorAccessor.SetInt(actor, XjActorDataKeys.FuQiJinDanNextAttemptYear, 0);
		XjYinSiTraitLifecycle.EnsureRemovedFromJinDan(actor);
		EnsureTrait(actor, JieLinXianTraitId);
		XjJinDanImmortalityRegistry.EnsureActivated(actor, safeYear);
		XjRealmPromotionHelper.ApplyCommonPostRealmWrite(actor, XjRealmIds.ZhenJunYuShi, safeYear);
		XjRealmTitleApplyService.ApplyOnJieLinPromotion(actor);
		...
		XjBroadcastSystem.BroadcastBLevelActorEvent(
			...
			iconId: XjEventIconCatalog.Jielin,
			category: XjAnnouncementCategory.HighRealm);
		XjAutoCollectSystem.TryCollectRealm(actor, XjRealmIds.ZhenJunYuShi, "FuQiJieLinPromotion");
		XjFaBaoAcquisition.TryGrantOnJieLinSuccess(actor, safeYear);
		return true;
	}


	internal static bool TryResolveYuYiXianOnJinDanFailure(
		Actor actor,
		in XjActorCultivationSnapshot snapshot,
		in XjXianJiState xianJiState,
		int currentYear)
	{
		if (actor?.data == null
			|| !string.Equals(snapshot.DaoTu, TaiYangDaoTu, StringComparison.Ordinal)
			|| !HasOnlyYuYiCompatibleXianJi(xianJiState)
			|| !XjGuoWeiRegistry.HasManifestedZhengWeiDaoTu(TaiYangDaoTu)
			|| !XjYuYiXianRegistry.HasCapacityFor(actor))
		{
			return false;
		}

		string jinXing = XjJinXingCalculator.Calculate(TaiYangDaoTu, GetActorId(actor));
		if (!XjCultivationStateTransitions.TrySetDaoTu(actor, TaiYangDaoTu, false)
			|| !XjCultivationStateTransitions.TrySetRealm(actor, XjRealmIds.JinDan, false))
		{
			return false;
		}
		...
		XjJinDanAccessor.WriteSpecialXianPayload(actor, jinXing, safeYear);
		XjActorAccessor.SetInt(actor, XjActorDataKeys.XjYuYiXian, 1);
		XjActorAccessor.SetInt(actor, XjActorDataKeys.XjYuYiXianYear, safeYear);
		XjActorAccessor.SetString(actor, XjActorDataKeys.XjNameRealmDisplay, "郁仪仙");
		if (!XjActorAccessor.TryGetInt(actor, XjActorDataKeys.XjJinDanYiXiang, out int yiXiang) || yiXiang <= 0)
		{
			XjActorAccessor.SetInt(actor, XjActorDataKeys.XjJinDanYiXiang,
				XjDeterministicHash.PositiveIndex(GetActorId(actor) + safeYear, "yuyixian_yixiang", 300) + 1);
		}

		if (!XjYuYiXianRegistry.TryRegister(actor, safeYear))
		{
			ClearYuYiAttempt(actor);
			XjCultivationStateTransitions.TrySetRealm(actor, XjRealmIds.ZiFu, true);
			return false;
		}
		...
		EnsureTrait(actor, YuYiXianTraitId);
		XjRealmPromotionHelper.ApplyCommonPostRealmWrite(actor, XjRealmIds.JinDan, safeYear);
		XjRealmTitleApplyService.ApplyOnYuYiPromotion(actor);
		int activeCount = XjYuYiXianRegistry.ActiveCount;
		XjChronicleWriter.RecordYuYiSucceeded(actor, safeYear, activeCount);
		XjThreeBookWriter.RecordYuYiSucceeded(actor, safeYear, activeCount);
		XjBroadcastSystem.BroadcastBLevelActorEvent(
			actor,
			XjAnnouncementText.BuildYuYiPromotion(actor, activeCount),
			iconId: XjEventIconCatalog.YuYi,
			category: XjAnnouncementCategory.HighRealm);
		XjFaBaoAcquisition.TryGrantOnJieLinSuccess(actor, safeYear);
		return true;
	}

	private static void ClearYuYiAttempt(Actor actor)
	{
		XjJinDanAccessor.ClearSpecialXianPayload(actor);
		XjActorAccessor.SetInt(actor, XjActorDataKeys.XjYuYiXian, 0);
	...
	private static void ClearFuQiJieLinAttempt(Actor actor)
	{
		XjJinDanAccessor.ClearSpecialXianPayload(actor);
		XjActorAccessor.SetInt(actor, XjActorDataKeys.XjJieLinXian, 0);
		XjActorAccessor.SetInt(actor, XjActorDataKeys.XjJieLinXianYear, 0);
	...
	private static bool HasOnlyYuYiCompatibleXianJi(in XjXianJiState state)
	{
		if (!state.Found || state.Count != XjXianJiState.MaxCount || state.Ids == null) return false;
		for (int i = 0; i < state.Ids.Length && i < XjXianJiState.MaxCount; i++)
		{
			if (XjXianJiCatalog.GetPoolKind(TaiYangDaoTu, state.Ids[i]) == XjXianJiPoolKind.Other) return false;
		}
		return true;
	}
	...
	private static void EnsureUnlockedTraitMarkers(Actor actor, in XjActorCultivationSnapshot snapshot, in XjXianJiState xianJiState)
	{
		...
		// 0.9.6.11 曾把神通名误注册为特殊身份特质。按角色真实身份迁移，
		// 旧标记永久隐藏且不受移除保护。
		if (actor.hasTrait(LegacyJieLinZhangTraitId)) actor.removeTrait(LegacyJieLinZhangTraitId);
		if (actor.hasTrait(LegacyYuYiWenTraitId)) actor.removeTrait(LegacyYuYiWenTraitId);
		if (IsJieLinXian(actor)) EnsureTrait(actor, JieLinXianTraitId);
		else RemoveSpecialIdentityTrait(actor, JieLinXianTraitId);
		if (IsYuYiXian(actor)) EnsureTrait(actor, YuYiXianTraitId);
		else RemoveSpecialIdentityTrait(actor, YuYiXianTraitId);
	...
	private static void TryCastYeTianMen(Actor actor, in XjActorCultivationSnapshot snapshot, in XjXianJiState xianJiState, int currentYear)
	{
		...
		if (!XjJinDanDaoSpellCooldown.IsReady(actorId, "Special:YeTianMen", combatNow))
		{
			return;
		}

		if (XjJinDanCombatApi.TryApplyTerrainEffect(actor, tile, "SmashTerrain", 3, 0.8f, "YeTianMen", out _))
		{
			XjActorAccessor.SetFloat(actor, XjActorDataKeys.XjYeTianMenLastYear, Time.unscaledTime);
			XjJinDanDaoSpellCooldown.MarkUsed(actorId, "Special:YeTianMen", combatNow, YeTianMenCooldownSeconds);
		}
	}
	...
	private static void EnsureYiDuiYingMirrorName(Actor mirror, Actor knownSource = null, bool force = false)
	{
		...
		XjActorStateWriteGateway.SetExternalString(
			mirror,
			ChineseFamilyNameKey,
			surname,
			XjActorStateDomain.Family | XjActorStateDomain.Identity);
		XjActorStateWriteGateway.SetDisplayName(mirror, baseName, customName: true);
	...
	private static void ObserveYiDuiYingMirror(Actor actor)
	{
		if (!IsYiDuiYingMirror(actor)) return;
		int actorId = SafeActorId(actor);
		if (actorId > 0)
		{
			YiDuiYingMirrorActorIds.Add(actorId);
		}
	}

	private static void ForgetYiDuiYingMirror(int actorId)
	{
		if (actorId > 0)
		{
			YiDuiYingMirrorActorIds.Remove(actorId);
		}
	}

	internal static void ClearRuntime()
	{
		YiDuiYingMirrorActorIds.Clear();
		YiDuiYingMirrorActorIdScratch.Clear();
	}
	...
	private static void EnsureYiDuiYingAptitudeFloor(Actor mirror)
	{
		...
		if (!XjCultivationPathTransitions.TryEnsureZiFuJinDan(mirror, syncVisibleTraits: false))
		{
			return;
		}
	...
	private static void SanitizeYiDuiYingMirror(Actor mirror)
	{
		...
		XjVisibleTraitSync.ClearIndependentCloneCultivationState(mirror);
		...
		XjActorGongFaCollection.Clear(mirror);
		...
		XjXianJiAccessor.RestoreSnapshot(mirror, string.Empty, 0);
		...
		XjActorAccessor.SetInt(mirror, XjActorDataKeys.XjQiuJinIntentState, 0);
		XjActorAccessor.SetInt(mirror, XjActorDataKeys.XjQiuJinIntentYear, 0);
		XjActorAccessor.SetString(mirror, XjActorDataKeys.XjQiuJinIntentReason, string.Empty);
		XjActorAccessor.SetInt(mirror, XjActorDataKeys.XjQiuJinIntentSignature, 0);
		...
		XjActorAccessor.SetInt(mirror, XjActorDataKeys.XjJinDanPromotionAnnouncementYear, 0);
		...
		XuanJianVNext.Systems.Sect.XjSectProjection.ClearActor(mirror);
		XjActorAccessor.SetString(mirror, XjActorDataKeys.XjNameTitle, string.Empty);
		XjActorAccessor.SetString(mirror, XjActorDataKeys.XjNameRealmDisplay, string.Empty);
		XjActorAccessor.SetInt(mirror, XjActorDataKeys.XjYiDuiYingMirrorActorId, 0);
		XjActorAccessor.SetFloat(mirror, XjActorDataKeys.XjYiDuiYingCooldownEndTime, 0f);
		XjActorAccessor.SetFloat(mirror, XjActorDataKeys.XjJiShengYuBuffUntilTime, 0f);
		XjActorAccessor.SetFloat(mirror, XjActorDataKeys.XjJiShengYuCooldownUntilTime, 0f);
		...
		RemoveSpecialIdentityTrait(mirror, JieLinXianTraitId);
		RemoveSpecialIdentityTrait(mirror, YuYiXianTraitId);
		if (mirror.hasTrait(LegacyJieLinZhangTraitId)) mirror.removeTrait(LegacyJieLinZhangTraitId);
		if (mirror.hasTrait(LegacyYuYiWenTraitId)) mirror.removeTrait(LegacyYuYiWenTraitId);
	...
	private static void ResetYiDuiYingMirrorAge(Actor mirror)
	{
		...
		catch (System.Exception xjCaught1196) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/HighRealm/XjXuanJianShenTongSpecials.cs:1196", xjCaught1196); }
		...
		catch (System.Exception xjCaught1215) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/HighRealm/XjXuanJianShenTongSpecials.cs:1215", xjCaught1215); }
	...
	private static void ClearSavedCultivationTraits(List<string> savedTraits)
	{
		...
		savedTraits.RemoveAll(traitId =>
		{
			...
			return traitId.StartsWith("XjRealm", StringComparison.Ordinal)
				...
				|| string.Equals(traitId, JieLinXianTraitId, StringComparison.Ordinal)
				|| string.Equals(traitId, YuYiXianTraitId, StringComparison.Ordinal)
				|| string.Equals(traitId, LegacyJieLinZhangTraitId, StringComparison.Ordinal)
				|| string.Equals(traitId, LegacyYuYiWenTraitId, StringComparison.Ordinal)
