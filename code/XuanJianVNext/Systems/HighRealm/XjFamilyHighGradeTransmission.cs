...
internal static class XjFamilyHighGradeTransmission
{
		...
		out string gongFaName)
	{
		...
		for (int i = 0; i < entries.Count; i++)
		{
			...
			// 直接优先使用自身道途上位神通功法，其次下位、相邻，最后才是其他。
		...
		in XjGongFaState current)
	{
		...
		string daoTu = ResolveDaoTu(current.DaoTu, snapshot.DaoTu);
		XjFamilyGongFaWarehouseEntry selected = SelectGrade6GongFa(entries, daoTu);
		if (!selected.Found)
		{
			return false;
		}
	...
	private static string FirstMappedXianJi(in XjXianJiState xianJi)
	{
		for (int i = 0; xianJi.Ids != null && i < xianJi.Ids.Length; i++)
		{
			if (!string.IsNullOrWhiteSpace(xianJi.Ids[i]))
			{
				return xianJi.Ids[i].Trim();
			}
		}
	...
	private static string GenerateFamilyMappedGrade5Name(string daoTu, long actorId, string mappedXianJi, int slot, HashSet<string> usedNames)
	{
		for (int attempt = 0; attempt < 16; attempt++)
		{
			long seed = actorId
				...
				+ (slot * 997L)
				+ (attempt * 131L)
				+ XjDeterministicHash.StableHash(mappedXianJi + "|family_jindan_gongfa");
			string name = XjGongFaNameLibrary.GenerateName(daoTu, 5, seed);
			if (!string.IsNullOrWhiteSpace(name)
				&& (usedNames == null || !usedNames.Contains(name.Trim())))
			{
				return name.Trim();
			}
		}
