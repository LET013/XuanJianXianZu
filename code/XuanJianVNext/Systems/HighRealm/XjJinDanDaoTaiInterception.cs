...
internal static class XjJinDanDaoTaiInterception
{
	...
	internal static bool TryResolve(Actor actor, string daoTu, int currentYear)
	{
		...
		// 道胎之姿通过全部修炼概率判定；道胎截杀同样属于求金后的随机判定。
		...
		bool died = XjVanillaDeathGuard.TryExecuteForceDeath(actor, (AttackType)5, true, XuanJianVNext.Systems.Death.XjDeathCause.BreakthroughFailure);
		if (!died)
		{
