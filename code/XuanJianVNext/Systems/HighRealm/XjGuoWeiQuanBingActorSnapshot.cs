...
internal static class XjGuoWeiQuanBingActorSnapshot
{
	...
	internal static bool TryReadActive(Actor actor, out XjGuoWeiQuanBingState state)
	{
		...
		// 0.9.9.22 后置道途将“神通名换皮权柄”迁为独立权柄名；角色镜像同样在读档边界迁移。
		local = XjGuoWeiAuthorityCatalog.NormalizeAuthoritySet(local, daoTu);
		seized = XjGuoWeiAuthorityCatalog.NormalizeAuthoritySet(seized);
		foreign = XjGuoWeiAuthorityCatalog.NormalizeAuthoritySet(foreign);
		withdrawn = XjGuoWeiAuthorityCatalog.NormalizeAuthoritySet(withdrawn);
		seizedSources = XjGuoWeiAuthorityCatalog.NormalizeAuthoritySourceMap(seizedSources);
