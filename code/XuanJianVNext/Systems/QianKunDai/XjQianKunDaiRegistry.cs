...
internal static class XjQianKunDaiRegistry
{
	...
	internal static int Count => entriesByActorId.Count;
	...
	internal static void AddOrUpdate(XjQianKunDaiState state)
	{
		...
		XjActorStateRevisionStore.Mark(actorId, XjActorStateDomain.Inventory);
	...
	/// <summary>
	/// 第三方换身迁移同一角色的乾坤袋当前库存。新 actorId 不应得到一份复制品，
	/// 也不能让旧 actorId 继续占据持久仓储记录。
	/// </summary>
	internal static bool RebindActorAfterExternalReplacement(long oldActorId, Actor target)
	{
		...
		if (newActorId <= 0L || newActorId == oldActorId || !entriesByActorId.TryGetValue(oldActorId, out XjQianKunDaiState state))
			return false;

		entriesByActorId.Remove(oldActorId);
		string actorName = state.ActorName;
		try
		{
			string liveName = target.getName();
			if (!string.IsNullOrWhiteSpace(liveName)) actorName = liveName;
		}
		catch { }
		entriesByActorId[newActorId] = new XjQianKunDaiState(
			true, newActorId, actorName, state.CaiQiSummary, state.FaBaoSummary,
			state.ResourceSummary, state.UpdatedYear, state.Summary, state.Capacity, state.Items);
		XjActorStateRevisionStore.Mark(oldActorId, XjActorStateDomain.Inventory);
		XjActorStateRevisionStore.Mark(newActorId, XjActorStateDomain.Inventory);
		XjWorldArchiveSystem.MarkChanged();
		return true;
	}
	...
	internal static string GetSummary(long actorId)
	{
		...
		string year = state.UpdatedYear > 0 ? " - " + XjChronology.FormatYear(state.UpdatedYear) : string.Empty;
