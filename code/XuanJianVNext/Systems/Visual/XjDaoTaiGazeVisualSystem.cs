...
internal static class XjDaoTaiGazeVisualSystem
{
	...
	internal static bool Show(long actorId, int currentYear)
	{
		...
		{
			try { world.locatePosition(focusPosition); } catch (System.Exception xjCaught41) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/Visual/XjDaoTaiGazeVisualSystem.cs:41", xjCaught41); }
		}
	...
	private static Vector3 ResolveScale(Sprite sprite)
	{
		...
		try { spriteWidth = Math.Max(1f, sprite.bounds.size.x); } catch (System.Exception xjCaught89) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/Visual/XjDaoTaiGazeVisualSystem.cs:89", xjCaught89); }
