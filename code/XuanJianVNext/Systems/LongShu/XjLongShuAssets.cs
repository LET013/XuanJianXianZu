...
internal static partial class XjLongShuSystem
{
	...
	private const string ActorIconId = "iconLongShu";
	...
	private static MethodInfo _loadTexturesAndSpritesMethod;
	private static FieldInfo _cachedSpriteField;
	private static FieldInfo _actorTileTargetField;
	private static PropertyInfo _actorTileTargetProperty;
	private static FieldInfo _actorSubspeciesField;
	private static PropertyInfo _actorSubspeciesProperty;
	...
	private static void TryRegisterActorAsset()
	{
		...
		try
		{
			...
			asset.base_stats["health"] = 900f;
			asset.base_stats["damage"] = 95f;
			asset.base_stats["armor"] = 18f;
			asset.base_stats["speed"] = 92f;
			asset.base_stats["attack_speed"] = 72f;
	...
	private static void SetVector2Member(object target, string name, Vector2 value)
	{
		...
		{
			// Shadow members differ across WorldBox builds; absence is harmless.
		}
	}
	...
	private static void TryReloadActorTextures(ActorAssetLibrary library, ActorAsset asset)
	{
		try
		{
			...
			_cachedSpriteField ??= typeof(ActorAsset).GetField("_cached_sprite", BindingFlags.Instance | BindingFlags.NonPublic);
			_cachedSpriteField?.SetValue(asset, null);
			asset._cached_sprite = null;
			asset.cached_sprite = null;

			if (library != null)
			{
				_loadTexturesAndSpritesMethod ??= typeof(ActorAssetLibrary).GetMethod(
					"loadTexturesAndSprites",
					BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
				_loadTexturesAndSpritesMethod?.Invoke(library, new object[] { asset });
			}
	...
	private static WorldTile TryGetTileTarget(Actor actor)
	{
		...
		try
		{
			...
			_actorTileTargetField ??= type.GetField("tileTarget", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
			if (_actorTileTargetField != null)
			{
				return _actorTileTargetField.GetValue(actor) as WorldTile;
			}

			_actorTileTargetProperty ??= type.GetProperty("tileTarget", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
			return _actorTileTargetProperty?.GetValue(actor, null) as WorldTile;
		}
		catch
		{
			return null;
	...
	internal static void NormalizeSubspecies(Subspecies subspecies)
	{
		...
		if (subspecies.data != null && !string.Equals(subspecies.data.name, "龙属", StringComparison.Ordinal))
		{
			...
			try { subspecies.data.custom_name = true; } catch { }
			changed = true;
		}
	...
	private static void AssignActorSubspecies(Actor actor, Subspecies subspecies)
	{
		...
		{
			Type type = actor.GetType();
			_actorSubspeciesField ??= type.GetField("subspecies", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
			if (_actorSubspeciesField != null && _actorSubspeciesField.FieldType.IsInstanceOfType(subspecies))
			{
				_actorSubspeciesField.SetValue(actor, subspecies);
				return;
			}

			_actorSubspeciesProperty ??= type.GetProperty("subspecies", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
			if (_actorSubspeciesProperty != null
				&& _actorSubspeciesProperty.CanWrite
				&& _actorSubspeciesProperty.PropertyType.IsInstanceOfType(subspecies))
			{
				_actorSubspeciesProperty.SetValue(actor, subspecies, null);
			}
		}
		catch
		{
