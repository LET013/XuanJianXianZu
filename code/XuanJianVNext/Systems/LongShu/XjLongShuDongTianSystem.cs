...
internal static class XjLongShuDongTianSystem
{
	...
	internal static bool TryResolveRetreatBuilding(Actor actor, out Building building)
	{
		...
		// 旧存档/首位原持有者已死亡时，建筑集合扫描必须取得统一年度
		// 门禁。多个龙属金丹同时恢复时，只允许其中一次全表扫描。
		int currentYear = ResolveWorldYear();
		bool mayRecover = currentYear <= 0
			|| XjDetectionGate.TryBeginAnnualJob(XjDetectionJob.LongShuBuildingRecovery, currentYear);
		if (mayRecover)
		{
			if (TryFindExistingSharedBuilding(out Building recovered))
			{
	...
	private static bool TryFindExistingSharedBuilding(out Building building)
	{
		building = null;
	...
	private static int ResolveWorldYear()
	{
		try
		{
			return World.world == null
				? 0
				: (int)Math.Floor(Math.Max(0.0, World.world.getCurWorldTime() / 60.0)) + 1;
		}
		catch
		{
			return 0;
