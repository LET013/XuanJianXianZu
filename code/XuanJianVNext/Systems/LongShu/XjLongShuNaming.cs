...
internal static partial class XjLongShuSystem
{
	...
	internal static bool TryApplyRealmTitle(Actor actor, string realmId, string daoTu)
	{
		...
		XjActorStateWriteGateway.SetExternalString(actor, ChineseFamilyNameKey, LongShuSurname, XjActorStateDomain.Identity);
		...
		XjActorStateWriteGateway.SetDisplayName(actor, title + "·" + baseName + "-" + realmDisplay, customName: true);
		return true;
	}
	...
	private static string ResolveRealmDisplay(string realmId)
	{
		...
		if (string.Equals(normalized, XjRealmIds.DaoTai, StringComparison.Ordinal)
			|| string.Equals(normalized, XjRealmIds.FuQiDaoTai, StringComparison.Ordinal)) return "道胎";
