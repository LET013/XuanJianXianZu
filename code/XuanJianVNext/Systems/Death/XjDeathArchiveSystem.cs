...
internal static class XjDeathPatchBridge
{
	...
	internal static bool CommitAfterDeath(Actor actor, in XjDeathSnapshot snapshot, XjDeathCause cause)
	{
		...
		// 道统积怨必须在角色修炼身份仍可读取时结算；释修轮回随后可能改写肉身状态。
		XjDoctrineConflictSystem.OnConfirmedDeath(actor, snapshot, cause);

		// 释修轮回和摩诃依附释放必须在角色数据仍可读取时完成；
		// 普通死亡归档随后仍按原有队列执行。
		XjShiDeathLifecycle.OnConfirmedDeath(actor, snapshot, cause);
...
internal static class XjDeathSnapshotBuilder
{
	internal static bool TryBuild(Actor actor, out XjDeathSnapshot snapshot)
	{
		...
		if (actorId <= 0L || XjGuZunRegistry.IsManifestationActor(actor))
		{
			return false;
		}
		...
		bool isCultivator = XjCultivatorCache.IsCultivator(actorId)
			...
			|| XjActorAccessor.TryGetInt(actor, XjActorDataKeys.XjZz, out int aptitude)
				&& aptitude > 0;
		// 普通非家族单位原本不会创建死亡快照，这是性能优化；但0.9.10.4起
		// 今释法师及以上主动击杀凡人必须有一次最小死亡归因，否则“主动击杀也算
		// 度化”会在无家族凡人身上失效。古释已经从此车道硬隔离；CombatTracker只为这一类今释普通目标保留
		// 最后一击，因此这里按O(1)记录命中时才临时放行，不恢复普通人口全量归档。
		bool preResolvedDuhuaAttribution = false;
		long lastAttackerId = 0L;
		string lastAttackerName = string.Empty;
		string lastAttackerDaoTu = string.Empty;
		int lastAttackerTier = XjRealmSuppression.TierNone;
		if (!isCultivator && familyStableId <= 0L)
		{
			preResolvedDuhuaAttribution = XjCombatTracker.TryGetValidKillerAttribution(
				actor, XjRealmSuppression.TierNone,
				out lastAttackerId, out lastAttackerName, out lastAttackerDaoTu, out lastAttackerTier);
			if (!preResolvedDuhuaAttribution) return false;
		}

		XjActorAccessor.TryGetString(actor, XjActorDataKeys.RealmId, out string realmId);
		XjActorAccessor.TryGetString(actor, XjActorDataKeys.DaoTu, out string daoTu);
		// 果位钟爱转世即便尚未重证金丹，死亡快照仍必须沿用原果位
		// 道途，避免错误载体道途被传给下一世。
		if (XjGuoWeiFavoredDaoTuLock.TryResolveLockedDaoTu(actor, out string lockedFavoredDaoTu))
		{
			daoTu = lockedFavoredDaoTu;
		}
		...
		XjJinDanState jinDan = hasCultivationPayload ? XjJinDanAccessor.BuildPositionCarrierState(actor) : default;
		...
		bool isYuYiXian = hasCultivationPayload && XjXuanJianShenTongSpecials.IsYuYiXian(actor);
		string snapshotJinXing = jinDan.Found
			? jinDan.JinXing
			: (isJieLinXian || isYuYiXian) && XjActorAccessor.TryGetString(actor, XjActorDataKeys.XjJinDanJinXing, out string specialXianJinXing)
				? specialXianJinXing
		...
		if (hasCultivationPayload && string.IsNullOrWhiteSpace(guoWeiZhongAi))
		{
			XjActorAccessor.TryGetString(actor, XjActorDataKeys.XjReincarnationGuoWeiZhongAi, out guoWeiZhongAi);
		}
		string snapshotGuoWei = jinDan.Found ? jinDan.GuoWei : shenDan.Found ? shenDan.GuoWei : string.Empty;
		if (string.IsNullOrWhiteSpace(snapshotGuoWei)
			&& !string.IsNullOrWhiteSpace(guoWeiZhongAi))
		{
			if (XjGuoWeiQuanBingRegistry.TryGetForLiveDisplay(actor, out XjGuoWeiQuanBingState favoredState)
				&& !string.IsNullOrWhiteSpace(favoredState.GuoWei))
			{
				snapshotGuoWei = favoredState.GuoWei;
			}
			else
			{
				XjJinDanState carrier = XjJinDanAccessor.BuildPositionCarrierState(actor);
				snapshotGuoWei = carrier.Found ? carrier.GuoWei : string.Empty;
			}
		}
		...
		if (!preResolvedDuhuaAttribution && (hasCultivationPayload || familyStableId > 0L))
		{
...
internal static class XjDeathArchiveWriter
{
	internal static void Write(XjDeathSnapshot snapshot)
	{
		...
		// 仙国借玄以在世国主为锚。死亡归档是权威终点，直接在这里清理法统，
		// 避免死者不再参加年度调度后留下永久 Active 的假丹记录。
		XjXianGuoSystem.OnActorDeath(snapshot);
		// 先释放死者果位，再结算同道果位承继/外道夺权，避免胜者因
		// 死者仍占用果位而无法完成后续果位迁移。
		...
		XjYuYiXianRegistry.Release(snapshot.ActorId);
