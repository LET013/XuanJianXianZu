...
internal static class XjSecretRealmConstructionSystem
{
	...
	internal static void TickYear(int currentYear)
	{
		...
		IReadOnlyList<XjSecretRealmArchiveRecord> realms = XjSecretRealmRegistry.ReadAll();
		for (int i = 0; i < realms.Count; i++)
		{
			XjSecretRealmArchiveRecord realm = realms[i];
			if (realm == null || realm.SectId <= 0L) continue;
			if (realm.SittingJinDanActorId <= 0L || !XjScheduler.ResolveActor(realm.SittingJinDanActorId, out Actor sitting) || sitting?.data == null || !sitting.isAlive())
			{
	...
	private static void TryAssignAvailableJinDan(long sectId, int currentYear)
	{
		...
		Dictionary<long, List<long>> actorIdsByCityId = new Dictionary<long, List<long>>();
		foreach (KeyValuePair<City, List<long>> pair in index)
		{
			if (pair.Key?.data == null || pair.Value == null) continue;
			actorIdsByCityId[pair.Key.data.id] = pair.Value;
		}
		long bestId = 0L;
		for (int c = 0; c < sect.CityIds.Count; c++)
		{
			...
			for (int i = 0; i < actorIds.Count; i++)
			{
				...
				if (XjRealmHelper.GetOrder(realmId) >= XjRealmHelper.GetOrder(XjRealmIds.JinDan)) bestId = actorId;
			}
		}
