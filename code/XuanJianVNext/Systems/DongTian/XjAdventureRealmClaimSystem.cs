...
internal static class XjAdventureRealmClaimSystem
{
	...
	private static IReadOnlyList<XjAdventureRealmClaimArchiveRecord> CachedReadAll = Array.Empty<XjAdventureRealmClaimArchiveRecord>();
	...
	internal static bool RemoveForTimelineMigration(string recordId)
	{
		string id = (recordId ?? string.Empty).Trim();
		if (id.Length == 0 || !ByRecordId.Remove(id)) return false;
	...
	internal static void OnRealmCreated(in XjDongTianRecord realm)
	{
		...
		// 水月照真是道尊私域：地标可以落在某城附近，但国朝/宗门永远不能据此取得属地、
		// 守卫、争夺或远征权限。旧记录若存在也直接迁移清除。
		if (IsYuanZhaoPrivateRealmId(realm.RecordId))
		{
			RemoveForTimelineMigration(realm.RecordId);
			return;
		}
	...
	{
		if (IsYuanZhaoPrivateRealmId(recordId)) return;
	...
	internal static bool TryGetClaimRecord(string recordId, out XjAdventureRealmClaimArchiveRecord claim)
	{
		claim = null;
		string id = (recordId ?? string.Empty).Trim();
		if (id.Length == 0 || !ByRecordId.TryGetValue(id, out XjAdventureRealmClaimArchiveRecord record) || record == null) return false;
		claim = Clone(record);
		return true;
	}
	...
	internal static IReadOnlyList<XjAdventureRealmClaimArchiveRecord> ReadAll()
	{
		...
		{
			CachedReadAll = Array.Empty<XjAdventureRealmClaimArchiveRecord>();
			_cachedReadRevision = _readRevision;
			return CachedReadAll;
		}

		XjAdventureRealmClaimArchiveRecord[] result = new XjAdventureRealmClaimArchiveRecord[ByRecordId.Count];
		int index = 0;
		...
		Array.Sort(result, (a, b) => string.Compare(a.RecordId, b.RecordId, StringComparison.Ordinal));
		CachedReadAll = result.Length == 0 ? Array.Empty<XjAdventureRealmClaimArchiveRecord>() : Array.AsReadOnly(result);
		_cachedReadRevision = _readRevision;
		return CachedReadAll;
	}

	/// <summary>
	/// Allocation-free projection for annual sect inheritance checks. The caller
	/// owns the target set and can snapshot claim ownership once per maintenance
	/// cycle instead of cloning/sorting every claim once for every sect.
	/// </summary>
	internal static void FillActiveClaimSectIds(HashSet<long> target)
	{
		if (target == null) return;
		target.Clear();
		foreach (XjAdventureRealmClaimArchiveRecord record in ByRecordId.Values)
		{
			if (record == null || record.ClaimSectId <= 0L
				|| string.Equals(record.State, XjAdventureRealmClaimState.Closed, StringComparison.Ordinal)) continue;
			target.Add(record.ClaimSectId);
		}
	}
	...
	internal static void ImportArchiveRecords(IReadOnlyList<XjAdventureRealmClaimArchiveRecord> source)
	{
		...
		if (source != null) for (int i = 0; i < source.Count; i++)
		{
			...
			if (record == null || string.IsNullOrWhiteSpace(record.RecordId) || IsYuanZhaoPrivateRealmId(record.RecordId)) continue;
	...
	{
		ByRecordId.Clear();
		PendingClaimRecordIds.Clear();
		ActiveContestRecordIds.Clear();
		CachedReadAll = Array.Empty<XjAdventureRealmClaimArchiveRecord>();
		BumpReadRevision();
		_cachedReadRevision = 0;
	}

	private static bool IsYuanZhaoPrivateRealmId(string recordId)
	{
		return string.Equals((recordId ?? string.Empty).Trim(),
			"qiyu_dongtian_" + XjDongTianRules.YuanZhaoDongTianId, StringComparison.Ordinal);
	}
	...
	private static int ScoreSectRepresentatives(long sectId)
	{
		...
		return ScoreActors(XjSectAuthorityStore.GetActorIdsForSect(sectId));
	}
	...
	private static void MarkChanged()
	{
		...
		XjCodexSnapshotPublisher.MarkDirty(XjCodexDirtyFlags.AdventureRealm | XjCodexDirtyFlags.History);
	}
