...
{
	private const int TriggerChancePercent = 30;
	private const int PawnCooldownYears = 30;
	private const int MaxPawnCandidateChecks = 256;
	...
	internal static bool ShouldCheck(long familyId, in XjFamilyLedgerAggregate aggregate, int currentYear)
	{
		...
		if (!XjDetectionGate.IsEntityMaintenanceSlot(
			XjEntityDetectionJob.HighRealmChess,
			familyId,
			currentYear))
		{
			return false;
	...
	{
		XjActorAccessor.TryGetFloat(pawn, XjActorDataKeys.HuiGuang, out float huiGuang);
		XjActorAccessor.SetFloat(pawn, XjActorDataKeys.HuiGuang, Math.Min(200f, Math.Max(0f, huiGuang) + 1f));
	}
	...
	private static void ApplyManipulatorInsight(Actor manipulator)
	{
		...
		XjActorAccessor.SetFloat(manipulator, XjActorDataKeys.HuiGuang, Math.Min(200f, Math.Max(0f, huiGuang) + 1f));
	}
		...
		string outcome)
	{
		...
		if (string.Equals(outcome, OutcomeEscaped, StringComparison.Ordinal))
		{
			...
			summary = pawnName + action + "，临事方察觉这一切皆是" + manipulatorName
				+ "借外族修士布下的棋局。其凭慧光窥见破绽，及时抽身，没有替旁人走完最后一步。";
		}
		else if (string.Equals(outcome, OutcomeSecret, StringComparison.Ordinal))
		{
			title = "借下修探知隐秘";
			...
			summary = pawnName + action + "。待事情收束，他才明白自己一路所见所闻都被" + manipulatorName
				+ "拿去拼出了背后的隐秘；上修慧光因此增长，他却未得半分，徒作嫁衣。";
		}
		else
		{
			title = "借下修收取气数";
