...
internal static class XjFamilyDisplayProjection
{
	...
	private static XjFamilyPendingDisplayItem ToPendingItem(in XjFamilyPendingRecord record)
	{
		...
		string actorName = string.IsNullOrWhiteSpace(record.ActorName)
			? "actor:" + record.ActorId.ToString(System.Globalization.CultureInfo.InvariantCulture)
			: record.ActorName;
		string displayText = actorName
			+ "（ID "
			+ record.ActorId.ToString(System.Globalization.CultureInfo.InvariantCulture)
			+ " - "
			+ status
			+ " - parent:"
			+ record.ParentId1.ToString(System.Globalization.CultureInfo.InvariantCulture)
			+ ","
			+ record.ParentId2.ToString(System.Globalization.CultureInfo.InvariantCulture)
			+ "）";
