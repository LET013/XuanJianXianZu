...
internal sealed class XjFamilyMemberIndex
{
	...
	private static string ResolveRealmFromVisibleTraits(Actor actor)
	{
		if (actor?.data == null)
		{
			return string.Empty;
		...
		if (actor.hasTrait("XjRealm5")) return XjRealmIds.JinDan;
		if (actor.hasTrait("XjRealm4")) return XjRealmIds.ZiFu;
		if (actor.hasTrait("XjRealm3")) return XjRealmIds.ZhuJi;
		if (actor.hasTrait("XjRealm2")) return XjRealmIds.LianQi;
		if (actor.hasTrait("XjRealm1")) return XjRealmIds.TaiXi;
		if (XjActorAccessor.TryGetString(actor, XjActorDataKeys.RealmId, out string realmId))
		{
			string normalized = (realmId ?? string.Empty).Trim();
			if (string.Equals(normalized, XjRealmIds.JinDan, StringComparison.Ordinal)
				...
				|| string.Equals(normalized, XjRealmIds.TaiXi, StringComparison.Ordinal))
			{
				return normalized;
			}
		}
	...
	private bool ShouldWaitForFather(Actor actor, long actorId)
	{
		...
		return !TryGetRecord(fatherActorId, out XjFamilyIdentity fatherIdentity) || !fatherIdentity.Found;
	}
	...
	private void MarkActorPendingByParents(Actor actor, long actorId)
	{
		...
		pendingRecordsByActorId[actorId] = new XjFamilyPendingRecord(
			true,
			actorId,
			actorName,
			parentId1,
	...
	private void RemovePendingActor(long actorId)
	{
		...
		pendingRecordsByActorId.Remove(actorId);
	...
	private static void PersistIdentityMirror(Actor actor, in XjFamilyIdentity identity)
	{
		...
		{
			BaseSystemData data = (BaseSystemData)actor.data;
			data.set(XjFamilyIdentity.FamilyStableId, identity.FamilyStableIdValue);
			data.set(XjFamilyIdentity.FamilyGeneration, Math.Max(1, identity.Generation));
		}
		catch
		{
		}
	...
	private static void ClearIdentityMirror(Actor actor)
	{
		...
		{
			BaseSystemData data = (BaseSystemData)actor.data;
			data.set(XjFamilyIdentity.FamilyStableId, 0L);
			data.set(XjFamilyIdentity.FamilyGeneration, 0);
		}
		catch
		{
		}
