...
internal static class XjFamilyGongFaWarehouse
{
		...
		string boundAuthority = "")
	{
		...
		bool isFuQiMethod = string.Equals(sourceType, SourceTypeGongFa, StringComparison.Ordinal)
			&& XjFuQiCoreCatalog.IsKnownMethodName(gongFaName);
		string normalizedBoundGongFaName = isFuQiMethod
			? string.Empty
			: string.IsNullOrWhiteSpace(boundGongFaName) ? string.Empty : boundGongFaName.Trim();
		string normalizedMappedXianJi = isFuQiMethod
			? string.Empty
			: string.IsNullOrWhiteSpace(mappedXianJi)
				...
				: mappedXianJi.Trim();
		string normalizedBoundAuthority = isFuQiMethod
			? string.Empty
			: string.IsNullOrWhiteSpace(boundAuthority)
	...
	private static bool AreEquivalent(in XjFamilyGongFaWarehouseEntry left, in XjFamilyGongFaWarehouseEntry right)
	{
		...
		if (XjFuQiCoreCatalog.IsKnownMethodName(left.GongFaName)
			&& XjFuQiCoreCatalog.IsKnownMethodName(right.GongFaName))
		{
			return true;
		}
	...
	private static string BuildStableEntryKey(in XjFamilyGongFaWarehouseEntry entry)
	{
		...
		{
			identity = XjFuQiCoreCatalog.IsKnownMethodName(entry.GongFaName)
				? "FuQiMethod"
				: !string.IsNullOrWhiteSpace(entry.MappedXianJi) ? entry.MappedXianJi.Trim() : NormalizeEntryName(entry.GongFaName);
		}
	...
	private static bool ShouldPreferEntry(in XjFamilyGongFaWarehouseEntry candidate, in XjFamilyGongFaWarehouseEntry existing)
	{
		...
		if (XjFuQiCoreCatalog.IsKnownMethodName(candidate.GongFaName)
			&& XjFuQiCoreCatalog.IsKnownMethodName(existing.GongFaName)
			&& XjFuQiCoreCatalog.TryResolveByDaoTu(candidate.DaoTu, out XjFuQiCoreDefinition currentFuQi))
		{
			bool candidateCurrent = string.Equals(candidate.GongFaName, currentFuQi.MethodName, StringComparison.Ordinal);
			bool existingCurrent = string.Equals(existing.GongFaName, currentFuQi.MethodName, StringComparison.Ordinal);
...
internal static class XjFamilyGongFaWarehouseUI
{
	...
	private static int FindEquivalentGongFaDisplayItem(List<XjFamilyGongFaWarehouseUIItem> items, in XjFamilyGongFaWarehouseUIItem target)
	{
		...
		for (int i = 0; i < items.Count; i++)
		{
			...
			bool sameFuQiMethod = item.Grade == target.Grade
				&& string.Equals(item.DaoTu ?? string.Empty, target.DaoTu ?? string.Empty, System.StringComparison.Ordinal)
				&& XjFuQiCoreCatalog.IsKnownMethodName(item.Name)
				&& XjFuQiCoreCatalog.IsKnownMethodName(target.Name);
			bool sameMappedXianJi = string.Equals(
				item.MappedXianJi ?? string.Empty,
				target.MappedXianJi ?? string.Empty,
				System.StringComparison.Ordinal);
			if (sameFuQiMethod || (sameName && item.Grade == target.Grade && sameMappedXianJi))
			{
				return i;
			}
		}
	...
	private static bool ShouldPreferDisplayItem(in XjFamilyGongFaWarehouseUIItem candidate, in XjFamilyGongFaWarehouseUIItem existing)
	{
		...
		if (XjFuQiCoreCatalog.IsKnownMethodName(candidate.Name)
			...
			&& XjFuQiCoreCatalog.TryResolveByDaoTu(candidate.DaoTu, out XjFuQiCoreDefinition currentFuQi))
		{
			bool candidateCurrent = string.Equals(candidate.Name, currentFuQi.MethodName, System.StringComparison.Ordinal);
			bool existingCurrent = string.Equals(existing.Name, currentFuQi.MethodName, System.StringComparison.Ordinal);
			if (candidateCurrent != existingCurrent) return candidateCurrent;
		}
