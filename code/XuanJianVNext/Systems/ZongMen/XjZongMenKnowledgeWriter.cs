...
internal static class XjZongMenKnowledgeWriter
{
	...
	private static void HandleCaiQi(in XjFamilyDomainEvent domainEvent)
	{
		...
		if (!XjZongMenCaiQiWarehouse.TryAddCaiQiResource(
	...
	private static void HandleCaiQiFa(in XjFamilyDomainEvent domainEvent)
	{
		...
		if (!XjZongMenCaiQiWarehouse.TryAddCaiQiFa(
			domainEvent.ZongMenId,
			domainEvent.ZongMenName,
	...
	private static void HandleGongFa(in XjFamilyDomainEvent domainEvent)
	{
		...
		if (!XjZongMenGongFaPavilion.TryAddGongFa(
			domainEvent.ZongMenId,
			domainEvent.ZongMenName,
			domainEvent.ActorId,
			domainEvent.ActorName,
	...
	private static void HandleQiuJinFa(in XjFamilyDomainEvent domainEvent)
	{
		...
		if (!XjZongMenGongFaPavilion.TryAddQiuJinFa(
			domainEvent.ZongMenId,
			domainEvent.ZongMenName,
			domainEvent.ActorId,
			domainEvent.ActorName,
	...
	private static string BuildCaiQiHistoryText(string displayName, int amount, string zongMenName)
	{
		string qiName = NormalizeCaiQiName(displayName);
		int count = amount <= 0 ? 1 : amount;
		return count == 1
			? "一缕" + qiName + "归入" + zongMenName + "纳气仓库。"
			: count + "缕" + qiName + "归入" + zongMenName + "纳气仓库。";
	}

	private static string NormalizeCaiQiName(string displayName)
	{
		string value = string.IsNullOrWhiteSpace(displayName) ? "先天之气" : displayName.Trim();
		if (value.EndsWith("之气", System.StringComparison.Ordinal) || value.EndsWith("杂气", System.StringComparison.Ordinal))
		{
			return value;
		}
		return value + "之气";
