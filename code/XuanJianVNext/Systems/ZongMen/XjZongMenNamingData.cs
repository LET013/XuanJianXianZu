...
internal static partial class XjZongMenCityData
{
	...
	private static readonly string[] FallbackZongMenTitles = { "玄鉴", "太玄", "清微", "玄真", "道玄", "玉清" };

	#region 宗门创建

	internal static bool TryCreateZongMen(City city, int currentYear, Dictionary<City, List<long>> cityIndex = null)
	{
	...
	internal static bool TryCreateZongMenWithFounder(City city, Actor zongZhu, int currentYear, Dictionary<City, List<long>> cityIndex = null, long sectId = 0L, string nameOverride = null, bool allowExistingIdentity = false)
	{
		if (city?.data == null || zongZhu?.data == null || HasZongMen(city)) return false;
		if (XjYinSiTraitLifecycle.IsYinSi(zongZhu)) return false;
		if (!CanFoundZongMen(zongZhu, allowExistingIdentity) || zongZhu.city != city) return false;
		if (GetRealmLevel(zongZhu) < ZiFuRealmLevel) return false;

		string name = string.IsNullOrWhiteSpace(nameOverride) ? GenerateZongMenName(city, zongZhu) : nameOverride.Trim();
		...
		city.data.set(KeySchemaVersion, CurrentSchemaVersion);
		city.data.set(KeyLegacyGeneration, 1);
		city.data.set(KeyZongZhuGeneration, 1);
		city.data.set(KeyZongMenName, name);
		long stableSectId = sectId > 0L ? sectId : GetCityId(city);
		city.data.set(KeySectIdMirror, stableSectId > 0L ? stableSectId.ToString(System.Globalization.CultureInfo.InvariantCulture) : string.Empty);
		city.data.set(KeyCreationYear, year);
		city.data.set(KeyFounderId, GetActorId(zongZhu).ToString());
		city.data.set(KeyFounderName, zongZhu.getName() ?? string.Empty);
		SetZongZhuId(city, 0L);
		EnsureMainPeak(city);
		XjZongMenCityTraitAccessor.EnsureDefaults(city);
		InitializeMembers(city, zongZhu, cityIndex, year);
		XjZongMenMembershipWriter.AssignZongZhu(city, zongZhu, year, true, "FounderCreated");
		MarkFounderHistory(zongZhu, city, year);
		PublishZongMenFoundation(zongZhu, name, year);
		XjActorAccessor.TryGetString(zongZhu, XjActorDataKeys.DaoTu, out string foundingDaoTu);
		XjThreeBookWriter.RecordSectFounded(zongZhu, stableSectId, name, foundingDaoTu, year);
		return true;
	}
	...
	private static void InitializeMembers(City city, Actor founder, Dictionary<City, List<long>> cityIndex, int currentYear)
	{
		if (city?.data == null)
		{
			return;
		...
		WriteIdList(city, KeyMemberIds, new List<long>(0));
		WriteIdList(city, KeyJoinEvaluatedIds, new List<long>(0));
		if (founder?.data != null)
		{
			XjZongMenMembershipWriter.EnsureMember(city, founder, currentYear, "FounderMember");
			MarkJoinEvaluated(city, founder);
		}

		if (cityIndex == null || !cityIndex.TryGetValue(city, out List<long> candidateIds) || candidateIds == null)
		{
			return;
		}
		...
		for (int i = 0; i < candidateIds.Count; i++)
		{
			...
			MarkJoinEvaluated(city, actor);
			if (SharedRandom.NextDouble() <= 0.5)
			{
				XjZongMenMembershipWriter.EnsureMember(city, actor, currentYear, "FoundationInitialMember");
			}
		}
	}
	...
	private static Actor FindZongMenMasterCandidate(City city, Dictionary<City, List<long>> cityIndex = null)
	{
		if (city == null) return null;

		if (cityIndex == null || !cityIndex.TryGetValue(city, out List<long> candidateIds) || candidateIds == null || candidateIds.Count == 0)
		{
			return null;
		}
	...
	private static bool CanFoundZongMen(Actor actor, bool allowExistingIdentity = false)
	{
		...
		return allowExistingIdentity || !XjZongMenAccessor.BuildIdentity(actor).Found;
	}
	...
	internal static string GenerateZongMenName(City city, Actor founder)
	{
		...
		string title = titles[XjDeterministicHash.PositiveIndex(seed, "zongmen_title", titles.Length)];
	...
	private static string[] ResolveZongMenTitlePool(string daoTu)
	{
		...
		if (IsDaoTuOneOf(value, "金德", "庚金", "齐金", "库金", "逍金", "兑金")) return JinDeZongMenTitles;
