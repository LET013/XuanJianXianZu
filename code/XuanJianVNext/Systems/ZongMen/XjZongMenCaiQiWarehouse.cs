...
internal readonly struct XjZongMenCaiQiWarehouseEntry
{
	...
	internal XjZongMenCaiQiWarehouseEntry(
		bool found,
		long zongMenId,
		string zongMenName,
		string resourceType,
...
internal static class XjZongMenCaiQiWarehouse
{
	...
	private static readonly Dictionary<long, List<XjZongMenCaiQiWarehouseEntry>> entriesByZongMenId = new Dictionary<long, List<XjZongMenCaiQiWarehouseEntry>>();
		...
		bool markChanged = true)
	{
		...
		if (!entriesByZongMenId.TryGetValue(zongMenId, out List<XjZongMenCaiQiWarehouseEntry> entries))
		{
			entries = new List<XjZongMenCaiQiWarehouseEntry>();
			entriesByZongMenId[zongMenId] = entries;
		}

		for (int i = 0; i < entries.Count; i++)
		{
			XjZongMenCaiQiWarehouseEntry existing = entries[i];
			if (existing.ResourceType == ResourceTypeCaiQi && existing.ResourceId == normalizedResourceId)
			{
				...
				entries[i] = new XjZongMenCaiQiWarehouseEntry(
					true,
					existing.ZongMenId,
		...
		entryKeys.Add(key);
		entries.Add(new XjZongMenCaiQiWarehouseEntry(
			true,
			zongMenId,
			zongMenName,
			ResourceTypeCaiQi,
	...
	internal static bool TryGetCaiQiCount(long zongMenId, string resourceId, out int count)
	{
		...
		if (zongMenId <= 0L || string.IsNullOrWhiteSpace(resourceId)
			|| !entriesByZongMenId.TryGetValue(zongMenId, out List<XjZongMenCaiQiWarehouseEntry> entries))
		{
			return false;
		}
	...
	internal static IReadOnlyList<XjZongMenCaiQiWarehouseEntry> ReadCaiQiResources(long zongMenId)
	{
		return ReadEntries(zongMenId, ResourceTypeCaiQi);
	}

	internal static IReadOnlyList<XjZongMenCaiQiWarehouseEntry> ReadCaiQiFaResources(long zongMenId)
	{
	...
	internal static bool TransferSectWarehouse(long sourceZongMenId, long targetZongMenId, string targetZongMenName, int year, out int caiQiStacks, out int caiQiAmount, out int caiQiFaEntries)
	{
		...
		if (sourceZongMenId <= 0L || targetZongMenId <= 0L || sourceZongMenId == targetZongMenId
			|| !entriesByZongMenId.TryGetValue(sourceZongMenId, out List<XjZongMenCaiQiWarehouseEntry> sourceEntries))
		{
			return false;
		}

		List<XjZongMenCaiQiWarehouseEntry> snapshot = new List<XjZongMenCaiQiWarehouseEntry>(sourceEntries);
		entriesByZongMenId.Remove(sourceZongMenId);
		if (!entriesByZongMenId.TryGetValue(targetZongMenId, out List<XjZongMenCaiQiWarehouseEntry> targetEntries))
		{
			targetEntries = new List<XjZongMenCaiQiWarehouseEntry>();
		...
		{
			XjZongMenCaiQiWarehouseEntry entry = snapshot[i];
			...
			if (existingIndex >= 0)
			{
				XjZongMenCaiQiWarehouseEntry existing = targetEntries[existingIndex];
				if (entry.ResourceType == ResourceTypeCaiQi)
				{
					long next = (long)existing.Amount + entry.Amount;
					targetEntries[existingIndex] = new XjZongMenCaiQiWarehouseEntry(
			...
			{
				targetEntries.Add(new XjZongMenCaiQiWarehouseEntry(
		...
		List<XjWorldArchiveZongMenCaiQiRecord> caiQiFaRecords)
	{
		...
		foreach (KeyValuePair<long, List<XjZongMenCaiQiWarehouseEntry>> zongMenEntry in entriesByZongMenId)
		{
			List<XjZongMenCaiQiWarehouseEntry> entries = zongMenEntry.Value;
			if (entries == null)
			{
				continue;
			}
	...
	private static IReadOnlyList<XjZongMenCaiQiWarehouseEntry> ReadEntries(long zongMenId, string resourceType)
	{
		if (zongMenId <= 0L || !entriesByZongMenId.TryGetValue(zongMenId, out List<XjZongMenCaiQiWarehouseEntry> entries) || entries.Count == 0)
		{
			return System.Array.Empty<XjZongMenCaiQiWarehouseEntry>();
		}

		List<XjZongMenCaiQiWarehouseEntry> result = new List<XjZongMenCaiQiWarehouseEntry>();
		for (int i = 0; i < entries.Count; i++)
		{
			XjZongMenCaiQiWarehouseEntry entry = entries[i];
	...
	{
		entryKeys.Clear();
		foreach (KeyValuePair<long, List<XjZongMenCaiQiWarehouseEntry>> pair in entriesByZongMenId)
		{
			List<XjZongMenCaiQiWarehouseEntry> entries = pair.Value;
