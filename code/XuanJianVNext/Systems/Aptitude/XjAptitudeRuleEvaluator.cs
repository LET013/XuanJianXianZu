...
internal static class XjAptitudeRuleEvaluator
{
	...
	internal static XjAptitudeRollResult EvaluateAtAgeFive(Actor actor)
	{
		...
		if (actor.hasTrait(XjHighRealmDescendantRules.DaoTaiDescendantTraitId))
		{
			int descendantXjZz = Math.Max(4, RollXjZz(actor, actorId, aptitudeRollYear));
			int descendantOverlayMask = RollXianTianDaoTi(actorId, aptitudeRollYear, descendantXjZz)
				? XjAptitudeTraitLifecycle.XjZz7Bit : 0;
			return new XjAptitudeRollResult(true, descendantXjZz, descendantOverlayMask, "DaoTaiDescendantGuaranteed");
		}
		if (actor.hasTrait(XjHighRealmDescendantRules.DaoTaiFamilyTraitId))
		{
			int familyXjZz = RollXjZz(actor, actorId, aptitudeRollYear);
			int familyOverlayMask = RollXianTianDaoTi(actorId, aptitudeRollYear, familyXjZz)
				? XjAptitudeTraitLifecycle.XjZz7Bit : 0;
			return new XjAptitudeRollResult(true, familyXjZz, familyOverlayMask, "DaoTaiFamilyGuaranteed");
		}
		...
		int directParentTierFloor = XjBloodlineBirthRules.GetDirectParentRealmAptitudeTierFloor(actor);
		if (directParentTierFloor > 0) xjZz = Math.Max(xjZz, directParentTierFloor);
	...
	internal static bool EnsureDaoTaiLineageAptitude(Actor actor, bool directDescendant)
	{
		if (actor?.data == null || actor.getAge() < 5f) return false;
		XjActorAccessor.TryGetInt(actor, XjActorDataKeys.XjZz, out int current);
		int requiredFloor = directDescendant ? 4 : 1;
		if (current >= requiredFloor) return false;

		long actorId = ((BaseSystemData)actor.data).id;
		if (actorId <= 0L) return false;
		int xjZz = Math.Max(requiredFloor, RollXjZz(actor, actorId, 5));
		XjActorAccessor.SetInt(actor, XjActorDataKeys.XjZz, xjZz);
		XjActorAccessor.SetInt(actor, XjActorDataKeys.XjZzCheckedAge5, 1);
		int overlayMask = RollXianTianDaoTi(actorId, 5, xjZz) ? XjAptitudeTraitLifecycle.XjZz7Bit : 0;
		if (overlayMask != 0) XjActorAccessor.SetInt(actor, XjActorDataKeys.XjZzOverlayMask, overlayMask);
		XjAptitudeRollResult migrated = new XjAptitudeRollResult(true, xjZz, overlayMask,
			directDescendant ? "DaoTaiDescendantMigration" : "DaoTaiFamilyMigration");
		XjAptitudeEffectRules.ApplyOnAgeFiveResult(actor, migrated);
		XjCultivationPathEntrySystem.EnsureInitialPath(actor, xjZz);
	...
	{
		return XjActorAccessor.TryGetFloat(actor, XjActorDataKeys.HuiGuang, out float value) ? XjDaoHuiPolicy.Clamp(value) : 0f;
	}
