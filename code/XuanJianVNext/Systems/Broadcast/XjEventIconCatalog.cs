...
internal static class XjEventIconCatalog
{
	...
	internal const string YuYi = "Yuyi";
	...
	internal const string LongShu = "iconLongShu";
	...
	internal static string BuildIconPath(string iconId)
	{
		...
		if (string.Equals(normalized, LongShu, StringComparison.Ordinal))
		{
			// LongShu has a dedicated mod icon outside ui/Icons/event. Use the exact
			// GameResources path so a same-name native/other-mod sprite cannot win first.
			return "GameResources/ui/Icons/iconLongShu.png";
		}
	...
	internal static string NormalizeIconId(string iconId)
	{
		...
		string value = iconId.Trim().Replace("\\", "/");
		if (value.EndsWith("/iconLongShu.png", StringComparison.OrdinalIgnoreCase)
			|| value.EndsWith("/iconLongShu", StringComparison.OrdinalIgnoreCase)
			|| string.Equals(value, LongShu, StringComparison.OrdinalIgnoreCase))
		{
			return LongShu;
		}
	...
	internal static string ResolveFromWorldHistoryText(string text)
	{
		...
		if (HasAny(text, "龙属肇生", "龙属出海", "龙君出海", "龙脉引水", "合水之征", "合水化泽", "龙属重现"))
		{
			return LongShu;
		}
		...
		if (HasAny(text, "郁仪仙入位", "结璘仙入位", "郁仪入位", "结璘入位"))
		{
			return JinDanUpgrade;
		}

		if (HasAny(text, "郁仪", "郁仪仙"))
		{
			return YuYi;
		}
