...
internal static class XjEraAssetRegistration
{
	internal static void Init()
	{
		...
		// 仅九项，允许在金丹切纪元前再次校准，避免其他初始化顺序覆盖图标字段。
	...
	internal static bool TrySetCurrentAgeForDaoTu(string daoTuOrGroup, string changeCause)
	{
		...
		try
		{
			...
			if (switched)
			{
				...
				try
				{
					var currentAge = eraManager.getCurrentAge();
					if (currentAge != null)
					{
						var yearField = currentAge.GetType().GetField("year",
						...
						if (yearField != null)
						{
							...
							if (worldYear > 0)
							{
								yearField.SetValue(currentAge, worldYear);
							}
						}
					}
				}
