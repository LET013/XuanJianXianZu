...
internal static class XjActorCultivationSnapshotBuilder
{
	...
	private static XjActorCultivationSnapshot BuildInternal(Actor actor, bool includeHighRealmState)
	{
		...
		XjActorAccessor.TryGetFloat(actor, XjActorDataKeys.HuiGuang, out float rawHuiGuang);
		// 快照构建保持纯读。旧档超界道慧由年度 Prepare 的显式迁移入口归一化；
		// UI/战斗/排行榜读取只使用钳制后的运行值，不反写持久状态。
		float huiGuang = XjDaoHuiPolicy.Clamp(rawHuiGuang);
		...
		huiGuang = XjDaoHuiPolicy.ApplyDaoTuRealmModifier(actor, huiGuang);
