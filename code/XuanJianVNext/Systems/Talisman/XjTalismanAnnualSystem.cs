...
internal static class XjTalismanAnnualSystem
{
	...
	private static void TryGatherMaterials(Actor actor, XjCraftOwnerKey owner, int currentYear)
	{
		...
		// 材料采集由两年降为四年一次，按两期累计量批发，维持长期资源吞吐。
		int lowPaper = (3 + XjDeterministicHash.PositiveIndex(actorId + currentYear, "talisman.paper.low", 5)) * 2;
		int lowInk = (2 + XjDeterministicHash.PositiveIndex(actorId + currentYear, "talisman.ink.low", 4)) * 2;
		...
		{
			int highAmount = (1 + XjDeterministicHash.PositiveIndex(actorId + currentYear, "talisman.material.high.amount", 2)) * 2;
	...
	private static void AdvanceTask(Actor actor, XjCraftOwnerKey owner, XjCraftTaskArchiveRecord task, int currentYear)
	{
		...
		if (!task.OutcomeCalculated)
		{
			...
			int baseQuantity = task.Success
				...
				: 0;
			// 新制符批次由两年规划降为四年规划，成功批次按两期产量结算，避免只降频却把长期供给砍半。
			task.Quantity = task.Success ? XjCraftProficiencySystem.ResolveTalismanYield(actor, baseQuantity) * 2 : 0;
