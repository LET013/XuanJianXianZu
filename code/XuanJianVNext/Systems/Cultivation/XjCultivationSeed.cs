...
internal static class XjCultivationSeed
{
	internal static void EnsureSeedState(Actor actor)
	{
		...
		BaseSystemData actorData = (BaseSystemData)actor.data;
		float legacyTotalMingShu = 0f;
		float legacyCongenitalMingShu = 0f;
		float legacyAcquiredMingShu = 0f;
		actorData.get(XjMingShuState.LegacyTotalKey, out legacyTotalMingShu, 0f);
		actorData.get(XjMingShuState.LegacyCongenitalKey, out legacyCongenitalMingShu, 0f);
		actorData.get(XjMingShuState.LegacyAcquiredKey, out legacyAcquiredMingShu, 0f);

		float existingMingShu = 0f;
		XjActorAccessor.TryGetFloat(actor, XjActorDataKeys.MingShu, out existingMingShu);
		if (existingMingShu <= 0f && legacyTotalMingShu > 0f) existingMingShu = legacyTotalMingShu;
		if (!XjActorAccessor.TryGetFloat(actor, XjActorDataKeys.MingShuCongenital, out float mingShuCongenital) || mingShuCongenital <= 0f)
		{
			float seed = legacyCongenitalMingShu > 0f
				? ToIntegerValue(legacyCongenitalMingShu)
				: existingMingShu > 0f
					? ToIntegerValue(Math.Max(0f, existingMingShu - Math.Max(0f, legacyAcquiredMingShu)))
					: XjCongenitalMingShuPolicy.Roll(actorId, actor.getName());
		...
		XjActorAccessor.TryGetFloat(actor, XjActorDataKeys.MingShuAcquired, out float mingShuAcquired);
		if (mingShuAcquired <= 0f && legacyAcquiredMingShu > 0f)
		{
			mingShuAcquired = legacyAcquiredMingShu;
		}
		mingShuAcquired = Math.Max(0f, ToIntegerValue(mingShuAcquired));
		...
		{
			XjActorAccessor.SetFloat(actor, XjActorDataKeys.HuiGuang, XjDaoHuiPolicy.Clamp(XjDeterministicHash.BuildSeedInteger(actorId, actor.getName(), 1, 6, 60)));
		}
		else
		{
			XjActorAccessor.SetFloat(actor, XjActorDataKeys.HuiGuang, XjDaoHuiPolicy.Clamp(huiGuang));
		}
	...
	internal static bool TryApplyBloodlineSeedInheritance(Actor actor)
	{
		...
		float inheritedHuiGuang = Math.Max(
			XjDaoHuiPolicy.Clamp(huiGuang),
			XjDaoHuiPolicy.Clamp(Math.Max(
				Lerp(Math.Max(1f, huiGuang), XjDaoHuiPolicy.Maximum, inheritanceBias),
				huiGuangFloor)));
	...
	private static int ResolveChuShenRank(Actor actor)
	{
		...
		bool hasZongMen = XjSectIdentityReader.BuildIdentity(actor).Found;
		if (hasZongMen)
		{
