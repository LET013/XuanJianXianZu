...
internal static class XjBreakthroughRules
{
		...
		string targetRealmId)
	{
		...
		if ((snapshot.XjZzOverlayMask & ((1 << 8) | (1 << 9))) != 0)
		{
			return new XjBreakthroughAttemptResult(false, false, "AptitudeInjuryBlocksRealm");
		}

		// 道主之资无视普通资质限制，但不能绕过经脉伤损；先天道体不提供突破增幅。
		if (!actor.hasTrait("ChuShen8") && !CanAttemptByAptitude(snapshot.XjZz, targetRealmId))
		{
	...
	private static bool MeetsNaturalAgeAndTenureGate(Actor actor, string targetRealmId, int currentYear, out string reason)
	{
		...
		int age;
		try { age = (int)Math.Floor(Math.Max(0f, actor.getAge())); } catch { age = 0; }
	...
	private static bool HasAvailableCaiQiForBreakthrough(Actor actor, in XjCaiQiSnapshot caiQiSnapshot)
	{
		...
		if (caiQiSnapshot.ResourceCount > 0 && !string.IsNullOrWhiteSpace(caiQiSnapshot.ResourceId))
		{
			return true;
		}
	...
	private static bool TryResolvePreferredCaiQiResourceId(Actor actor, out string resourceId)
	{
		...
		return XjFamilyDaoTuRules.TryResolvePreferredDaoTu(actor, out string preferredDaoTu)
	...
	{
		// 道主之资通过全部境界成败判定，包括紫府；先天道体仍不绕过紫府70%上限。
		...
		float finalCap = string.Equals(targetRealmId, XjRealmIds.ZiFu, StringComparison.Ordinal)
			? 0.70f
			: 0.98f;
		float chance = Math.Max(0f, Math.Min(finalCap, maxChance * mingShuFactor + faBaoBonus + pillBonus + talismanBonus));
