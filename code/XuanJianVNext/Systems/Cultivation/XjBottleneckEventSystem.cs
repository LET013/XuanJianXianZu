...
internal static class XjBottleneckEventSystem
{
	...
	internal static string BuildStatusSummary(Actor actor)
	{
		...
		XjActorAccessor.TryGetInt(actor, XjActorDataKeys.XjBottleneckNextAttemptYear, out int nextAttemptYear);
		string years = startedYear > 0 ? "，始于" + XjChronology.FormatYear(startedYear) : string.Empty;
		string retry = nextAttemptYear > 0 ? "，下次冲关" + XjChronology.FormatYear(nextAttemptYear) : string.Empty;
		float stored = ReadStoredZhenYuan(actor);
		...
		string chanceLabel = nextAttemptYear > currentYear ? "下次冲关把握" : "当前冲关把握";
		string chanceText = "，" + chanceLabel + "：" + DescribeBreakthroughChance(chance);
		return definition.DisplayName + chanceText + years + retry + reserve;
	}

	private static string DescribeBreakthroughChance(float chance)
	{
		if (chance < 0.12f) return "渺茫";
		if (chance < 0.28f) return "偏低";
		if (chance < 0.48f) return "尚可一试";
		if (chance < 0.68f) return "颇有把握";
		return "十拿九稳";
	}
	...
	private static int GetCurrentYear()
	{
		...
		catch (System.Exception xjCaught281_1)
		{
			XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/Cultivation/XjBottleneckEventSystem.cs:281", xjCaught281_1);
	...
	private static bool HasBottleneckBypass(Actor actor)
	{
		...
		catch (System.Exception xjCaught293_2)
		{
			XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/Cultivation/XjBottleneckEventSystem.cs:293", xjCaught293_2);
	...
	private static string ResolveActorName(Actor actor)
	{
		...
		catch (System.Exception xjCaught351) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/Cultivation/XjBottleneckEventSystem.cs:351", xjCaught351); }
