...
internal static class XjAlchemyInventoryRegistry
{
	...
	internal static int RuntimeEntryCount
	{
		get
		{
			lock (sync)
			{
				return materialsByOwner.Count + recipesByOwner.Count + pillBatchesById.Count
					+ batchIdsByOwner.Count + pillCountsByOwner.Count + lastPrunedYearByOwner.Count;
			}
		}
	}
	...
	private static int GetRecipeBit(string recipeId)
	{
		...
		if (string.Equals(recipeId, XjAlchemyCatalog.RecipeBaoYiWenJin, StringComparison.Ordinal)) return 1 << 14;
		if (string.Equals(recipeId, XjAlchemyCatalog.RecipeCunShenYangShen, StringComparison.Ordinal)) return 1 << 15;
		if (string.Equals(recipeId, XjAlchemyCatalog.RecipeXingMingHeZhen, StringComparison.Ordinal)) return 1 << 16;
		return 0;
	}
