...
internal static class XjAlchemyPillEffectSystem
{
	...
	private static bool ApplyLifespan(Actor actor, int years, int currentYear)
	{
		...
		try { actor.stats["lifespan"] = Math.Max(0f, actor.stats["lifespan"]) + years; } catch { }
	...
	private static string ResolveHealingPill(string realmId)
	{
		if (string.Equals(realmId, XjRealmIds.JinDan, StringComparison.Ordinal)
			|| string.Equals(realmId, XjRealmIds.ZiFu, StringComparison.Ordinal)) return XjAlchemyCatalog.PillTianYiTuCui;
		if (string.Equals(realmId, XjRealmIds.ZhuJi, StringComparison.Ordinal)) return XjAlchemyCatalog.PillHuMai;
		if (string.Equals(realmId, XjRealmIds.TaiXi, StringComparison.Ordinal)
			|| string.Equals(realmId, XjRealmIds.LianQi, StringComparison.Ordinal)) return XjAlchemyCatalog.PillYangYuan;
		return string.Empty;
	}
