...
internal static class XjCombatEffectPool
{
    ...
    // ==================== 自定义精灵特效硬预算 ====================
    // 0.9.8.4 以后具名神通大量增加，旧实现只有“失败熔断”，成功生成本身没有
    // 总量上限。大规模高境斗法时，同一帧可把大量 fx_slash 容器推入原生对象池，
    // 原生池会保留历史峰值，从而把一次视觉洪峰变成长期内存高水位。这里对玄鉴
    // 自定义 SpriteAnimation 增加无日志硬预算：战斗结算照常进行，只允许视觉降级。
    private static int _visualBudgetFrame = -1;
    private static int _visualSpawnsThisFrame;
    private static int _visualBudgetSecond = -1;
    private static int _visualSpawnsThisSecond;
    private static long _visualSpawnsDropped;

    private static bool TryReserveCustomVisualSpawn()
    {
        int frame = Time.frameCount;
        if (_visualBudgetFrame != frame)
        {
            _visualBudgetFrame = frame;
            _visualSpawnsThisFrame = 0;
        }

        int second = Mathf.FloorToInt(Mathf.Max(0f, Time.unscaledTime));
        if (_visualBudgetSecond != second)
        {
            _visualBudgetSecond = second;
            _visualSpawnsThisSecond = 0;
        }

        int actors = Math.Max(0, XjScheduler.GetKnownActorCount());
        int perFrame;
        int perSecond;
        if (actors >= 8000)
        {
        ...
        if (_visualSpawnsThisFrame >= perFrame || _visualSpawnsThisSecond >= perSecond)
        {
            _visualSpawnsDropped++;
            return false;
        }

        _visualSpawnsThisFrame++;
        _visualSpawnsThisSecond++;
        return true;
    }

    internal static long GetDroppedCustomVisualSpawnCount() => _visualSpawnsDropped;
        ...
        int repeatFrameSequence = 1)
    {
        ...
        if (IsEffectBlacklisted(failureKey))
            return false;
        if (!TryReserveCustomVisualSpawn())
            return false;
        ...
        catch
        {
            ...
            catch (System.Exception xjCaught133) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/Combat/XjCombatEffectPool.cs:133", xjCaught133); }
        ...
        float holdSeconds)
    {
        ...
        catch
        {
            ...
            catch (System.Exception xjCaught214) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/Combat/XjCombatEffectPool.cs:214", xjCaught214); }
        ...
        int sortingOrder = 0)
    {
        ...
        catch
        {
            ...
            catch (System.Exception xjCaught296) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/Combat/XjCombatEffectPool.cs:296", xjCaught296); }
        ...
        Func<bool> keepAlive = null)
    {
        ...
        catch
        {
            ...
            catch (System.Exception xjCaught394) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/Combat/XjCombatEffectPool.cs:394", xjCaught394); }
    ...
    private static void ResetAnimationPlayback(BaseEffect effect, SpriteAnimation animation)
    {
        ...
        catch (System.Exception xjCaught531) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/Systems/Combat/XjCombatEffectPool.cs:531", xjCaught531); }
    }
    ...
    internal static void ClearFrameCache()
    {
        _cachedFrames.Clear();
        _cachedActorCount = 0;
        _visualBudgetFrame = -1;
        _visualSpawnsThisFrame = 0;
        _visualBudgetSecond = -1;
        _visualSpawnsThisSecond = 0;
        _visualSpawnsDropped = 0L;
    }
