...
internal static class XjSectFormationRegistry
{
	...
	private static IReadOnlyList<XjSectFormationArchiveRecord> _cachedReadAll = Array.Empty<XjSectFormationArchiveRecord>();
	...
	{
		if (_cachedReadRevision == _readRevision) return _cachedReadAll;
		if (BySectId.Count == 0)
		{
			_cachedReadAll = Array.Empty<XjSectFormationArchiveRecord>();
	...
	{
		return RescaleDurabilityForSectPower(sectId, ziFuCount, jinDanCount, 60);
	}

	internal static bool RescaleDurabilityForSectPower(long sectId, int ziFuCount, int jinDanCount, int prosperityValue)
	{
		if (!BySectId.TryGetValue(sectId, out XjSectFormationArchiveRecord record) || record == null || record.Grade <= 0) return false;
		return RescaleDurability(record, XjSectFormationBalance.GetMaxDurability(record.Grade, ziFuCount, jinDanCount, prosperityValue));
	}

	/// <summary>
	/// 第三方技术换身只替换 actorId，不应让仍在主持的宗门阵法失去主持者。
	/// 历史任务记录不改写，仅迁当前阵法主持引用。
	/// </summary>
	internal static bool RebindLeadActorAfterExternalReplacement(long oldActorId, long newActorId)
	{
		if (oldActorId <= 0L || newActorId <= 0L || oldActorId == newActorId) return false;
		bool changed = false;
		foreach (XjSectFormationArchiveRecord record in BySectId.Values)
		{
			if (record == null || record.LeadFormationMasterId != oldActorId) continue;
	...
	internal static void ExportArchiveRecords(List<XjSectFormationArchiveRecord> target)
	{
		...
		target.Clear();
		IReadOnlyList<XjSectFormationArchiveRecord> snapshot = ReadAll();
		for (int i = 0; i < snapshot.Count; i++) target.Add(Clone(snapshot[i]));
	}
