...
internal static class XjSectFormationOccupationAdapter
{
	...
	private static readonly Dictionary<(long CityId, long AttackerId), float> XianGuoCaptureRemainders = new Dictionary<(long, long), float>();
	...
	internal static void BeginNativeCapture(City city, Kingdom attackingKingdom, ref int nativeDelta)
	{
		...
		if (cityId <= 0L || attackerId <= 0L || defenderKingdomId <= 0L) return;
		int currentYear = GetCurrentYear();
		PruneTransientCaches(currentYear);

		// 仙国法直接接入 WorldBox 原生 City.addCapturePoints：敌国攻占仙国城市时，
		// 原生占领点增量只保留 40%，即被占领速度降低 60%。使用小数余量累计，
		// 即使原生每次只加 1 点也不会因为整数截断变成“永久无法占领”。
		if (XjXianGuoSystem.IsActiveDynastyKingdom(defenderKingdomId))
		{
			nativeDelta = ScaleNativeDelta(
				XianGuoCaptureRemainders,
				cityId,
				attackerId,
				nativeDelta,
				XjXianGuoSystem.NativeCaptureSpeedMultiplier);
	...
	internal static void Clear()
	{
		...
		XianGuoCaptureRemainders.Clear();
	...
	{
		return ScaleNativeDelta(CaptureRemainders, cityId, attackerId, originalDelta, multiplier);
	}

	private static int ScaleNativeDelta(
		Dictionary<(long CityId, long AttackerId), float> remainders,
		long cityId,
		long attackerId,
		int originalDelta,
		float multiplier)
	...
	private static bool TryResolveStoredActorSectId(Actor actor, out long sectId)
	{
		...
		return actorId > 0L
			&& XjSectAuthorityStore.TryGetSectId(actorId, out sectId)
			&& sectId > 0L
			&& XjSectRepository.TryGetBySectId(sectId, out _);
	}
	...
	{
		return XjNativeCityCaptureInterop.GetCapturingUnits(city);
	}

	private static Actor ExtractActor(object value)
	{
		return XjNativeCityCaptureInterop.ExtractActor(value);
	}
