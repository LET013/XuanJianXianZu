...
internal static class XjFormationEngineeringSystem
{
	...
	private static void GatherBasicMaterials(Actor actor, XjCraftOwnerKey owner, int currentYear)
	{
		...
		// 三年一次批结算，资源与熟练度按三年累计量发放，降低调用频率但不砍长期产出。
		XjCraftDomainRegistry.TryAddResource(owner, XjCraftCollaborationSystem.FormationRune, (5 + rank * 3) * 3, currentYear);
		XjCraftDomainRegistry.TryAddResource(owner, XjCraftCollaborationSystem.SpiritMaterial, Math.Max(3, rank + 1) * 3, currentYear);
		XjCraftDomainRegistry.TryAddResource(owner, XjCraftCollaborationSystem.FormationFlagLow, (3 + rank * 2) * 3, currentYear);
		if (rank >= XjCraftProficiencySystem.RankZiFu)
		{
			XjCraftDomainRegistry.TryAddResource(owner, XjCraftCollaborationSystem.FormationFlagHigh, (2 + rank / 2) * 3, currentYear);
			XjCraftDomainRegistry.TryAddResource(owner, XjCraftCollaborationSystem.FormationPlateHigh, 6, currentYear);
		}
		XjCraftProficiencySystem.RecordFormationProgress(actor, 3);
	}
