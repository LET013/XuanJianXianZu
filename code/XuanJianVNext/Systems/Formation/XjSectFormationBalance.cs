...
internal static class XjSectFormationBalance
{
	...
	{
		return GetMaxDurability(grade, ziFuCount, jinDanCount, 60);
	}

	internal static int GetMaxDurability(int grade, int ziFuCount, int jinDanCount, int prosperityValue)
	{
		...
		multiplier *= GetProsperityMultiplier(prosperityValue);
		double scaled = baseDurability * multiplier;
		if (scaled >= int.MaxValue) return int.MaxValue;
		return Math.Max(100, (int)Math.Round(scaled));
	}

	internal static float GetProsperityMultiplier(int prosperityValue)
	{
		// 兴衰只作为温和修正：濒灭宗门仍有残阵，鼎盛宗门也不会凭兴衰无限膨胀。
		int value = Math.Clamp(prosperityValue, 0, 100);
		return 0.65f + value * 0.005f;
	}
