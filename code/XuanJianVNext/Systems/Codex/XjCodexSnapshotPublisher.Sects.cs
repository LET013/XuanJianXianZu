...
private static List<XjCodexSectItem> BuildSects(
		...
		IReadOnlyDictionary<long, int> ziFuPathBySect,
		IReadOnlyDictionary<long, int> fuQiZhenRenBySect,
		IReadOnlyDictionary<long, int> ziJinJinDanBySect,
		IReadOnlyDictionary<long, int> zhenJunYuShiBySect,
		IReadOnlyDictionary<long, XjDaoTuTraditionSummary> daoTuTraditionBySect,
		...
		IReadOnlyDictionary<long, string> familyNames)
	{
		...
		for (int i = 0; i < records.Count; i++)
		{
			...
			XjDaoTuTraditionSummary tradition = default;
			if (daoTuTraditionBySect != null) daoTuTraditionBySect.TryGetValue(record.SectId, out tradition);
			XjDaoTuControlSummary control = tradition.Found
				? XjDaoTuHeritageService.ResolveSectControl(record.SectId, tradition.DaoTu)
				: default;
			result.Add(new XjCodexSectItem
			{
				...
				ProsperityValue = Math.Clamp(record.ProsperityValue, 0, 100),
				ProsperityTier = string.IsNullOrWhiteSpace(record.ProsperityTier) ? XjSectProsperitySystem.ResolveTier(record.ProsperityValue) : record.ProsperityTier,
				ProsperitySummary = record.ProsperitySummary ?? string.Empty,
				FormationProsperityMultiplier = XjSectFormationBalance.GetProsperityMultiplier(record.ProsperityValue),
				...
				ZiFuPathCount = ziFuPathBySect.TryGetValue(record.SectId, out int ziFuPath) ? ziFuPath : 0,
				FuQiZhenRenCount = fuQiZhenRenBySect.TryGetValue(record.SectId, out int fuQiZhenRen) ? fuQiZhenRen : 0,
				ZiJinJinDanCount = ziJinJinDanBySect.TryGetValue(record.SectId, out int ziJinJinDan) ? ziJinJinDan : 0,
				ZhenJunYuShiCount = zhenJunYuShiBySect.TryGetValue(record.SectId, out int zhenJunYuShi) ? zhenJunYuShi : 0,
				PrimaryDaoTu = tradition.Found ? tradition.DaoTu : string.Empty,
				DaoTuInheritancePercent = tradition.Found ? (control.IsMonopoly ? 100 : tradition.InfluencePercent) : 0,
				DaoTuHighRealmSourceCount = tradition.Found ? tradition.SourceCount : 0,
				DaoTuHeldPositions = control.HeldPositions,
				DaoTuActivePositions = control.ActivePositions,
				DaoTuPositionCapacity = control.TotalCapacity,
				DaoTuHoldsFruit = control.HoldsFruit,
				DaoTuMonopoly = control.IsMonopoly,
				DaoTuControlSummary = tradition.Found ? control.Display : string.Empty,
	...
	private static void AttachSectRelations(List<XjCodexSectItem> items, IReadOnlyList<XjSectArchiveRecord> records)
	{
		...
		for (int i = 0; i < hostilities.Count; i++)
		{
			...
			UpsertSectRelationCandidate(candidates, hostility.LeftSectId, hostility.RightSectId, relation, hostilityValue,
				XjSectWarSystem.NormalizeReasonForDisplay(hostility.LastReason, "两宗已有难以忽略的嫌隙"));
		}
		...
		ref long bestActorId)
	{
		...
		if (preferStoredRank)
		{
			if (!XjSectAuthorityStore.TryGetMember(actorId, out XjSectMemberArchiveRecord member)
				...
				|| !string.Equals(XjSectMemberRole.Normalize(member.Role), XjSectMemberRole.Sovereign, StringComparison.Ordinal)) return;
		}
		...
		if (records != null)
		{
			for (int i = 0; i < records.Count; i++)
			{
				...
				for (int j = 0; j < actorIds.Count; j++)
				{
					...
					int storedPeakId = XjSectAuthorityStore.TryGetMember(actorId, out XjSectMemberArchiveRecord member)
						&& member.SectId == record.SectId ? member.PeakId : 0;
