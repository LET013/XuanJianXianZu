...
internal static partial class XjCodexSnapshotPublisher
{
		...
		IReadOnlyList<XjTalismanBatchArchiveRecord> talismanBatches)
	{
		...
		for (int i = 0; i < records.Count; i++)
		{
			...
			IReadOnlyList<XjZongMenGongFaPavilionEntry> gongFa = XjZongMenGongFaPavilion.ReadGongFaEntries(sect.SectId);
			int displayedGongFa = 0;
			for (int g = 0; g < gongFa.Count; g++)
			{
				XjZongMenGongFaPavilionEntry entry = gongFa[g];
				...
				result.Add(new XjCodexSectResourceItem
				{
					...
					Year = entry.Year,
					Detail = "映照仙基：" + EmptyText(entry.MappedXianJi, "未明")
				});
			}
			AddOverflowResource(result, sect, sectName, "功法", gongFa.Count - displayedGongFa);

			IReadOnlyList<XjZongMenGongFaPavilionEntry> qiuJinFa = XjZongMenGongFaPavilion.ReadQiuJinFaEntries(sect.SectId);
			int displayedQiuJinFa = 0;
			for (int q = 0; q < qiuJinFa.Count; q++)
			{
				XjZongMenGongFaPavilionEntry entry = qiuJinFa[q];
				if (!entry.Found || string.IsNullOrWhiteSpace(entry.Name)) continue;
			...
			IReadOnlyList<XjZongMenCaiQiWarehouseEntry> caiQi = XjZongMenCaiQiWarehouse.ReadCaiQiResources(sect.SectId);
			for (int c = 0; c < caiQi.Count; c++)
			{
				XjZongMenCaiQiWarehouseEntry entry = caiQi[c];
			...
			IReadOnlyList<XjZongMenCaiQiWarehouseEntry> caiQiFa = XjZongMenCaiQiWarehouse.ReadCaiQiFaResources(sect.SectId);
			int displayedCaiQiFa = 0;
			for (int f = 0; f < caiQiFa.Count; f++)
			{
				XjZongMenCaiQiWarehouseEntry entry = caiQiFa[f];
