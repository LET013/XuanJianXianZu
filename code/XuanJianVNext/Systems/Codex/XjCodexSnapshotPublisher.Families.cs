		...
		out Dictionary<long, string> familyNames)
	{
		...
		for (int i = 0; i < aggregates.Count; i++)
		{
			...
			byFamily[aggregate.FamilyStableId] = new XjCodexFamilyItem
			{
				...
				ZiFuPathCount = aggregate.ZiFuPathCount,
				FuQiZhenRenCount = aggregate.FuQiZhenRenCount,
				ZiJinJinDanCount = aggregate.ZiJinJinDanCount,
				ZhenJunYuShiCount = aggregate.ZhenJunYuShiCount,
		...
		for (int i = 0; i < items.Count; i++)
		{
			...
			{
				item.ActiveAspiration = ResolveFamilyAspirationDisplay(stageState.ActiveAspiration);
				...
				if (item.SupportedActorId > 0L
					&& XjScheduler.ResolveActor(item.SupportedActorId, out Actor supportedActor)
					&& XjCultivationPathRules.IsFuQiYangXing(supportedActor))
				{
					if (string.Equals(item.SupportPurpose, XjCenturyFamilySupportPurpose.FuChiZiFu, StringComparison.Ordinal))
					{
						item.SupportPurpose = "扶持真人";
					}
					else if (string.Equals(item.SupportPurpose, XjCenturyFamilySupportPurpose.FuChiQiuJin, StringComparison.Ordinal))
					{
						item.SupportPurpose = "扶持真君羽士";
					}
				}
		...
		for (int i = 0; i < items.Count; i++)
		{
			...
			ApplyLiveFamilyRealmProjection(item, familyEntries, includeMemberDetails);
	...
	bool includeMemberDetails)
{
	...
	for (int i = 0; i < entries.Count; i++)
	{
		...
		int historicalOrder = ResolveFamilyRealmOrder(entry.RealmId, entry.RealmDisplay);
		...
		if (historicalOrder > historicalHighestOrder)
		{
			...
			historicalHighestRealm = ResolveFamilyRealmDisplay(entry.RealmId, entry.RealmDisplay);
		}
		...
		Actor actor = null;
		float power = 0f;
		bool needsLiveCultivationProjection = historicalOrder > 0;
		bool live = needsLiveCultivationProjection && entry.ActorId > 0L
			&& XjScheduler.ResolveActor(entry.ActorId, out actor)
			&& actor?.data != null
			&& actor.isAlive();
		XjShiSnapshot shiSnapshot = default;
		bool liveShi = live && XjCultivationPathRules.IsShi(actor)
			&& XjShiState.TryBuildSnapshot(actor, out shiSnapshot);
		if (liveShi)
		{
			realmId = XjShiCatalog.GetRealmTraitId(shiSnapshot.Realm);
			realmDisplay = XjShiCatalog.GetRealmDisplay(shiSnapshot.Realm);
			actorName = SafeActorName(actor);
			power = XjShiPowerRules.GetCombatTier(actor) * 1000f;
		}
		else if (live)
		{
		...
		int order = liveShi ? ResolveShiFamilyRealmOrder(shiSnapshot.Realm) : ResolveFamilyRealmOrder(realmId, realmDisplay);
		if (order > 0) cultivators++;
		if (liveShi)
		{
			shiCount++;
			if (XjShiCatalog.GetRank(shiSnapshot.Realm) >= XjShiCatalog.GetRank(XjShiRealmIds.MoHe)) shiHighRealmCount++;
		}
		if (includeMemberDetails && order > 0 && item.Members.Count < MaxSnapshotFamilyMembers)
		{
			XjCodexFamilyMemberItem member = new XjCodexFamilyMemberItem
			{
				ActorId = entry.ActorId,
				Name = actorName ?? string.Empty,
				Realm = ResolveFamilyRealmDisplay(realmId, realmDisplay),
				DaoTu = liveShi ? "释修" : live && XjActorAccessor.TryGetString(actor, XjActorDataKeys.DaoTu, out string daoTu) ? daoTu ?? string.Empty : string.Empty,
				Role = liveShi ? "家族释修" : "家族修士",
				RealmOrder = order
			};
			if (live)
			{
				...
				if (liveShi)
				{
					member.XianJiCount = 0;
					member.KeySummary = "释修旁脉";
				}
				else
				{
					if (order >= XjRealmHelper.GetOrder(XjRealmIds.ZhuJi)) member.XianJiCount = XjActorCultivationSnapshotBuilder.Build(actor).XianJiCount;
		...
		if (!liveShi)
		{
			switch (XjHighRealmIdentity.ResolveKind(live ? actor : null, realmId))
			{
				case XjHighRealmKind.ZiFu: ziFu++; ziFuPath++; break;
				case XjHighRealmKind.FuQiZhenRen: ziFu++; fuQiZhenRen++; break;
				case XjHighRealmKind.ZiJinJinDan: jinDan++; ziJinJinDan++; break;
				case XjHighRealmKind.ZhenJunYuShi: jinDan++; zhenJunYuShi++; break;
			}
		}
		if (order > highestOrder)
		{
			highestOrder = order;
			highestRealm = ResolveFamilyRealmDisplay(realmId, realmDisplay);
		}

		bool qualifiedShi = liveShi && XjShiCatalog.GetRank(shiSnapshot.Realm) >= XjShiCatalog.GetRank(XjShiRealmIds.MoHe);
		if (live && (qualifiedShi || !liveShi && (order > XjRealmHelper.GetOrder(XjRealmIds.ZhuJi) || IsStrictZhuJiLate(actor))))
		{
			item.QualifiedHighRealmCount++;
			int qualifiedScore = qualifiedShi
				? XjShiCatalog.GetRank(shiSnapshot.Realm) >= XjShiCatalog.GetRank(XjShiRealmIds.DharmaForm) ? 40 : 8
				: order >= XjRealmHelper.GetOrder(XjRealmIds.JinDan) ? 40
			...
			if (order > item.StrongestQualifiedRealmOrder)
			{
				...
				item.StrongestQualifiedRealm = ResolveFamilyRealmDisplay(realmId, realmDisplay);
			}
		}
	...
	item.ZiFuPathCount = ziFuPath;
	item.FuQiZhenRenCount = fuQiZhenRen;
	item.ZiJinJinDanCount = ziJinJinDan;
	item.ZhenJunYuShiCount = zhenJunYuShi;
	item.ShiCount = shiCount;
	item.ShiHighRealmCount = shiHighRealmCount;
	...
	item.HighestRealm = !string.IsNullOrWhiteSpace(highestRealm) ? highestRealm : historicalCultivators > 0 ? "凡俗余脉" : alive > 0 ? "凡俗" : string.Empty;
...
private static int ResolveFamilyRealmOrder(string realmId, string realmDisplay)
{
	if (XjShiCatalog.TryResolveRealmByTrait((realmId ?? string.Empty).Trim(), out string shiRealm)) return ResolveShiFamilyRealmOrder(shiRealm);
	string display = (realmDisplay ?? string.Empty).Trim();
	if (display.Contains("世尊", StringComparison.Ordinal)) return 7;
	if (display.Contains("法相", StringComparison.Ordinal)) return 6;
	if (display.Contains("摩诃", StringComparison.Ordinal)) return 5;
	if (display.Contains("怜愍", StringComparison.Ordinal)) return 4;
	if (display.Contains("法师", StringComparison.Ordinal)) return 3;
	if (display.Contains("僧侣", StringComparison.Ordinal)) return 1;
	return XjRealmHelper.GetOrder(realmId);
}

private static int ResolveShiFamilyRealmOrder(string shiRealm)
{
...
private static string ResolveFamilyRealmDisplay(string realmId, string fallback)
{
	if (XjShiCatalog.TryResolveRealmByTrait((realmId ?? string.Empty).Trim(), out string shiRealm)) return XjShiCatalog.GetRealmDisplay(shiRealm);
	return string.IsNullOrWhiteSpace(fallback) ? XjRealmHelper.GetDisplayName(realmId) : fallback;
}
	...
	IReadOnlyList<XjFamilyMemberLedgerEntry> entries)
{
	...
	if (XjDaoTuHeritageService.TryResolveFamilyTradition(
		item.FamilyId, 0L, out XjDaoTuTraditionSummary tradition))
	{
		XjDaoTuControlSummary control = XjDaoTuHeritageService.ResolveFamilyControl(item.FamilyId, tradition.DaoTu);
		item.PrimaryDaoTu = tradition.DaoTu;
		item.DaoTuInheritancePercent = control.IsMonopoly ? 100 : tradition.InfluencePercent;
		item.DaoTuHighRealmSourceCount = tradition.SourceCount;
		item.DaoTuHeldPositions = control.HeldPositions;
		item.DaoTuActivePositions = control.ActivePositions;
		item.DaoTuPositionCapacity = control.TotalCapacity;
		item.DaoTuHoldsFruit = control.HoldsFruit;
		item.DaoTuMonopoly = control.IsMonopoly;
		item.DaoTuControlSummary = control.Display;
	}
...
private static string BuildPrivilegeSummary(XjSectFamilySeatArchiveRecord seat)
	{
		...
		string year = seat.LastPrivilegeIncidentYear > 0
			? "，最近纪事：" + XjChronology.FormatYear(seat.LastPrivilegeIncidentYear)
			: string.Empty;
...
private static string ResolveFamilyAspirationDisplay(string aspiration)
{
	string value = aspiration?.Trim() ?? string.Empty;
	if (string.Equals(value, XjCenturyFamilyAspiration.YuFu, StringComparison.Ordinal)) return "育真人";
	if (string.Equals(value, XjCenturyFamilyAspiration.QiuJin, StringComparison.Ordinal)) return "求真君";
	return value;
}
