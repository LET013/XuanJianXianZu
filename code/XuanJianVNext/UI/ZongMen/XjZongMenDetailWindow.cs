...
internal class XjZongMenDetailWindow : AutoLayoutWindow<XjZongMenDetailWindow>
{
	...
	private void CreateTabBar()
	{
		...
		tabRoles = CreateTabButton(tabBarPanel.transform, "ui/icons/iconOptions", 1, "职介", "查看宗主、峰主、洞天老祖与门人。");
	...
	private void CreateOverviewSection(in XjZongMenUiReadModel model)
	{
		...
		CreateInfoRow(container, "创建者:", string.IsNullOrWhiteSpace(XjZongMenCityData.GetFounderName(currentCity)) ? "-" : XjZongMenCityData.GetFounderName(currentCity));
		CreateInfoRow(container, "当代宗主:", XjZongMenCityData.GetZongZhu(currentCity)?.getName() ?? "-");
		CreateInfoRow(container, "宗门规模:", GetScaleName());
		CreateInfoRow(container, "修士数量:", XjZongMenCityData.GetStoredMemberCount(currentCity).ToString());
		CreateInfoRow(container, "山峰数量:", XjZongMenCityData.GetPeaks(currentCity).Count.ToString());
	}
	...
	{
		List<(int id, string name)> peaks = XjZongMenCityData.GetPeaks(currentCity);
		if (peaks.Count == 0)
		{
	...
	private void CreatePeakCard(AutoHoriLayoutGroup parent, int peakId, string title)
	{
		...
		if (peakId == XjZongMenCityData.SupremePeakId)
		{
			List<Actor> elders = XjZongMenCityData.GetSupremeElders(currentCity);
			representative = elders.Count > 0 ? elders[0] : null;
			vacantText = "暂无老祖";
			populationText = "老祖:" + elders.Count;
		}
		else if (peakId == XjZongMenCityData.MainPeakId)
		{
			representative = XjZongMenCityData.GetZongZhu(currentCity);
			vacantText = "暂无宗主";
			populationText = "门人:" + XjZongMenCityData.GetPeakDiscipleCount(currentCity, peakId);
		}
		else
		{
			representative = XjZongMenCityData.GetPeakFengZhu(currentCity, peakId);
			vacantText = "暂无峰主";
			populationText = "门人:" + XjZongMenCityData.GetPeakDiscipleCount(currentCity, peakId);
		}
	...
	private void CreateRolesSection()
	{
		...
		CreateRoleGridSection("洞天老祖", XjZongMenCityData.GetSupremeElders(currentCity), 6, "ui/SLbiaoqian04", LaoZuCardWidth, LaoZuCardHeight, "ui/ZhiJie05", 0.5f);
		List<Actor> masters = new List<Actor>();
		List<(int id, string name)> peaks = XjZongMenCityData.GetPeaks(currentCity);
		for (int i = 0; i < peaks.Count; i++)
		{
			Actor master = XjZongMenCityData.GetPeakFengZhu(currentCity, peaks[i].id);
			if (master != null && master != XjZongMenCityData.GetZongZhu(currentCity))
			{
				masters.Add(master);
			}
		}
		CreateRoleGridSection("峰主", masters, 3, "ui/SLbiaoqian03", FengZhuCardWidth, FengZhuCardHeight, "ui/ZhiJie04", 0.6f);
		CreateRoleGridSection("门人", XjZongMenCityData.CollectMembers(currentCity), 7, "ui/SLbiaoqian00", MenRenCardWidth, MenRenCardHeight, "ui/ZhiJie01", 0.52f);
	}

	private void CreateZongZhuSection()
	{
		Actor actor = XjZongMenCityData.GetZongZhu(currentCity);
		...
		CreateTextElement(left.transform, "门人:" + XjZongMenCityData.GetStoredMemberCount(currentCity), 6, FontStyle.Normal, TextAnchor.MiddleLeft, new Color(0.7f, 0.75f, 0.85f), new Vector2(55f, 14f));
		...
		CreateTextElement(right.transform, "山峰:" + XjZongMenCityData.GetPeaks(currentCity).Count, 6, FontStyle.Normal, TextAnchor.MiddleRight, new Color(0.6f, 0.5f, 0.9f), new Vector2(55f, 14f));
		CreateTextElement(right.transform, "老祖:" + XjZongMenCityData.GetSupremeElderCount(currentCity), 6, FontStyle.Normal, TextAnchor.MiddleRight, new Color(0.7f, 0.75f, 0.85f), new Vector2(55f, 14f));
		string founder = XjZongMenCityData.GetFounderName(currentCity);
	...
	{
		int count = XjZongMenCityData.GetStoredMemberCount(currentCity);
	...
	private static string FormatGongFaTip(in XjZongMenUiGongFaItem item)
	{
		...
		return item.Grade + "品 · " + (string.IsNullOrWhiteSpace(item.DaoTu) ? "未载道途" : item.DaoTu) + mapped;
	}
	...
	private static string FormatQiuJinFaTip(in XjZongMenUiQiuJinFaItem item)
	{
		...
		return source + " · " + (string.IsNullOrWhiteSpace(item.DaoTu) ? "未载道途" : item.DaoTu);
	}

	private static string FormatCaiQiTip(in XjZongMenUiCaiQiItem item)
	{
		return (string.IsNullOrWhiteSpace(item.DaoTu) ? "未载道途" : item.DaoTu)
			+ " x"
			+ item.Amount
