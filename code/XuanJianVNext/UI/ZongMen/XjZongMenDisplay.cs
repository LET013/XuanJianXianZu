...
{
	internal readonly XjZongMenIdentitySnapshot Identity;
	...
	private XjZongMenUiReadModel(
		XjZongMenIdentitySnapshot identity,
	...
	internal static XjZongMenUiReadModel BuildForActor(Actor actor)
	{
		...
		XjZongMenCityData.TryResolveZongMenCity(identity.ZongMenId, out City zongMenCity);
	...
	{
		XjZongMenIdentitySnapshot identity = XjZongMenAccessor.BuildForCity(city);
		if (!identity.Found || identity.ZongMenId <= 0L)
		{
			return Empty(identity);
		}
	...
	{
		IReadOnlyList<XjZongMenGongFaPavilionEntry> entries = XjZongMenGongFaPavilion.ReadGongFaEntries(zongMenId);
		if (entries.Count == 0)
		{
	...
	{
		IReadOnlyList<XjZongMenGongFaPavilionEntry> entries = XjZongMenGongFaPavilion.ReadQiuJinFaEntries(zongMenId);
		if (entries.Count == 0)
		{
	...
	{
		IReadOnlyList<XjZongMenCaiQiWarehouseEntry> entries = XjZongMenCaiQiWarehouse.ReadCaiQiResources(zongMenId);
		if (entries.Count == 0)
		{
	...
	{
		IReadOnlyList<XjZongMenCaiQiWarehouseEntry> entries = XjZongMenCaiQiWarehouse.ReadCaiQiFaResources(zongMenId);
		if (entries.Count == 0)
		{
	...
	private static XjZongMenUiReadModel Empty(XjZongMenIdentitySnapshot identity)
	{
		return new XjZongMenUiReadModel(
			identity,
	...
	{
		if (!XjZongMenCityData.HasZongMen(city))
		{
			return System.Array.Empty<string>();
		}

		List<string> items = new List<string>(10);
		string founder = XjZongMenCityData.GetFounderName(city);
		string zongZhu = XjZongMenCityData.GetZongZhuName(city);
		...
		items.Add("宗主代数：第" + XjZongMenCityData.GetGeneration(city).ToString(CultureInfo.InvariantCulture) + "代");
		items.Add("门人：" + XjZongMenCityData.GetStoredMemberCount(city).ToString(CultureInfo.InvariantCulture));
		items.Add("山峰：" + XjZongMenCityData.GetPeaks(city).Count.ToString(CultureInfo.InvariantCulture));
		items.Add("洞天：" + (XjZongMenCityData.HasDongTianPeak(city) ? "已立" : "未立"));
		int elders = XjZongMenCityData.GetSupremeElderCount(city);
		if (elders > 0)
		{
			items.Add("洞天老祖：" + elders.ToString(CultureInfo.InvariantCulture));
		}

		XjZongMenCityTraitState traitState = XjZongMenCityTraitAccessor.BuildState(city);
...
internal static class XjZongMenDisplayFormatter
{
	...
	private static XjZongMenDisplaySection BuildIdentitySection(in XjZongMenIdentitySnapshot identity)
	{
		if (!identity.Found || identity.ZongMenId <= 0L)
		{
		...
		{
			items.Add((identity.ActorId > 0L ? "入门：" : "创立：") + identity.JoinYear.ToString(CultureInfo.InvariantCulture) + "年");
		}
	...
	private static string FormatSource(string actorName, int year)
	{
		...
		if (year > 0)
		{
			parts.Add(year.ToString(CultureInfo.InvariantCulture) + "年");
		}
