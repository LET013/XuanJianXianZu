...
internal static class XjAlchemyUiModel
{
	...
	internal static XjAlchemyUiItem[] BuildPills(XjAlchemyInventorySnapshot snapshot)
	{
			...
			.Where(batch => batch.Quantity > 0).GroupBy(batch => batch.PillId, StringComparer.Ordinal))
		{
			...
			bool fuQiProjectEffect = pill.EffectType == XjAlchemyEffectType.FuQiBodyProjectYears
				|| pill.EffectType == XjAlchemyEffectType.FuQiPerfectionProjectYears
				|| pill.EffectType == XjAlchemyEffectType.FuQiJinXingNurtureYears;
			...
			string effectMode = lastingEffect
				...
				: fuQiProjectEffect
					? "对应服气修持工程存在时自动服用，立即缩短完成年份"
				: immediateEffect
					? "即时生效；服丹动作持续6秒，不是属性持续时间"
					: "仅作用于下一次对应判定，判定时立即消耗";
			string details = "适用修法：" + FormatPath(pill.PathAffinity)
				+ "\n药效：" + XjAlchemyText.PillShortDescription(group.Key)
				+ "\n库存：" + total + "枚\n最近失效：" + XjChronology.FormatYear(nearestExpiry) + "\n生效方式：" + effectMode;
	...
	internal static XjAlchemyUiItem[] BuildRecipes(XjAlchemyInventorySnapshot snapshot)
	{
		...
		for (int i = 0; i < snapshot.Recipes.Count; i++)
		{
			...
			string details = "来源：" + sourceText
				+ "\n撰录年份：" + XjChronology.FormatYear(holding.AcquiredYear)
				+ "\n适用修法：" + FormatPath(recipe.PathAffinity)
	...
	private static string FormatPath(XjAlchemyCultivationPath path)
	{
		return path switch
		{
	...
	internal static void CreateIconCell(Transform parent, in XjAlchemyUiItem item, Font font, string objectName, bool blockWorldInput = false)
	{
		...
		XjIconShelfRenderer.CreateIconSlot(parent, objectName, sprite, fallback, Math.Max(1, item.Count),
			...
			blockWorldInput: blockWorldInput);
	}
