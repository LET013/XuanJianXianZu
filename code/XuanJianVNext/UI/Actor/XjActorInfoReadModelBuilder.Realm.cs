...
{		private static string BuildStageBonusSummary(string realmId, float zhenYuan, int xianJiCount, int jinDanYiXiang)
		...
		private static string BuildEraBonusSummary(string daoTu)
		{
			if (!XjEraBonusService.TryGetProfileForDaoTu(daoTu, out XjEraBonusProfile profile))
			{
				return "<color=#808080>无</color>";
			}
	
			bool active = XjEraBonusService.IsActiveForDaoTu(daoTu);
		...
		private static string BuildMingShuBonusSummary(string realmId, float congenital, float acquired)
		{
			...
			return "\n    " + realmName + "->" + targetRealmName + "突破加成：" + FormatPercent0(bonus)
				+ "\n    有效命数 " + FormatMingShuDisplay(effectiveDisplayValue) + "-" + FormatMingShuDisplay(cap);
		}
		...
		private static string FormatPercent0(float value)
		{
			return Math.Round(Math.Max(0f, value) * 100f).ToString("0", CultureInfo.InvariantCulture) + "%";
		}
