...
internal static class XjGuoWeiQuanBingWindow
{
	...
	private static void CreateHeader(Transform parent)
	{
		...
		CreateHeaderText(h.transform, "展示当前持有、夺得权柄、永久丢失、果位封锁与果位钟爱",
	...
	internal sealed class DaoTuOverview { internal string DaoTu; internal List<string> DetailLines; }

	internal static List<DaoTuOverview> BuildViews()
	{
		...
		var lineages = XjDaoLineageStateRegistry.ReadAllLineages();
		...
		{
			if (!string.IsNullOrWhiteSpace(lineages[i]?.DaoTu)) daoTus.Add(lineages[i].DaoTu.Trim());
		}
		// 永久封锁果位即使尚无现世持有人，也必须在权柄总览中明确显示。
		daoTus.Add("谪炁");
		daoTus.Add("下仪");
		daoTus.Add("保木");
		// 渊照空证前太阴/坎水要让玩家明确知道“正果有人”，但绝不披露持位者。
		// 空证事务完成后再公开渊照道途并恢复二果正常公开状态。
		if (!XjYuanZhaoKongZhengEvent.IsTriggered)
		{
			daoTus.Add(XjYuanZhaoKongZhengEvent.SourceTaiYin);
			daoTus.Add(XjYuanZhaoKongZhengEvent.SourceKanShui);
		}
		else
		{
			daoTus.Add(XjYuanZhaoKongZhengEvent.DaoTu);
		}
		if (XjHongXiaLuoXiaEvent.IsTriggered)
		{
			daoTus.Add(XjHongXiaLuoXiaEvent.DaoTu);
			daoTus.Add(XjHongXiaLuoXiaEvent.SourceWuTu);
		}
		...
		for (int d = 0; d < orderedDaoTus.Count; d++)
		{
			...
			var seized = new List<string>();
			var historical = new List<string>();
			var lost = new List<string>();
			var locks = new List<string>();
			var favored = new List<string>();
			var shiChanges = new List<string>(XjShiFruitPositionLockSystem.BuildDaoTuStatusSummaries(daoTu));
			if (string.Equals(daoTu, "谪炁", System.StringComparison.Ordinal))
			{
				locks.Add("果位未显，暂不可求证");
				locks.Add("第一余位闭锁，暂不可求证");
			}
			else if (string.Equals(daoTu, "下仪", System.StringComparison.Ordinal))
			{
				locks.Add("果位未显，暂不可求证");
			}
			else if (string.Equals(daoTu, "保木", System.StringComparison.Ordinal))
			{
				locks.Add("果位未显，暂不可求证");
			}
			if (XjYuanZhaoFruitSealPolicy.TryBuildVisibleLockSummary(daoTu, XjYearTracker.CurrentYear, out string yuanZhaoSeal))
			{
				locks.Add(yuanZhaoSeal);
			}
			if (string.Equals(daoTu, XjHongXiaLuoXiaEvent.SourceWuTu, System.StringComparison.Ordinal))
			{
				locks.Add(XjHongXiaLuoXiaEvent.IsTriggered
					? "戊土一席闰位随落霞显世；该席不向玩家开放"
					: "戊土一席闰位未显；该席不向玩家开放");
			}
			if (XjHongXiaLuoXiaEvent.IsTriggered
				&& string.Equals(daoTu, XjHongXiaLuoXiaEvent.DaoTu, System.StringComparison.Ordinal))
			{
				locks.Add("虹霞正果已显世；此位不向玩家开放");
			}
			...
			for (int i = 0; i < states.Count; i++)
			{
				...
				{
					locks.Add(ResolveActorName(state.ActorName) + "封锁至" + XjChronology.FormatYear(state.LockUntilYear));
				}
			...
			for (int i = 0; i < lineages.Count; i++)
			{
				...
				if (lineage?.Authorities == null
					|| !string.Equals(lineage.DaoTu?.Trim(), daoTu, System.StringComparison.Ordinal)) continue;
				for (int a = 0; a < lineage.Authorities.Count; a++)
				{
					...
					if (authority == null || string.IsNullOrWhiteSpace(authority.Name)
						...
						|| string.Equals(authority.SourceDaoTu.Trim(), daoTu, System.StringComparison.Ordinal)) continue;
					string status = (authority.Status ?? string.Empty).Trim();
					if (!string.Equals(status, "借", System.StringComparison.Ordinal)
						&& !string.Equals(status, "易", System.StringComparison.Ordinal)) continue;
					string suffix = string.Equals(status, "借", System.StringComparison.Ordinal) ? "（待融）" : "（已融）";
					seized.Add(authority.Name.Trim() + "←" + authority.SourceDaoTu.Trim() + suffix);
				}
			}
			...
			for (int i = 0; i < lostRecords.Count; i++)
			{
				...
				string year = record.Year > 0 ? "（" + XjChronology.FormatYear(record.Year) + "）" : string.Empty;
				lost.Add(record.Authority + "→" + target + year);
			}

			if (gained.Count == 0 && seized.Count == 0 && historical.Count == 0 && lost.Count == 0 && locks.Count == 0
				&& favored.Count == 0 && shiChanges.Count == 0) continue;
			...
			string authorityLore = XjAuthorityLoreCatalog.BuildSummary(daoTu);
			if (!string.IsNullOrWhiteSpace(authorityLore)) lines.Add("权柄释义：" + authorityLore);
			if (gained.Count > 0) lines.Add("当前持有：" + string.Join("；", gained));
			if (seized.Count > 0) lines.Add("夺得权柄：" + string.Join("；", seized));
			if (historical.Count > 0) lines.Add("历代持有：" + string.Join("；", historical));
			if (lost.Count > 0) lines.Add("永久丢失：" + string.Join("；", lost));
			if (locks.Count > 0) lines.Add("果位封锁：" + string.Join("；", locks));
			if (shiChanges.Count > 0) lines.Add("释土异变：" + string.Join("；", shiChanges));
	...
	private static string FormatHistoricalHolder(in XjGuoWeiQuanBingState state)
	{
		...
		string acquired = state.AcquiredYear > 0 ? XjChronology.FormatYear(state.AcquiredYear) + "得位，" : string.Empty;
		string released = state.ReleasedYear > 0 ? XjChronology.FormatYear(state.ReleasedYear) : string.Empty;
	...
	private static string FormatAuthoritySnapshot(in XjGuoWeiQuanBingState state, bool historical)
	{
		...
		AddAuthorityPart(parts, "融入", state.ForeignQuanBing);
	...
	{
		return XjDisplayNameSanitizer.ReleaseReason(value);
	}
	...
	private static void UpdateRow(GuoWeiQuanBingRowRefs rf, int index, float y, float rh, DaoTuOverview v)
	{
		...
		rf.daoTuText.text = XjDisplayNameSanitizer.GameTerm(v.DaoTu, "未定道途");
		rf.detailsText.text = CleanDetailLines(v.DetailLines);
		rf.detailsText.rectTransform.sizeDelta = new Vector2(360f, Mathf.Max(18f, rh - 12f));
	}

	private static string CleanDetailLines(IReadOnlyList<string> lines)
	{
		if (lines == null || lines.Count == 0) return string.Empty;
		List<string> clean = new List<string>(lines.Count);
		for (int i = 0; i < lines.Count; i++)
		{
			string line = XjDisplayNameSanitizer.Clean(lines[i], string.Empty);
			if (!string.IsNullOrWhiteSpace(line)) clean.Add(line);
		}
		return string.Join("\n", clean);
	}
