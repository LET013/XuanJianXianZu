...
internal static class XjBloodlineReadModel
{
	...
	private static void PopulateFamilyDetails(XjBloodlineFamilyItem item)
	{
		...
		XjFamilyChronicleDisplayFormatter.ChronicleUiEntry[] chronologyEntries =
			XjFamilyChronicleDisplayFormatter.BuildEntriesForFamily(item.FamilyStableId);
		for (int i = 0; i < chronologyEntries.Length; i++)
		{
			int displayYear = XjChronology.ToXuanJianYear(chronologyEntries[i].Year);
			if (displayYear > item.RecentChronicleYear) item.RecentChronicleYear = displayYear;
		}
	}
	...
	private static string BuildMemberCard(in XjFamilyMemberDisplayItem member, in XjActorInfoReadModel info)
	{
		...
		string daoTu = XjDisplayNameSanitizer.GameTerm(info.DaoTu, "未定道途");
		string aptitude = XjDisplayNameSanitizer.AptitudeName(info.XjZz);
	...
	private static string BuildBloodlineQualityText(long actorId)
	{
		...
		string origin = string.IsNullOrWhiteSpace(state.OriginDaoTu)
			? string.Empty
			: " - 源道途：" + XjDisplayNameSanitizer.GameTerm(state.OriginDaoTu, "未知道途");
		...
		string talent = state.ExtraTalentInheritance > 0
			...
			: string.Empty;
		string quality = XjDisplayNameSanitizer.GameTerm(state.Quality, "血脉未定");
		return "血脉：" + quality + " - " + state.Concentration.ToString(CultureInfo.InvariantCulture)
			+ "%" + generation + origin + source + talent;
	}

	private static string FormatBloodlineSource(string source)
	{
		return XjDisplayNameSanitizer.BloodlineSource(source);
	}
	...
	{
		// 使用统一境界顺序，确保黄冠/真人/真君羽士与筑基/紫府/金丹
		// 在血脉代表人物、排序和家族概览中保持同阶权重。
		return Math.Max(0, XjRealmHelper.GetOrder(realmId));
	}
...
internal static class XjBloodlineFormatter
{
	...
	private static void AppendSortSummary(StringBuilder builder, IReadOnlyList<XjBloodlineFamilyItem> items)
	{
		...
		AppendFamilyIndex(builder, "最近纪事索引", byChronicle, item => item.RecentChronicleYear > 0 ? "玄鉴历" + item.RecentChronicleYear.ToString(CultureInfo.InvariantCulture) + "年" : "暂无纪事");
...
internal static class XjBloodlineWindow
{
	...
	Text line1 = CreateText(card.transform, "Line1",
		"道途：" + XjDisplayNameSanitizer.GameTerm(item.DominantDaoTu, "未定")
	...
	{
		return "道途：" + XjDisplayNameSanitizer.GameTerm(item.DominantDaoTu, "未定")
