...
private static void RefreshKingdomFilters(IReadOnlyList<Actor> actors)
	{
		...
		for (int i = 0; i < actors.Count; i++)
		{
			...
			if (kingdom?.data == null || !seen.Add(kingdom.data.id)) continue;
...
private static GameObject CreateKingdomFilterButton(Kingdom kingdom)
	{
		...
		Image image = CreateImage("Kingdom_" + kingdom.data.id.ToString(CultureInfo.InvariantCulture), kingdomFilterContainer, background, mainColor, new Vector2(25, 25));
...
private static GameObject CreateAssetFilterButton(ActorAsset asset)
	{
		...
		Image image = CreateImage("Asset_" + asset.id, assetFilterContainer, icon, Color.white, new Vector2(25, 25));
...
private static void RefreshDynamicGridLayout(Transform gridTransform)
	{
		...
		int rows = Mathf.Max(1, Mathf.CeilToInt(gridTransform.childCount / (float)columns));
...
private static GameObject CreateTraitFilterButton(ActorTrait trait)
	{
		...
		Image image = CreateImage("Trait_" + trait.id, traitFilterContainer, icon, Color.white, new Vector2(25, 25));
		image.preserveAspect = true;
		Button button = image.gameObject.AddComponent<Button>();
		button.targetGraphic = image;
		button.transition = Selectable.Transition.None;
		...
		image.gameObject.AddComponent<XjRankTooltipTrigger>().TooltipText = traitName;
		return image.gameObject;
	}
...
private static void RefreshSelectedFilters()
	{
		...
		activeFilters.RemoveAll(f => f == null || f.ToBeRemoved);
...
private static GameObject CreateSelectedFilterButton(XjRankFilterSetting filter)
	{
		Image border = CreateImage(filter.Id, selectedFiltersContainer, GetSafeSprite("ui/special/windowInnerSliced"), filter.GetTypeColor(), new Vector2(25, 25));
...
private static void ClearAllFilters()
	{
		...
		try
		{
			...
			realmDropdown?.SetValueWithoutNotify(0);
			filterDropdown?.RefreshShownValue();
			realmDropdown?.RefreshShownValue();
		}
		finally
		{
