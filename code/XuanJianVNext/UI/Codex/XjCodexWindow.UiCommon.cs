...
{	
		...
		{
			DrawPageHeader("玄鉴正在照录", "当前卷页正在整理天下账本。");
			GUILayout.BeginVertical(GUI.skin.box);
			GUILayout.Label("切换卷页后会自动照录，无需等待世界继续流转。若刚刚载入大型存档，可稍候片刻。");
			GUILayout.Space(8f);
			if (GUILayout.Button("立即重照", GUILayout.Width(160f), GUILayout.Height(38f)))
			{
...
private static void EnsureTexturesAndStyles()
		{
			...
			if (_largeLabel == null)
			{
				...
				_archiveSelectorButton = new GUIStyle(_largeButton)
				{
					// 名录按钮按文本实际高度计算；额外上下内边距保证三行文字不贴边、不截底。
...
private void NormalizeWindowRect()
		{
			float horizontalMargin = Screen.width >= 1500 ? 40f : Screen.width >= 1100 ? 28f : 14f;
			float verticalMargin = Screen.height >= 900 ? 32f : 16f;
			float availableWidth = Mathf.Max(320f, Screen.width - horizontalMargin * 2f);
			float availableHeight = Mathf.Max(360f, Screen.height - verticalMargin * 2f);
			_windowRect.width = Mathf.Min(1660f, availableWidth);
			_windowRect.height = Mathf.Min(1240f, availableHeight);
			_windowRect.x = Mathf.Clamp(_windowRect.x, horizontalMargin,
				Mathf.Max(horizontalMargin, Screen.width - _windowRect.width - horizontalMargin));
			_windowRect.y = Mathf.Clamp(_windowRect.y, verticalMargin,
				Mathf.Max(verticalMargin, Screen.height - _windowRect.height - verticalMargin));
		}
...
private static void CreateOverlayBlocker()
		{
			...
			catch (System.Exception xjCaught425) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/UI/Codex/XjCodexWindow.UiCommon.cs:425", xjCaught425); }
		}

private static void DestroyOverlayBlocker()
		{
			if (_overlayBlocker == null) return;
			try { GameObject.Destroy(_overlayBlocker); } catch (System.Exception xjCaught431) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/UI/Codex/XjCodexWindow.UiCommon.cs:431", xjCaught431); }
...
private void HandleUiException(Exception ex)
		{
			...
			DestroyOverlayBlocker();
				try { WorldTip.showNow("玄鉴仙鉴绘制异常，已自动关闭。请查看 Player.log 中的 [玄鉴][玄鉴仙鉴]。", false, "top", 5f); } catch (System.Exception xjCaught442) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/UI/Codex/XjCodexWindow.UiCommon.cs:442", xjCaught442); }
		}
private static bool TryFocusMapLayerTarget(long cityId, int tileX, int tileY)
	{
		...
		if (cityId > 0L && TryResolveCityForFocus(cityId, out City city) && TryExtractMapPosition(city, 0, out Vector3 cityPosition))
		{
			...
			catch (System.Exception xjCaught454) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/UI/Codex/XjCodexWindow.UiCommon.cs:454", xjCaught454); }
		}
...
private static bool TryExtractMapPosition(object source, int depth, out Vector3 position)
	{
		// UI asks only for a map position. Native member/method traversal is owned
		// by Interop and cached by runtime type.
		position = default;
		return source != null && XjNativeMapPositionInterop.TryResolve(source, out position);
	}
