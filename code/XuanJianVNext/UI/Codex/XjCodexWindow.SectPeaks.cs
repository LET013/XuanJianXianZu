...
internal static void ShowSectPeaks(long sectId)
	{
		...
		_instance._sectPeakDetailSectId = sectId;
...
private void DrawSectPeakDetailButton(XjCodexSectItem sect, params GUILayoutOption[] options)
	{
		...
		{
			_sectPeakDetailSectId = sect.SectId;
...
private void DrawSectPeakDetailPage(XjCodexSnapshot snapshot)
	{
		...
		DrawOverviewPill("峰位", sect.PeakCount + "/" + Math.Max(0, sect.PeakCapacity), "#9CD7FF", GUILayout.Width(125f));
		DrawOverviewPill("修士", sect.CultivatorCount.ToString(), "#CFC7B2", GUILayout.Width(125f));
		DrawOverviewPill("高境", "紫府" + sect.ZiFuCount + " 真君" + sect.JinDanCount, sect.JinDanCount > 0 ? "#FFD37A" : "#B7A7FF", GUILayout.Width(175f));
		GUILayout.EndHorizontal();
		GUILayout.Space(10f);
...
private void DrawSectPeakDetail(XjCodexSectItem sect, XjCodexSectPeakItem peak)
	{
		...
		DrawTag(masterLabel + " · " + Empty(peak.PeakMasterName, "未任命"), string.IsNullOrWhiteSpace(peak.PeakMasterName) ? "#888888" : color);
		DrawTag("门下 " + Math.Max(0, peak.MemberCount) + " 人", "#CFC7B2");
		DrawTag("紫府 " + Math.Max(0, peak.ZiFuCount), "#B7A7FF");
		...
		{
			GUILayout.BeginHorizontal();
			DrawSectPeakMemberCard(master, 650f, 112f);
			GUILayout.FlexibleSpace();
			GUILayout.EndHorizontal();
		}
		else DrawEmptyCard(string.IsNullOrWhiteSpace(peak.PeakMasterName) ? "本峰尚未任命" + masterLabel + "。" : masterLabel + "记录暂未进入成员快照。", "#777777");
...
private void DrawSectPeakMemberCard(XjCodexSectPeakMemberItem member, float width = 310f, float height = 108f)
	{
		...
		GUILayout.EndHorizontal();
		GUILayout.Label("<color=" + color + "><b>" + Rich(Empty(member.Realm, "未载")) + "</b></color>　"
