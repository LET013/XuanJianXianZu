...
private void DrawCenturyAnnalsCard(XjCodexCenturyAnnalsItem item)
	{
		...
		GUILayout.Label("<color=grey>成卷于 " + item.GeneratedYear + " 年</color>", GUILayout.Width(145f));
		GUILayout.EndHorizontal();
		...
		GUILayout.Space(5f);
		GUILayout.BeginHorizontal();
		DrawOverviewPill("入谱家族", item.FamilyCount.ToString(), "#A7E08A", GUILayout.Width(180f));
		DrawOverviewPill("宗门变迁", item.SectCount.ToString(), "#FFD37A", GUILayout.Width(180f));
		DrawOverviewPill("代表人物", item.ActorCount.ToString(), "#B7A7FF", GUILayout.Width(180f));
		DrawOverviewPill("境界分布", (item.RealmStatistics?.Count ?? 0).ToString(), "#FF8877", GUILayout.Width(180f));
		DrawOverviewPill("道途纪要", (item.DaoSummaries?.Count ?? 0).ToString(), "#9CD7FF", GUILayout.Width(180f));
		DrawOverviewPill("传承器物", (item.ArtifactSummaries?.Count ?? 0).ToString(), "#B7A7FF", GUILayout.Width(180f));
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();
...
private void DrawCenturyAnnalsCategoryButtons()
	{
		...
		for (int i = 0; i < CenturyAnnalsCategories.Length; i++)
		{
			...
			GUI.backgroundColor = string.Equals(_centuryAnnalsCategory, category, StringComparison.Ordinal)
				? new Color(0.25f, 0.43f, 0.34f)
				: new Color(0.20f, 0.23f, 0.22f);
			if (GUILayout.Button("◇ " + category, GUILayout.Height(38f), GUILayout.Width(132f)))
			{
...
private void DrawCenturyFamiliesSection(IReadOnlyList<XjCodexCenturyFamilyItem> families)
	{
		...
		for (int i = 0; i < families.Count; i++)
		{
			...
			DrawMiniStat("紫府", Math.Max(0, family.ZiFuCount).ToString(), "#B7A7FF", GUILayout.Width(105f));
			DrawMiniStat("金丹", Math.Max(0, family.JinDanCount).ToString(), "#FFD37A", GUILayout.Width(105f));
...
private static string BuildCenturyFamilyRealmText(XjCodexCenturyFamilyItem family)
	{
		...
		{
			parts.Add("紫府：" + family.ZiFuCount.ToString());
		}
		if (family.JinDanCount > 0)
		{
			parts.Add("金丹：" + family.JinDanCount.ToString());
		}
...
private void DrawCenturyDaoFilters()
	{
		GUILayout.BeginVertical(GUI.skin.box);
		GUILayout.Label("<b>道途筛选</b> <color=grey>按九大道途体系查看本世纪纪要。</color>");
		GUILayout.BeginHorizontal();
		for (int i = 0; i < CenturyDaoFilters.Length; i++)
		{
			...
			if (GUILayout.Button(filter, GUILayout.Width(102f), GUILayout.Height(38f))) _centuryDaoFilter = filter;
			GUI.backgroundColor = old;
		}
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();
...
private void DrawCenturyActorsSection(IReadOnlyList<XjCodexCenturyActorItem> actors)
	...
	private void DrawCenturyActorCard(XjCodexCenturyActorItem actor)
	{
		...
		GUILayout.FlexibleSpace();
		GUILayout.Label("<color=" + color + "><b>" + Rich(Empty(actor.Realm, "境界未载")) + "</b></color>", GUILayout.Width(110f));
		GUILayout.EndHorizontal();
...
private void DrawFamilyChronicleButton(XjCodexFamilyItem item, params GUILayoutOption[] options)
	{
		...
		{
			ShowHistoryFamily(item.FamilyId);
			_focusMessage = string.Empty;
		}
		GUI.enabled = oldEnabled;
	}
...
private void DrawFamilyTreasureButton(XjCodexFamilyItem item, params GUILayoutOption[] options)
	{
		...
		{
			_familyWarehouseDetailFamilyId = item.FamilyId;
