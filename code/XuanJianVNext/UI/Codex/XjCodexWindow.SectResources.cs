...
private void DrawSectResourceViewSelector(XjCodexSnapshot snapshot)
	{
		...
		GUILayout.Label("展示框", GUILayout.Width(80f));
...
private void DrawSectResourceDetailPage(XjCodexSnapshot snapshot, XjCodexSectItem sect, string view)
	{
		...
		DrawOverviewPill("当前仓库", view, SectResourceViewColor(view), GUILayout.Width(145f));
		DrawOverviewPill("库存数量", CountSectResources(snapshot.SectResources, sect.SectId, view).ToString(), SectResourceViewColor(view), GUILayout.Width(135f));
		DrawOverviewPill("宗门层次", TranslateSectStatus(sect.Status, sect.JinDanCount), "#B7A7FF", GUILayout.Width(145f));
		GUILayout.EndHorizontal();
		GUILayout.Space(10f);
...
private void DrawSectResourceSectCard(IReadOnlyDictionary<string, int> resourceCounts, XjCodexSectItem sect, int totalCount)
	{
		...
		GUILayout.BeginHorizontal();
		GUILayout.Label("<b><size=22>" + Rich(sect.Name) + "</size></b>", GUILayout.Width(260f));
		DrawMiniStat("山门", Empty(sect.CapitalCityName, "未定"), "#9CD7FF", GUILayout.Width(170f));
		DrawMiniStat("层次", TranslateSectStatus(sect.Status, sect.JinDanCount), color, GUILayout.Width(145f));
		DrawMiniStat("总藏", totalCount.ToString(), "#FFD37A", GUILayout.Width(120f));
...
private void DrawSectResourceWarehouseTile(XjCodexSectItem sect, string view, int count)
	{
		...
		{
			_sectResourceDetailSectId = sect.SectId;
			_sectResourceDetailView = view;
			_sectResourceView = view;
			_scrollPosition = Vector2.zero;
		}
...
private void DrawSectResourceDetailGrid(IReadOnlyList<XjCodexSectResourceItem> resources, long sectId, string view)
	{
		...
		for (int i = 0; i < shown; i++)
		{
			...
			if (i % columns == columns - 1 || i == shown - 1) GUILayout.EndHorizontal();
		}
...
private void DrawSectResourceItemTile(SectResourceDisplayGroup item, string view)
	{
		...
		string canonTag = CanonTagForResource(view, item.Name, item.Kind, item.DaoTu);
...
private static string FormatSectResourceLine(XjCodexSectResourceItem item)
	{
		...
		if (!string.IsNullOrWhiteSpace(item.DaoTu)) text += "  道途：" + Rich(item.DaoTu.Trim());
		if (item.Grade > 0) text += "  品阶：" + Rich(FormatGongFaGrade(item.Grade));
		if (item.Amount > 1) text += "  数量：" + item.Amount;
		if (item.Year > 0) text += "  入库：" + item.Year + "年";
...
private static XjCodexSectItem FindSectById(IReadOnlyList<XjCodexSectItem> sects, long sectId)
	{
		if (sects == null || sectId <= 0L) return null;
