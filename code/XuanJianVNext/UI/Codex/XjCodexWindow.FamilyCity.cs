...
{		private void DrawFamilyChronicleDetail(XjCodexSnapshot snapshot)
		{
			...
			for (int i = events.Count - 1; i >= 0; i--)
			{
				...
				GUILayout.Label("玄鉴历" + entry.Timestamp + "年", GUILayout.Width(130f));
...
private static XjCodexFamilyItem FindFamily(XjCodexSnapshot snapshot, long familyId)
		{
			if (snapshot?.Families == null || familyId <= 0L)
			{
				return null;
			}
...
private bool MatchesFamilyRealmFilter(XjCodexFamilyItem item)
		{
			...
			string realm = item.HighestRealm ?? string.Empty;
			if (filter == "金丹")
			{
				return ContainsRealm(realm, "金丹") || ContainsRealm(realm, "神丹");
			}
			return ContainsRealm(realm, filter);
		}
