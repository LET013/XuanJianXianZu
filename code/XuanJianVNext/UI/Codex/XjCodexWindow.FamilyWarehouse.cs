...
private void DrawFamilyWarehouseDetailPage(XjCodexSnapshot snapshot)
	{
		...
		GUILayout.Space(8f);
		GUILayout.BeginHorizontal();
		DrawOverviewPill("家族", family.Name, "#FFD37A", GUILayout.Width(220f), GUILayout.Height(82f));
		if (family.IsMigrationBranch)
		{
			DrawOverviewPill("来源主家", Empty(family.SourceFamilyName, "未载"), "#FFAA66", GUILayout.Width(170f), GUILayout.Height(82f));
		}
		DrawOverviewPill("当前仓库", view, SectResourceViewColor(view), GUILayout.Width(145f), GUILayout.Height(82f));
		DrawOverviewPill("库存数量", CountFamilyWarehouseItemsCached(family.FamilyId, view).ToString(), SectResourceViewColor(view), GUILayout.Width(135f), GUILayout.Height(82f));
		if (view == "丹药")
		{
			CountFamilyPillsAndRecipes(family.FamilyId, out int pillCount, out int recipeCount);
			DrawOverviewPill("成丹库存", pillCount.ToString(), pillCount > 0 ? "#A7E08A" : "#888888", GUILayout.Width(125f), GUILayout.Height(82f));
			DrawOverviewPill("丹方收录", recipeCount.ToString(), "#FFD37A", GUILayout.Width(125f), GUILayout.Height(82f));
		}
		DrawOverviewPill("最高境界", Empty(family.HighestRealm, "未载"), "#B7A7FF", GUILayout.Width(145f), GUILayout.Height(82f));
		GUILayout.EndHorizontal();
		GUILayout.Space(10f);
