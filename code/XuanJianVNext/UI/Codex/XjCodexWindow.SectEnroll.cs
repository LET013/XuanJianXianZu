...
internal sealed partial class XjCodexWindow
{
	...
	private void DrawSectEnrollmentWizard(XjCodexSnapshot snapshot)
	{
		...
		bool eligible = XjCultivationEligibility.CanReceiveXuanJianContent(actor)
		...
		if (!eligible)
		{
			DrawEmptyCard("只有人类、精灵、矮人、兽人中的合法玄鉴修士，且非龙属、非阴司化身，才可以收入宗门。", "#FFAA66");
			return;
		}
		...
		if (_sectEnrollSectId <= 0L)
		{
			...
			for (int i = 0; i < snapshot.Sects.Count; i += 3)
			{
				...
				for (int j = 0; j < 3 && i + j < snapshot.Sects.Count; j++)
				{
					...
					GUILayout.Label("<color=grey>宗主 " + Rich(ClampInlineText(Empty(sect.SovereignName, "待继任"), 24)) + "　·　真人 " + sect.ZiFuCount + "　·　真君 " + sect.JinDanCount + "</color>");
	...
	private bool TryEnrollActorIntoSect(Actor actor, XjCodexSectItem sect, int peakId, out string message)
	{
		...
		if (!XjCultivationEligibility.CanReceiveXuanJianContent(actor)
			|| !XjCultivationEligibility.HasCultivationAptitudeTrait(actor)
			|| XjLongShuSystem.IsLongShu(actor) || XjYinSiTraitLifecycle.IsYinSi(actor))
		{ message = "该角色不符合入宗条件。"; return false; }
		if (!XjSectCityData.TryResolveZongMenCity(sect.SectId, out City city) || city?.data == null)
		{ message = "未找到该宗门山门。"; return false; }

		int targetPeakId = ResolveEnrollPeakId(city, peakId);
		string actorName = SafeActorName(actor);
		string sectName = string.IsNullOrWhiteSpace(sect.Name) ? XjSectCityData.GetZongMenName(city) : sect.Name.Trim();
		string peakName = XjSectCityData.GetPeakName(city, targetPeakId);
		int currentYear = XjSectCityData.GetCurrentYearOrZero();
		XjSectIdentitySnapshot identity = XjSectIdentityReader.BuildIdentity(actor);
		...
		bool changed = XjSectMembershipService.AssignDisciple(city, targetPeakId, actor, currentYear, PlayerEnrollReason);
		if (!changed && !XjSectCityData.IsMember(city, actor))
		{ message = "收入失败：该角色不符合入宗条件。"; return false; }

		XjSectCultivatorCityIndex.Observe(actor);
		XjActorInfoPanelRenderer.Invalidate();
		XjCodexSnapshotPublisher.MarkDirty(XjCodexDirtyFlags.World | XjCodexDirtyFlags.Sect | XjCodexDirtyFlags.Family);
		XjSectIdentitySnapshot assigned = XjSectIdentityReader.BuildIdentity(actor);
	...
	{
		if (city?.data == null || requestedPeakId == int.MinValue) return XjSectCityData.MainPeakId;
		return XjSectCityData.ReadPeakIds(city).Contains(requestedPeakId) ? requestedPeakId : XjSectCityData.MainPeakId;
	}
	...
	private static void TryShowSectEnrollTip(string message)
	{
		...
		catch (System.Exception xjCaught227) { XuanJianVNext.Core.XjExceptionDiagnostics.Report("code/XuanJianVNext/UI/Codex/XjCodexWindow.SectEnroll.cs:227", xjCaught227); }
	}
}
