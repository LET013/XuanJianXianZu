...
internal sealed partial class XjCodexWindow
{
	private static void DrawYaoShuGreatSagePage()
	{
		...
		GUILayout.Label("十二果位映照 · 大圣身灭后静候150年 · 妖民走原生繁衍，灭绝后低频再点化", GUI.skin.label);
		...
		for (int i = 0; i < items.Count; i++)
		{
			...
			GUILayout.Label("道途：" + item.DaoTu + "　映照果位：" + item.Fruit + "　神通：【" + item.Skill + "】");
			GUILayout.Label("对应妖民：" + item.YaoMin + "　现存：" + item.YaoMinPopulation + "　历次化生：" + item.ManifestationCount + "　最近化生：" + FormatYaoShuYear(item.LastManifestationYear));
			if (!item.Alive)
			{
